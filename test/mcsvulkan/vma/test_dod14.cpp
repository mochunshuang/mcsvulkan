...
namespace mesh
{
    ...
    { // 计算包围盒和左上角坐标
        ...
        return glm::vec3(
            minPos.x, minPos.y,
            maxPos
                .z); // Vulkan NDC 中 minY 是顶部 //diff: [test_dod14] BUG的来源。卧槽了，坐标系太狠了。
    }
...
namespace camera
{
    ...
    struct VulkanNDCConfig
    {
        ...
        */
                                             ...
                                             float top, float zNear, float zFar) noexcept
        {
            ...
            if constexpr (H == Handedness::RightHanded)
            {
    ...
    static_assert(VulkanNDCConfig::makeOrtho<Handedness::LeftHanded>(-1, 1, -1, 1, 0,
                                                                     1) == glm::mat4(1));
    ...
    class ViewMatrixObject
    {
      private:
        ...
        static constexpr glm::vec3 LOCAL_FORWARD = (H == Handedness::RightHanded)
      ...
      public:
        ...
        constexpr const glm::quat &getOrientation() const noexcept
        {
        ...
        constexpr glm::vec3 getForward() const noexcept
        {
                                                 ...
                                                 const glm::vec3 &up)
        {
            ...
            return ViewMatrixObject(eye, H == Handedness::RightHanded
    ...
    // -------- 为了方便使用，定义别名 --------
    using RightHandedView = ViewMatrixObject<Handedness::RightHanded>;
    using LeftHandedView = ViewMatrixObject<Handedness::LeftHanded>;
    ...
    class ProjectionMatrix<H, NDCConfig, ProjectionMode::Perspective>
    {
      private:
      ...
      public:
        ...
        // -------- Getter --------
        constexpr float getFov() const noexcept
        {
            return m_fovDeg;
        }
    ...
    // -------- 正交模式特化（只包含正交参数）--------
    template <Handedness H, typename NDCConfig>
    class ProjectionMatrix<H, NDCConfig, ProjectionMode::Orthographic>
    {
      private:
    ...
    using PerspectiveProjection =
        ProjectionMatrix<H, NDCConfig, ProjectionMode::Perspective>;

    template <Handedness H, typename NDCConfig>
    using OrthographicProjection =
        ProjectionMatrix<H, NDCConfig, ProjectionMode::Orthographic>;

    // 具体到 Vulkan
    using VulkanPerspectiveProjection =
        PerspectiveProjection<Handedness::RightHanded, VulkanNDCConfig>;
    using VulkanOrthographicProjection =
        OrthographicProjection<Handedness::RightHanded, VulkanNDCConfig>;

    using VulkanUIOrthographicProjection =
        OrthographicProjection<Handedness::LeftHanded, VulkanNDCConfig>;
    ...
    class GenCamera
    {
      public:
        ...
        constexpr glm::mat4 getProjMatrix() const noexcept
        {
            return projection_();
        }
