    ...
    auto generate_line_meshes =
        ...
        for (const auto &line : lines)
        {
            ...
            for (const auto *g : line_glyphs)
            {
                ...
                std::array<uint32_t, 6> VIndices{baseVertex + 0, baseVertex + 2,
                                                 baseVertex + 1, baseVertex + 0,
                                                 baseVertex + 3, baseVertex + 2};
                indices.append_range(VIndices);

                //NOTE: 不通过
                // assert(validateMeshWinding({VArray.begin() , 4},
                //                            {VIndices.begin(), 6}) &&
