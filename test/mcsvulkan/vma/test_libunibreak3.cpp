...
*/
    ...
    */
    ...
    // diff: [test_libunibreak2] start
    std::u32string text =
        U"Hello, World! 我我我我 世界 的 一 是 在 不 世 界 😊 😃 😄\n"
    ...
    */
    ...
    // 新增：行内视觉重排函数（保持不变，正确）
    auto reorder_line =
        [](const std::vector<const ShapedGlyph *> &lineGlyphs, size_t startChar,
           size_t endChar,
           const std::vector<SBLevel> &levels) -> std::vector<const ShapedGlyph *> {
        ...
        while (i < endChar)
        {
            bool isRTL = (levels[i] % 2) == 1;
            size_t j = i + 1;
            while (j < endChar && ((levels[j] % 2) == 1) == isRTL)
                ++j;
            runs.push_back({i, j, isRTL});
            i = j;
        }
        ...
        for (const auto &run : runs)
        {
            if (!run.isRTL)
            {
                for (size_t ch = run.start; ch < run.end; ++ch)
                {
            ...
            {
                for (size_t offset = run.end - run.start; offset > 0; --offset)
                {
                    size_t ch = run.start + offset - 1;
                    auto it = charToGlyphs.find(ch);
                    if (it != charToGlyphs.end())
                    {
                        for (auto *g : it->second)
    ...
    // 执行流程：双向分析 → 整形 → 分行 → 重排 → 生成网格
    std::vector<SBLevel> levels = bidi_analyze(text);
    LogicalGlyphs logical = shape_with_harfbuzz(text, levels, font_lib);
    ...
    for (const auto &line : lines)
    {
        visualLines.push_back(
            reorder_line(line.glyphs, line.startChar, line.endChar, levels));
    }
                             ...
                             allocator, commandPool, GRAPHICS_AND_PRESENT);
    {
        ...
        auto debug_advance_units = [&](const std::u32string &text,
                                       const std::vector<SBLevel> &levels,
                                       const font::FontLibrary &fontLib) {
            std::println("\n=== 字形完整度量调试（设计单位） ===");
            const size_t n = text.size();
            ...
            {
                font::glyph_info item;
                auto fontOpt = fontLib.getFont(text[i]);
                if (!fontOpt)
                {
                    fontOpt = fontLib.getFont(0);
                    item = (*fontOpt).getDefaultGlyph(0, fontLib);
                }
                else
                {
                    item = (*fontOpt).getDefaultGlyph(text[i], fontLib);
                }
                charDefaultInfo.push_back(item);
            }
            ...
            for (const auto &run : runs)
            {
                ...
                double emUnits = run.font->getEmUnits();
                double fontSizePixels = run.font->font.atlas.size;

                std::println("\n字体: {}, emUnits={:.0f}, fontSizePixels={:.0f}",
                             run.font->name, emUnits, fontSizePixels);

                for (unsigned k = 0; k < count; ++k)
                {
                    size_t charIdx = run.start + info[k].cluster;
                    char32_t ch = text[charIdx];
                    uint32_t glyphIdx = info[k].codepoint;

                    std::println("\n--- Glyph [{}] (字符 U+{:04X})('{}') ---", k,
                                 static_cast<uint32_t>(ch), to_utf8(ch));
                    std::println("  HarfBuzz 字形索引: {}", glyphIdx);

                    // ----- HarfBuzz 原始数据 (26.6 定点) -----
                    hb_position_t hb_x_adv = pos[k].x_advance;
                    hb_position_t hb_y_adv = pos[k].y_advance;
                    hb_position_t hb_x_off = pos[k].x_offset;
                    hb_position_t hb_y_off = pos[k].y_offset;

                    double hb_pix_adv_x = static_cast<double>(hb_x_adv) / 64.0;
                    double hb_pix_adv_y = static_cast<double>(hb_y_adv) / 64.0;
                    double hb_pix_off_x = static_cast<double>(hb_x_off) / 64.0;
                    double hb_pix_off_y = static_cast<double>(hb_y_off) / 64.0;

                    double hb_des_adv_x = hb_pix_adv_x * emUnits / fontSizePixels;
                    double hb_des_adv_y = hb_pix_adv_y * emUnits / fontSizePixels;
                    double hb_des_off_x = hb_pix_off_x * emUnits / fontSizePixels;
                    double hb_des_off_y = hb_pix_off_y * emUnits / fontSizePixels;

                    std::println("  [HarfBuzz]");
                    std::println(
                        "    x_advance (26.6): {}, 像素: {:.3f}, 设计单位: {:.3f}",
                        hb_x_adv, hb_pix_adv_x, hb_des_adv_x);
                    std::println(
                        "    y_advance (26.6): {}, 像素: {:.3f}, 设计单位: {:.3f}",
                        hb_y_adv, hb_pix_adv_y, hb_des_adv_y);
                    std::println(
                        "    x_offset  (26.6): {}, 像素: {:.3f}, 设计单位: {:.3f}",
                        hb_x_off, hb_pix_off_x, hb_des_off_x);
                    std::println(
                        "    y_offset  (26.6): {}, 像素: {:.3f}, 设计单位: {:.3f}",
                        hb_y_off, hb_pix_off_y, hb_des_off_y);

                    // ----- JSON 数据 -----
                    double json_norm_advance = 0.0;
                    const font::Font::glyphs_type *glyphMeta = nullptr;
                    auto it = run.font->glyph_index_to_glyphs.find(glyphIdx);
                    if (it == run.font->glyph_index_to_glyphs.end())
                    {
                        throw std::runtime_error(
                            "Glyph index " + std::to_string(glyphIdx) +
                            " not found in JSON for font " + run.font->name);
                    }
                    glyphMeta = it->second;
                    json_norm_advance = glyphMeta->advance;
                    double json_des_advance = json_norm_advance * emUnits;

                    std::println("  [JSON]");
                    std::println("    advance (归一化): {:.6f}, 设计单位: {:.3f}",
                                 json_norm_advance, json_des_advance);
                    if (glyphMeta && glyphMeta->planeBounds.has_value())
                    {
                        auto &pb = glyphMeta->planeBounds.value();
                        std::println("    planeBounds: left={:.3f}, bottom={:.3f}, "
                                     "right={:.3f}, top={:.3f}",
                                     pb.left, pb.bottom, pb.right, pb.top);
                    }
                    else
                    {
                        std::println("    planeBounds: (none)");
                    }

                    // ----- FreeType 度量 (设计单位，不缩放) -----
                    FT_Face face = *run.font->face;
                    FT_UInt ft_glyph_index =
                        glyphIdx; // 使用 HarfBuzz 输出的字形索引，保证一致
                    if (ft_glyph_index != 0)
                    {
                        FT_Load_Glyph(face, ft_glyph_index, FT_LOAD_NO_SCALE);
                        FT_Glyph_Metrics &m = face->glyph->metrics;

                        std::println("  [FreeType] (设计单位)");
                        std::println("    width        : {}", m.width);
                        std::println("    height       : {}", m.height);
                        std::println("    horiBearingX : {}", m.horiBearingX);
                        std::println("    horiBearingY : {}", m.horiBearingY);
                        std::println("    horiAdvance  : {}", m.horiAdvance);
                        std::println("    vertBearingX : {}", m.vertBearingX);
                        std::println("    vertBearingY : {}", m.vertBearingY);
                        std::println("    vertAdvance  : {}", m.vertAdvance);
                        // linear advances 是 16.16 格式，除以 65536 得设计单位
                        double linearHori =
                            static_cast<double>(face->glyph->linearHoriAdvance) / 65536.0;
                        double linearVert =
                            static_cast<double>(face->glyph->linearVertAdvance) / 65536.0;
                        std::println("    linearHoriAdvance (设计单位): {:.3f}",
                                     linearHori);
                        std::println("    linearVertAdvance (设计单位): {:.3f}",
                                     linearVert);
                    }
                    else
                    {
                        std::println("  [FreeType] 字形索引为0，无法加载");
                    }

                    if (k + 1 < count)
                    {
                        size_t nextCharIdx = run.start + info[k + 1].cluster;
                        char32_t nextCh = text[nextCharIdx];
                        uint32_t nextGlyphIdx = info[k + 1].codepoint;

                        bool has_kerning = false;
                        std::string kerning_details;

                        // ----- JSON Kerning -----
                        double json_kern = 0.0;
                        bool json_found = false;
                        for (const auto &kernItem : run.font->font.kerning)
                        {
                            if (std::holds_alternative<font::Font::Kerning::GLYPH_INDEX>(
                                    kernItem.value))
                            {
                                auto &kg = std::get<font::Font::Kerning::GLYPH_INDEX>(
                                    kernItem.value);
                                if (kg.index1 == static_cast<int>(glyphIdx) &&
                                    kg.index2 == static_cast<int>(nextGlyphIdx))
                                {
                                    json_kern = kg.advance;
                                    json_found = true;
                                    break;
                                }
                            }
                            else if (std::holds_alternative<
                                         font::Font::Kerning::UNICODE_CODEPOINT>(
                                         kernItem.value))
                            {
                                auto &ku =
                                    std::get<font::Font::Kerning::UNICODE_CODEPOINT>(
                                        kernItem.value);
                                if (ku.unicode1 == ch && ku.unicode2 == nextCh)
                                {
                                    json_kern = ku.advance;
                                    json_found = true;
                                    break;
                                }
                            }
                        ...
                        if (json_found)
                        {
                            double json_kern_des = json_kern * emUnits;
                            kerning_details +=
                                "    JSON kerning (归一化): " +
                                std::to_string(json_kern) +
                                ", 设计单位: " + std::to_string(json_kern_des) + "\n";
                            has_kerning = true;
                        }
                        ...
                        FT_Face face = *run.font->face;
                        FT_Vector kern;
                        if (FT_Get_Kerning(face, glyphIdx, nextGlyphIdx,
                                           FT_KERNING_DEFAULT, &kern) == 0)
                        {
                            double ft_kern_x = static_cast<double>(kern.x) / 64.0;
                            double ft_kern_y = static_cast<double>(kern.y) / 64.0;
                            if (ft_kern_x != 0.0 || ft_kern_y != 0.0)
                            {
                                kerning_details += "    FreeType kerning (设计单位): x=" +
                                                   std::to_string(ft_kern_x) +
                                                   ", y=" + std::to_string(ft_kern_y) +
                                                   "\n";
                                has_kerning = true;
                            }
                        }
                        ...
                        if (has_kerning)
                        {
                            std::println("  [Kerning between {} and {}]", k, k + 1);
                            std::print("{}", kerning_details);
                        }
                    }
                }
        ...
        std::u32string text = U"Hello سلام的世界 😊";
        debug_advance_units(text, bidi_analyze(text), font_lib);

        // TODO 完善单位统一测试. 比较3的数据源的度量信息，目前太简陋
        {
            std::println("\n=== 全面度量验证（设计单位） ===");
            std::u32string test_text = U"Hello سلام的世界 😊";
            auto test_levels = bidi_analyze(test_text);
            const size_t n = test_text.size();

            // 预计算每个字符的默认字形信息（用于分组，同前）
            std::vector<font::glyph_info> charDefaultInfo;
            charDefaultInfo.reserve(n);
            for (size_t i = 0; i < n; ++i)
            {
                font::glyph_info item;
                auto fontOpt = font_lib.getFont(test_text[i]);
                if (!fontOpt)
                {
                    fontOpt = font_lib.getFont(0);
                    item = (*fontOpt).getDefaultGlyph(0, font_lib);
                }
                else
                {
                    item = (*fontOpt).getDefaultGlyph(test_text[i], font_lib);
                }
                charDefaultInfo.push_back(item);
            }
            ...
            while (idx < n)
            {
                const auto *currentFont = charDefaultInfo[idx].font_ctx;
                bool isRTL = (test_levels[idx] % 2) == 1;
                size_t j = idx + 1;
                while (j < n && charDefaultInfo[j].font_ctx == currentFont &&
                       (test_levels[j] % 2) == isRTL)
                    ++j;
                runs.push_back({idx, j, currentFont, isRTL});
                idx = j;
            }
            ...
            struct GlyphMetrics
            {
                ...
                double json_advance;
                double ft_advance;
                // HarfBuzz offset (设计单位)
                double hb_off_x, hb_off_y;
                // JSON planeBounds (设计单位)
                double json_left, json_bottom, json_right, json_top;
                bool has_plane;
                // FreeType 包围盒 (设计单位)
                double ft_left, ft_bottom, ft_right, ft_top;
                // 误差（相对于FreeType）
                double err_advance;
                double err_left, err_bottom, err_right, err_top;
                double max_err;
            };
            std::vector<GlyphMetrics> results;

            for (const auto &run : runs)
            {
                hb_buffer_t *buf = hb_buffer_create();
                hb_buffer_add_utf32(
                    buf, reinterpret_cast<const uint32_t *>(test_text.data()) + run.start,
                    static_cast<int>(run.end - run.start), 0,
                    static_cast<int>(run.end - run.start));
                hb_buffer_guess_segment_properties(buf);
                hb_shape(*run.font->hb_font, buf, nullptr, 0);
                ...
                {
                    size_t charIdx = run.start + info[k].cluster;
                    char32_t ch = test_text[charIdx];
                    uint32_t glyphIdx = info[k].codepoint;

                    // HarfBuzz advance & offset
                    double hb_pix_adv = static_cast<double>(pos[k].x_advance) / 64.0;
                    double hb_adv = hb_pix_adv * emUnits / fontSizePixels;
                    double hb_pix_off_x = static_cast<double>(pos[k].x_offset) / 64.0;
                    double hb_pix_off_y = static_cast<double>(pos[k].y_offset) / 64.0;
                    double hb_off_x = hb_pix_off_x * emUnits / fontSizePixels;
                    double hb_off_y = hb_pix_off_y * emUnits / fontSizePixels;
                    ...
                    const auto *glyphMeta = it->second;
                    double json_adv = glyphMeta->advance * emUnits;
                    bool has_plane = glyphMeta->planeBounds.has_value();
                    double json_left = 0, json_bottom = 0, json_right = 0, json_top = 0;
                    if (has_plane)
                    {
                        const auto &pb = glyphMeta->planeBounds.value();
                        json_left = pb.left * emUnits;
                        json_bottom = pb.bottom * emUnits;
                        json_right = pb.right * emUnits;
                        json_top = pb.top * emUnits;
                    }

                    // FreeType 加载字形获取设计单位度量
                    FT_Face face = *run.font->face;
                    double ft_adv = 0.0;
                    double ft_left = 0, ft_bottom = 0, ft_right = 0, ft_top = 0;
                    if (glyphIdx != 0)
                    {
                        FT_Load_Glyph(face, glyphIdx, FT_LOAD_NO_SCALE);
                        FT_Glyph_Metrics &m = face->glyph->metrics;
                        ft_adv = m.horiAdvance;
                        ft_left = m.horiBearingX;
                        ft_right = m.horiBearingX + m.width;
                        ft_top = m.horiBearingY;
                        ft_bottom = m.horiBearingY - m.height;
                    }

                    GlyphMetrics gm;
                    gm.ch = ch;
                    gm.glyphIdx = glyphIdx;
                    gm.hb_advance = hb_adv;
                    gm.json_advance = json_adv;
                    gm.ft_advance = ft_adv;
                    gm.hb_off_x = hb_off_x;
                    gm.hb_off_y = hb_off_y;
                    gm.has_plane = has_plane;
                    gm.json_left = json_left;
                    gm.json_bottom = json_bottom;
                    gm.json_right = json_right;
                    gm.json_top = json_top;
                    gm.ft_left = ft_left;
                    gm.ft_bottom = ft_bottom;
                    gm.ft_right = ft_right;
                    gm.ft_top = ft_top;
                    gm.err_advance = std::abs(hb_adv - ft_adv); // 可选择比较 JSON vs FT
                    if (has_plane)
                    {
                        gm.err_left = std::abs(json_left - ft_left);
                        gm.err_bottom = std::abs(json_bottom - ft_bottom);
                        gm.err_right = std::abs(json_right - ft_right);
                        gm.err_top = std::abs(json_top - ft_top);
                        gm.max_err = std::max({gm.err_left, gm.err_bottom, gm.err_right,
                                               gm.err_top, gm.err_advance});
                    }
                    else
                    {
                        gm.err_left = gm.err_bottom = gm.err_right = gm.err_top =
                            gm.max_err = gm.err_advance;
                    }
            ...
            // 打印表头（带三个数据源对比）
            std::println("{:>4} {:>6} {:>8} | {:>10} {:>10} {:>10} | {:>8} {:>8} | {:>8} "
                         "{:>8} {:>8} {:>8} | {:>8} {:>8} {:>8} {:>8} | {:>8} | {:>8}",
                         "Idx", "Char", "Glyph", "HB_Adv", "JSON_Adv", "FT_Adv",
                         "HB_OffX", "HB_OffY", "J_Left", "J_Btm", "J_Right", "J_Top",
                         "FT_Left", "FT_Btm", "FT_Right", "FT_Top", "MaxErr", "Status");
            const double EPS = 5.0;
            int fail_count = 0;
            for (size_t i = 0; i < results.size(); ++i)
            {
                const auto &r = results[i];
                std::string status = (r.max_err <= EPS) ? "OK" : "⚠️";
                if (r.max_err > EPS)
                    ++fail_count;
                std::string utf8_char;
                if (r.ch < 0x80)
                    utf8_char = std::string(1, static_cast<char>(r.ch));
                else if (r.ch < 0x800)
                {
                    utf8_char += static_cast<char>(0xC0 | (r.ch >> 6));
                    utf8_char += static_cast<char>(0x80 | (r.ch & 0x3F));
                }
                else if (r.ch < 0x10000)
                {
                    utf8_char += static_cast<char>(0xE0 | (r.ch >> 12));
                    utf8_char += static_cast<char>(0x80 | ((r.ch >> 6) & 0x3F));
                    utf8_char += static_cast<char>(0x80 | (r.ch & 0x3F));
                }
                else
                {
                    utf8_char += static_cast<char>(0xF0 | (r.ch >> 18));
                    utf8_char += static_cast<char>(0x80 | ((r.ch >> 12) & 0x3F));
                    utf8_char += static_cast<char>(0x80 | ((r.ch >> 6) & 0x3F));
                    utf8_char += static_cast<char>(0x80 | (r.ch & 0x3F));
                }
                std::println("{:4d} U+{:04X} {:8d} | {:10.2f} {:10.2f} {:10.2f} | "
                             "{:8.2f} {:8.2f} | {:8.2f} {:8.2f} {:8.2f} {:8.2f} | "
                             "{:8.2f} {:8.2f} {:8.2f} {:8.2f} | {:8.2f} | {:>8}",
                             i, static_cast<uint32_t>(r.ch), r.glyphIdx, r.hb_advance,
                             r.json_advance, r.ft_advance, r.hb_off_x, r.hb_off_y,
                             r.json_left, r.json_bottom, r.json_right, r.json_top,
                             r.ft_left, r.ft_bottom, r.ft_right, r.ft_top, r.max_err,
                             status);
            }

            // 统计误差
            double sum_adv_err = 0, sum_left_err = 0, sum_bottom_err = 0,
                   sum_right_err = 0, sum_top_err = 0;
            int plane_count = 0;
            for (const auto &r : results)
            {
                sum_adv_err += r.err_advance;
                if (r.has_plane)
                {
                    sum_left_err += r.err_left;
                    sum_bottom_err += r.err_bottom;
                    sum_right_err += r.err_right;
                    sum_top_err += r.err_top;
                    ++plane_count;
                }
            }
            std::println("\n=== 误差统计 ===");
            std::println("总字形数: {}", results.size());
            std::println("有 planeBounds 的字形数: {}", plane_count);
            if (!results.empty())
            {
                std::println("advance 平均误差: {:.2f}", sum_adv_err / results.size());
            }
            if (plane_count > 0)
            {
                std::println("planeBounds left  平均误差: {:.2f}",
                             sum_left_err / plane_count);
                std::println("planeBounds bottom 平均误差: {:.2f}",
                             sum_bottom_err / plane_count);
                std::println("planeBounds right 平均误差: {:.2f}",
                             sum_right_err / plane_count);
                std::println("planeBounds top   平均误差: {:.2f}",
                             sum_top_err / plane_count);
            }
            std::println("误差超过 {:.1f} 设计单位的个数: {}", EPS, fail_count);
        }
    }
    // diff: [test_libunibreak] end
