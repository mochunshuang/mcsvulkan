...
try
{
    ...
    static constexpr std::array<Vertex, 4> quadVerts = {
        Vertex{{-0.5f, -0.5f, 0}, {0.0F, 0.0F}}, // 左下
        Vertex{{0.5, -0.5, 0}, {1.0F, 0.0F}},    // 右下
        Vertex{{0.5, 0.5, 0}, {1.0F, 1.0F}},     // 右上
        Vertex{{-0.5, 0.5, 0}, {0.0F, 1.0F}}     // 左上
    };
    ...
    constexpr auto make_quadVerts_instance_matrix =
        [](const std::span<const glm::vec3> &target) {
            // 标准 origin（与 quadVerts 的 pos 一致）
            constexpr std::array<glm::vec3, 4> standardOrigin = {{{-0.5f, -0.5f, 0.0f},
                                                                  {0.5f, -0.5f, 0.0f},
                                                                  {0.5f, 0.5f, 0.0f},
                                                                  {-0.5f, 0.5f, 0.0f}}};
            [[assume((target.size() == 4))]];
            return make_instance_matrix_from_verts(
                std::span<const glm::vec3>(standardOrigin), target);
        };
    ...
    auto autoSpinId = autoSpinStore.new_entity(
        InstanceData{global_id++, 0, 0,
                     object_data{make_quadVerts_instance_matrix(std::array<glm::vec3, 4>{
                         glm::vec3{-0.5f, -0.5f, 0}, glm::vec3{0.5, -0.5, 0},
    ...
    auto interactiveId = interactiveStore.new_entity(
        ...
        model_state{make_model_state_from_matrix(
            make_quadVerts_instance_matrix( //diff: [test_dod9] model_state 会覆盖 object_data 的。 object_data 才是最终提交的矩阵
                std::array<glm::vec3, 4>{
                    glm::vec3{-0.5f, -0.5f, 0.2}, glm::vec3{0.5, -0.5, 0.2},
                    glm::vec3{0.5, 0.5, 0.2}, glm::vec3{-0.5, 0.5, 0.2}}))});
