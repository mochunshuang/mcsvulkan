...
*/
    ...
    // 为解析图像创建视图（原来只有 resolveImage，没有单独视图）

    // 拾取推送常量（与 shader 相同）
    struct PushConstantsPicking
    {
        glm::mat4 model;
        uint32_t objectId;
    };
            ...
            .setCreateInfo( // diff: [Uniform] 将描述符集合布局设置到管线布局
                {.setLayouts = {*descriptorSetLayout},
                 .pushConstantRanges = {{
                     ...
                     .offset = 0,
                     .size = sizeof(PushConstantsPicking) // 64+4=68，对齐到4即可
                 }}})
            .build(device);
    ...
    auto pickingPipeline =
        create_graphics_pipeline{}
            .setCreateInfo({
                ...
                .stages = create_graphics_pipeline::makeStages(
                    stage_info{.stage = VK_SHADER_STAGE_VERTEX_BIT,
                               .filePath = "shaders/picking.vert.spv",
                               .pName = "main"},
                    stage_info{.stage = VK_SHADER_STAGE_FRAGMENT_BIT,
                               .filePath = "shaders/picking.frag.spv",
                               .pName = "main"}),
                .vertexInputState =
                    {//NOTE: 顶点输入
                ...
                .rasterizationState =
                    {.depthClampEnable = VK_FALSE,
                     .rasterizerDiscardEnable = VK_FALSE,
                     .polygonMode = VK_POLYGON_MODE_FILL,
                     .cullMode = VK_CULL_MODE_NONE, // NOTE: 与主渲染一致因为是附加信息
                     .frontFace = VK_FRONT_FACE_CLOCKWISE,
                     .lineWidth = 1.0F},
                .multisampleState =
                    {
                        .rasterizationSamples =
                            physical_device
                                .getMaxUsableSampleCount(), // NOTE:与颜色图像一致
                    },
                .depthStencilState =
    ...
    const auto recordCommandBuffer = [&](const CommandBufferView &commandBuffer,
...

        {
            ...
            mesh_base::updateObjectData(input_mesh.frameBuffers[currentFrame], [&] {
                return glm::rotate(glm::mat4(1.0F), mesh1_model,
        ...
        if (mouseValid)
        {
            ...
            // ② 深度 → 只读
            my_render::transition_image_layout(
                commandBuffer, depthImg, VK_IMAGE_ASPECT_DEPTH_BIT,
            ...
            // ④ 开始拾取渲染
            /*
错误 1：整数格式的 resolve 模式无效
VUID-VkRenderingAttachmentInfo-imageView-06130

text
resolveMode (VK_RESOLVE_MODE_AVERAGE_BIT) must be VK_RESOLVE_MODE_NONE or VK_RESOLVE_MODE_SAMPLE_ZERO_BIT for integer formats (VK_FORMAT_R32G32_UINT)
原因：拾取图像的格式是 VK_FORMAT_R32G32_UINT（整数格式），而渲染附件 pickColorAtt 设置了 resolveMode = VK_RESOLVE_MODE_AVERAGE_BIT。对整数格式使用平均解析（averaging）是非法的。

修复：将解析模式改为 VK_RESOLVE_MODE_SAMPLE_ZERO_BIT（直接取第 0 号样本）：
*/
            ...
            /*
NOTE: 切换管线要注意了
[ ERROR ] [create_debugger.hpp:26:static VkBool32 __cdecl mcs::vulkan::tool::create_debugger::defaultDebugCallback(VkDebugUtilsMessageSeverityFlagBitsEXT, VkDebugUtilsMessageTypeFlagsEXT, const VkDebugUtilsMessengerCallbackDataEXT *, void *)]: 941228658 Validation Layer: Error: VUID-vkCmdDrawIndexed-None-08600: vkCmdDrawIndexed(): The VkPipeline 0x7d000000007d (created with VkPipelineLayout 0x7a000000007a) statically uses descriptor set 0, but set 0 is not compatible with the pipeline layout bound with vkCmdBindDescriptorSets (VkPipelineLayout 0x430000000043)
Pipeline layout pipeline bound with last call to vkCmdBindDescriptorSets has following push constant ranges:
VkPushConstantRange[0]: range [0, 32) for VK_SHADER_STAGE_VERTEX_BIT
But pipeline layout of last bound pipeline or last bound shaders has following push constant ranges:
VkPushConstantRange[0]: range [0, 68) for VK_SHADER_STAGE_VERTEX_BIT|VK_SHADER_STAGE_FRAGMENT_BIT
The Vulkan spec states: If a a bound shader was created as a VkShaderEXT without the VK_SHADER_CREATE_DESCRIPTOR_HEAP_BIT_EXT flag or as part of a pipeline without the VK_PIPELINE_CREATE_2_DESCRIPTOR_HEAP_BIT_EXT flag, and that shader statically uses a set n, a descriptor set must have been bound to n at the same pipeline bind point, with a VkPipelineLayout that is compatible for set n, with the VkPipelineLayout used to create the current VkPipeline or the VkDescriptorSetLayout array used to create the current VkShaderEXT , as described in Pipeline Layout Compatibility (https://docs.vulkan.org/spec/latest/chapters/drawing.html#VUID-vkCmdDrawIndexed-None-08600)
*/
            commandBuffer.bindPipeline(VK_PIPELINE_BIND_POINT_GRAPHICS, *pickingPipeline);
            commandBuffer.bindDescriptorSets(VK_PIPELINE_BIND_POINT_GRAPHICS,
                                             *pickingPipelineLayout, 0, 1, &descriptorSet,
                                             0, nullptr); //NOTE: 用到了描述符
            ...
            // 绘制物体1
            {
                PushConstantsPicking push{};
                push.model = glm::rotate(glm::mat4(1), mesh1_model, glm::vec3(0, 0, 1));
                push.objectId = input_mesh_id;
                commandBuffer.pushConstants(*pickingPipelineLayout,
                                            VK_SHADER_STAGE_VERTEX_BIT |
                                                VK_SHADER_STAGE_FRAGMENT_BIT,
                                            0, sizeof(push), &push);
                commandBuffer.bindVertexBuffers(
                    0, 1, {input_mesh.frameBuffers[currentFrame].vertexBuffer.buffer()},
                    {0});
                commandBuffer.bindIndexBuffer(
                    input_mesh.frameBuffers[currentFrame].indexBuffer.buffer(), 0,
            ...
            {
                PushConstantsPicking push{};
                push.model = mesh2_model;
                push.objectId = input_mesh2_id;
                commandBuffer.pushConstants(*pickingPipelineLayout,
                                            VK_SHADER_STAGE_VERTEX_BIT |
                                                VK_SHADER_STAGE_FRAGMENT_BIT,
                                            0, sizeof(push), &push);
                commandBuffer.bindVertexBuffers(
                    0, 1, {input_mesh2.frameBuffers[currentFrame].vertexBuffer.buffer()},
                    {0});
                commandBuffer.bindIndexBuffer(
                    input_mesh2.frameBuffers[currentFrame].indexBuffer.buffer(), 0,
                    VK_INDEX_TYPE_UINT32);
            ...
            commandBuffer.copyImageToBuffer(
                *resolveImage, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL,
                ...
                std::array<VkBufferImageCopy, 1>{VkBufferImageCopy{
                    ...
                    .imageExtent = {1, 1, 1}}});
            /*
[ ERROR ] [create_debugger.hpp:26:static VkBool32 __cdecl mcs::vulkan::tool::create_debugger::defaultDebugCallback(VkDebugUtilsMessageSeverityFlagBitsEXT, VkDebugUtilsMessageTypeFlagsEXT, const VkDebugUtilsMessengerCallbackDataEXT *, void *)]: -606812001 Validation Layer: Error: VUID-VkImageMemoryBarrier2-newLayout-01198: vkCmdPipelineBarrier2(): pDependencyInfo->pImageMemoryBarriers[0].newLayout is VK_IMAGE_LAYOUT_UNDEFINED.                                                                                                                                                                                       
The Vulkan spec states: If layouts are not ignored, newLayout must not be VK_IMAGE_LAYOUT_UNDEFINED or VK_IMAGE_LAYOUT_ZERO_INITIALIZED_EXT or VK_IMAGE_LAYOUT_PREINITIALIZED (https://docs.vulkan.org/spec/latest/chapters/synchronization.html#VUID-VkImageMemoryBarrier2-newLayout-01198)    
NOTE: Vulkan 规范禁止 pipelineBarrier2 的 newLayout 为 VK_IMAGE_LAYOUT_UNDEFINED（只有 oldLayout 可以是）
*/
            // ⑦ 解析图像 → UNDEFINED（为下一帧的 resolve 目标重置）
            // my_render::transition_image_layout(
            //     commandBuffer, *resolveImage, VK_IMAGE_ASPECT_COLOR_BIT,
            //     VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, VK_IMAGE_LAYOUT_UNDEFINED,
            //     VK_ACCESS_2_TRANSFER_READ_BIT, 0, VK_PIPELINE_STAGE_2_TRANSFER_BIT,
            ...
            my_render::transition_image_layout(
                commandBuffer, *resolveImage, VK_IMAGE_ASPECT_COLOR_BIT,
                VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL,
                VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL, // 不再是 UNDEFINED
                VK_ACCESS_2_TRANSFER_READ_BIT, VK_ACCESS_2_COLOR_ATTACHMENT_WRITE_BIT,
                VK_PIPELINE_STAGE_2_TRANSFER_BIT,
                VK_PIPELINE_STAGE_2_COLOR_ATTACHMENT_OUTPUT_BIT);

            // // ⑧ 深度图像 → UNDEFINED（下一帧主渲染会自行转到 ATTACHMENT）
            // my_render::transition_image_layout(
            //     commandBuffer, depthImg, VK_IMAGE_ASPECT_DEPTH_BIT,
            //     VK_IMAGE_LAYOUT_DEPTH_STENCIL_READ_ONLY_OPTIMAL,
            //     VK_IMAGE_LAYOUT_UNDEFINED, VK_ACCESS_2_DEPTH_STENCIL_ATTACHMENT_READ_BIT,
            ...
            my_render::transition_image_layout(
                commandBuffer, depthImg, VK_IMAGE_ASPECT_DEPTH_BIT,
                VK_IMAGE_LAYOUT_DEPTH_STENCIL_READ_ONLY_OPTIMAL,
                VK_IMAGE_LAYOUT_DEPTH_ATTACHMENT_OPTIMAL, // 不再是 UNDEFINED
                VK_ACCESS_2_DEPTH_STENCIL_ATTACHMENT_READ_BIT,
                VK_ACCESS_2_DEPTH_STENCIL_ATTACHMENT_WRITE_BIT,
                VK_PIPELINE_STAGE_2_EARLY_FRAGMENT_TESTS_BIT |
                    VK_PIPELINE_STAGE_2_LATE_FRAGMENT_TESTS_BIT,
    ...
    const auto recreateSwapChain = [&]() constexpr {
        ...
        build_pick.updateImageExtent({newExtent.width, newExtent.height, 1});
        pickingImage = build_pick.rebuild();
        pickingImageView = build_pick.makeImageView(*pickingImage);

        build_resolve.updateImageExtent({newExtent.width, newExtent.height, 1});
        resolveImage = build_resolve.rebuild();
        // 重建解析图像的视图
        pickingResolveImageView = build_resolve.makeImageView(*resolveImage);
        // 暂时禁用拾取，直到新尺寸下的鼠标坐标更新
        mouseValid = false;

        /*
[ ERROR ] [create_debugger.hpp:26:static VkBool32 __cdecl mcs::vulkan::tool::create_debugger::defaultDebugCallback(VkDebugUtilsMessageSeverityFlagBitsEXT, VkDebugUtilsMessageTypeFlagsEXT, const VkDebugUtilsMessengerCallbackDataEXT *, void *)]: -370888023 Validation Layer: Error: VUID-vkAcquireNextImageKHR-semaphore-01286: vkAcquireNextImageKHR(): Semaphore must not be currently signaled.
...
*/
    ...
    const auto drawFrame = [&]() constexpr {
        ...
        /*
之前没有拾取，所有附件都能正确自动同步，不需要 SUBOPTIMAL。
现在有拾取，且拾取附件的更新存在 bug（不能正确同步尺寸），所以借助 SUBOPTIMAL 的早期触发来弥补这个 bug
*/
        //VK_SUBOPTIMAL_KHR:  “交换链已不是最佳尺寸，但仍可被渲染”
        if (result == VK_ERROR_OUT_OF_DATE_KHR ||
            result ==
                VK_SUBOPTIMAL_KHR) //diff: [test_picking5] 图像与表面不完全匹配要重建，否则可能拾取错误
        {
            recreateSwapChain();
            return;
        }
    ...
    while (window.shouldClose() == 0)
    {
        ...
        if (cur.xpos >= 0 && cur.xpos < (int)ext.width && cur.ypos >= 0 &&
            cur.ypos < (int)ext.height)
        {
