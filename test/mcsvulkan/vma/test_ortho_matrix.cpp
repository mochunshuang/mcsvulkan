...
*/
    ...
    const auto recordCommandBuffer = [&](const CommandBufferView &commandBuffer,
        ...
        auto mvp = [&]() {
            ...
            // diff: [test_ortho_matrix] start
            // ============================================================
            // 正交投影测试用例（激活其中一个，其余注释掉）
            // ============================================================

            float aspect = static_cast<float>(swapchain.imageExtent().width) /
                           static_cast<float>(swapchain.imageExtent().height);

            // NOTE: 使用标准顺序：bottom < top
            // ----------------------------- 用例 1：标准对称正交
            // ------------------------------ 左右 -1 到 1，下上 -1 到 1，近 0.1，远 10.0
            // 效果：物体大小与距离无关，无透视变形。物体在 [-1,1] 范围内的部分可见
            // --------------------------------------------------------------------
            // ubo.proj = glm::ortho(-1.0f, 1.0f, -1.0f, 1.0f, 0.1f, 10.0f);

            // ----------------------------- 用例 2：左右范围扩展（更宽）
            // ---------------------- 左右范围扩大到
            // [-2,2]，物体显得更小（相同屏幕空间显示更大世界范围）
            // 效果：原本占满屏幕的物体现在缩小，可以看到更多场景
            // --------------------------------------------------------------------
            // ubo.proj = glm::ortho(-2.0f, 2.0f, -1.0f, 1.0f, 0.1f, 10.0f);

            // ----------------------------- 用例 3：上下范围扩展（更高）
            // ---------------------- 上下范围扩大到 [-2,2]，物体垂直方向缩小
            // --------------------------------------------------------------------
            // ubo.proj = glm::ortho(-1.0f, 1.0f, -2.0f, 2.0f, 0.1f, 10.0f);

            // ----------------------------- 用例 4：非对称正交（左右偏移）
            // -------------------- 左=0，右=2，下=-1，上=1，视景体中心偏移到 x=1
            // 效果：物体整体向右偏移，因为屏幕左边界对应世界 x=0
            // --------------------------------------------------------------------
            // ubo.proj = glm::ortho(0.0f, 2.0f, -1.0f, 1.0f, 0.1f, 10.0f);

            // ----------------------------- 用例 5：非对称正交（上下偏移）
            // -------------------- 左=-1，右=1，下=0，上=1，视景体中心偏移到 y=0.5
            // 效果：物体整体向上偏移
            // --------------------------------------------------------------------
            // NOTE: 看不到一个物体了
            // ubo.proj = glm::ortho(-1.0f, 1.0f, 0.0f, 1.0f, 0.1f, 10.0f);

            // ----------------------------- 用例 6：适配屏幕宽高比（保持物体比例）
            // ------------
            // 根据窗口宽高比调整左右范围，使得正方形在屏幕上显示为正方形（无拉伸）
            // 例如：aspect = 800/600 ≈ 1.333，则 left=-1.333, right=1.333
            // 效果：无论窗口比例如何，几何体形状保持正确
            // --------------------------------------------------------------------
            // 保持高度范围为 [-1,1]，宽度范围按比例缩放
            // float halfWidth = 1.0f * aspect;
            // ubo.proj = glm::ortho(-halfWidth, halfWidth, -1.0f, 1.0f, 0.1f, 10.0f);

            // ----------------------------- 用例 7：适配屏幕宽高比（另一种方式）
            // -------------- 保持宽度范围为 [-1,1]，按比例调整高度范围
            // 效果：同样保持几何体比例，但高度范围变化
            // --------------------------------------------------------------------
            float halfHeight = 1.0f / aspect;
            ubo.proj = glm::ortho(-1.0f, 1.0f, -halfHeight, halfHeight, 0.1f, 10.0f);

            // ----------------------------- 用例 8：极近近平面
            // --------------------------------- 将近平面设为
            // 0.01，极近距离仍可见（但可能引起深度精度问题）
            // --------------------------------------------------------------------
            // ubo.proj = glm::ortho(-1.0f, 1.0f, -1.0f, 1.0f, 0.01f, 10.0f);

            // ----------------------------- 用例 9：极远远平面
            // --------------------------------- 远平面设为
            // 100.0，可以看到更远的物体（但深度精度下降）
            // --------------------------------------------------------------------
            // ubo.proj = glm::ortho(-1.0f, 1.0f, -1.0f, 1.0f, 0.1f, 100.0f);

            // ----------------------------- 用例 10：翻转 Y 轴（Vulkan NDC 适配）
            // -------------- 默认 glm::ortho 生成 OpenGL 风格矩阵（Y 向上）。Vulkan 的
            // NDC 是 Y 向下， 如果不翻转，物体在屏幕上会上下颠倒。通过翻转矩阵的 Y
            // 分量可以修正。
            // 注意：是否需要翻转取决于您的视图矩阵和期望。如果您希望屏幕原点在左上角，
            // 通常需要翻转。这里提供翻转版本（通过乘以缩放矩阵）。
            // 您可以手动将矩阵的[1][1]乘以-1，或者使用 glm::ortho 后执行翻转。
            // 下面的例子展示如何同时实现翻转和适配宽高比（用例6的翻转版本）
            // --------------------------------------------------------------------
            // float halfWidth = 1.0f * aspect;
            // auto ortho = glm::ortho(-halfWidth, halfWidth, -1.0f, 1.0f, 0.1f, 10.0f);
            // ortho[1][1] *= -1; // 翻转 Y 轴
            // ubo.proj = ortho;

            // ----------------------------- 用例 11：完全自定义（使用 glm::ortho 重载）
            // ------- glm::ortho 有多个重载，可以传入左右下上近远。这里展示所有参数自定义
            // --------------------------------------------------------------------
            // ubo.proj = glm::ortho(-1.5f, 1.5f, -0.75f, 0.75f, 0.5f, 5.0f);

            // diff: [test_ortho_matrix] end
