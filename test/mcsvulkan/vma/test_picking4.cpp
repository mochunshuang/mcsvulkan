...
*/
    ...
    // diff: [test_picking3] start
    //  创建解析图像（单样本）
    auto build_resolve = create_image{device, allocator}.setCreateInfo(
    ...
    const auto recordCommandBuffer = [&](const CommandBufferView &commandBuffer,
        ...
        if (enable_picking)
        {
            ...
            std::cout << "拾取中....\n";
            // 获取当前拾取图像的实际尺寸（会随 swapchain 重建更新）
            VkExtent3D pickExt = build_pick.createInfo().extent; //diff: [test_picking4]
            ...
            // diff:[test_picking4] 🔧 使用实际附件尺寸
            VkRenderingInfo pickRenderingInfo = {
                .sType = VK_STRUCTURE_TYPE_RENDERING_INFO,
                .renderArea = {.offset = {0, 0},
                               .extent = {pickExt.width,
                                          pickExt.height}}, // 原为 {WIDTH, HEIGHT}
                .layerCount = 1,
                .colorAttachmentCount = 1,
            ...
            // diff:[test_picking4] 🔧 视口和裁剪也使用实际尺寸
            // 设置视口和裁剪
            VkViewport viewport = {0.0F,
                                   0.0F,
                                   static_cast<float>(pickExt.width),
                                   static_cast<float>(pickExt.height),
                                   0.0F,
                                   1.0F}; // 原用 WIDTH, HEIGHT
            VkRect2D scissor = {{0, 0},
                                {pickExt.width, pickExt.height}}; // 原用 {WIDTH, HEIGHT}
            ...
            // diff:[test_picking4] 🔧 解析时 extent 也需要使用实际尺寸
            VkImageResolve resolveRegion = {
                .srcSubresource = {VK_IMAGE_ASPECT_COLOR_BIT, 0, 0, 1},
                .srcOffset = {0, 0, 0},
                .dstSubresource = {VK_IMAGE_ASPECT_COLOR_BIT, 0, 0, 1},
                .dstOffset = {0, 0, 0},
                .extent = {pickExt.width, pickExt.height, 1}}; // 原为 {WIDTH, HEIGHT, 1}

            commandBuffer.resolveImage(
                *pickingImage, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, *resolveImage,
                VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, std::span{&resolveRegion, 1});
    ...
    const auto recreateSwapChain = [&]() constexpr {
        ...
        //diff: [test_picking4] start
        // === FIX 1: 重建拾取图像与解析图像 (尺寸跟随 swapchain) ===
        auto newExtent = swapchain.refImageExtent();
        build_pick.updateImageExtent({newExtent.width, newExtent.height, 1});
        pickingImage = build_pick.makeImage();
        pickingImageView = build_pick.makeImageView(*pickingImage);

        build_resolve.updateImageExtent({newExtent.width, newExtent.height, 1});
        resolveImage = build_resolve.makeImage();
        // 同时重建 resolveImageView（之前未创建，需要添加）
        // resolveImageView = build_resolve.makeImageView(*resolveImage);   // 如果需要

        // === FIX 2: 重置拾取图像布局状态，因为新图像的布局是 UNDEFINED ===
        pickingImageCurrentLayout = VK_IMAGE_LAYOUT_UNDEFINED;
        //diff: [test_picking4] end
    };
    // NOLINTNEXTLINE
    ...
    while (window.shouldClose() == 0)
    {
        ...
        if (curPos != lastMouse)
        {
            ...
            //NOTE: 无法完全解决，说明存在逻辑BUG
            //diff: [test_picking4] start 解决拉伸引发越界，应该用旧窗口大小
            // 获取拾取图像的实际尺寸（不会瞬间改变）或者从 pickingImage 查询
            VkExtent3D pickExtent = build_pick.createInfo().extent;
            int px = static_cast<int>(std::round(curPos.xpos));
            int py = static_cast<int>(std::round(curPos.ypos));

            // 边界检查用实际图像尺寸
            if (px < 0 || px >= pickExtent.width || py < 0 || py >= pickExtent.height)
            {
