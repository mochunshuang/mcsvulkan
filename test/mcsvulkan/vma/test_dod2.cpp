...
struct mesh_base
{
    ...
    static void updateObjectData(FrameBuffers &fb, const auto &get_model_matrix) noexcept
    {
        ...
        ::memcpy(fb.objectDataBuffer.mapPtr(), &data, sizeof(data));
    }
    // diff: [test_model_matrix2] end
...
namespace mesh
{
                                          ...
                                          const std::vector<index_type> &indices)
    {
        ...
        fb.index_sequence = indices; //copy
...
try
{
    ...
    std::array<auto_map_buffer, MAX_FRAMES_IN_FLIGHT> uniformBuffers;
    for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++)
    {
        uniformBuffers[i] = auto_map_buffer(create_buffer(
            allocator,
            {.sType = sType<VkBufferCreateInfo>(),
             .size = BUFFER_SIZE,
    ...
    // autoSpin
    // 两个容器：按行为分离
    std::vector<mesh_base> autoSpinMeshes;
    autoSpinMeshes.reserve(1);
    autoSpinMeshes.emplace_back(mesh_base{allocator,
                                          commandPool,
                                          GRAPHICS_AND_PRESENT,
                                          {.vertices = vertices, .indices = indices}});

    std::vector<mesh_base> interactiveMeshes;
    interactiveMeshes.reserve(1);
    interactiveMeshes.emplace_back(
        mesh_base{allocator,
                  commandPool,
                  GRAPHICS_AND_PRESENT,
    ...
    // mesh_base &input_mesh2 = interactiveMeshes[0];
    ...
    auto autoSpinMeshCtx =
        make_aggregate_ref<"autoSpinMeshCtx", "vertexData", "objectDataMap",
            ...
            autoSpinVertexData, autoSpinObjectDataMap, autoSpinVertexUpdateQueue,
            autoSpinTexture, autoSpinSampler);
    ...
    auto dataCtx = make_aggregate_ref<"dataCtx", "autoSpinMeshes", "interactiveMeshes",
                                      "autoSpinTexture", "interactiveTexture",
                                      "autoSpinSampler", "interactiveSampler">(
        autoSpinMeshes, interactiveMeshes, autoSpinTexture, interactiveTexture,
        autoSpinSampler, interactiveSampler);

    auto inputCtx = make_aggregate_ref<"inputCtx", "input">(input);

    auto world = make_aggregate_ref<"world", "globalCtx", "mainCtx", "mainShaderCtx",
                                    "pickCtx", "dataCtx", "inputCtx", "meshCtx">(
        globalCtx, mainCtx, mainShaderCtx, pickCtx, dataCtx, inputCtx, meshCtx);
...
#define DOD_TEST 1 // NOLINT

#if (DOD_TEST == 0)
    //diff: [test_dod2] 过度阶段到此
    auto modelState = model_state{};
    glm::vec3 topLeftLocal = mesh::get_topLeftLocal(
        {.vertices = vertices | std::views::transform([](const Vertex &vertex) {
                         auto newvertex = vertex;
                         newvertex.pos[2] = 0.5;
                         return newvertex;
    ...
    auto views_matrix_update = [](world_type &world) {
    ...
    auto views_perspective_update = [](world_type &world) {
        const auto &input = world.inputCtx.input;
        auto &camera = world.globalCtx.camera;
        using mcs::vulkan::event::Key;
...
#if (DOD_TEST == 0)
    auto model_update = [&modelState, &topLeftLocal](world_type &world) {
        auto &globalCtx = world.globalCtx;
    ...
    static constexpr auto prepareBatch = [](world_type &world, uint32_t currentFrame) {
        ...
        auto autoSpinObjs =
            std::views::zip(dataCtx.autoSpinMeshes, dataCtx.autoSpinTexture,
                            dataCtx.autoSpinSampler) |
        ...
        auto interactiveObjs =
            std::views::zip(dataCtx.interactiveMeshes, dataCtx.interactiveTexture,
                            dataCtx.interactiveSampler) |
            std::views::transform([](auto &&tuple) {
                auto &[mesh, tex, samp] = tuple;
                return BatchObj{.mesh = &mesh, .tex = tex, .samp = samp};
            }) |
        ...
        for (uint32_t i = 0; i < objectCount; ++i)
        {
            ...
            // 拷贝索引数据到合并缓冲
            memcpy((uint32_t *)batch.mergedIndexBuffer.mapPtr() + idxOff,
    ...
    const auto drawFrame = [
        ...
        mesh_base::updateObjectData(dataCtx.autoSpinMeshes[0].frameBuffers[currentFrame],
                                    [&] {
                                        return glm::rotate(glm::mat4(1.0F), mesh1_model,
                                                           glm::vec3(0.0F, 0.0F, 1.0F));
                                    });
        mesh_base::updateObjectData(
            dataCtx.interactiveMeshes[0].frameBuffers[currentFrame],
            [&] { return mesh2_model; });
        // diff: [test_indirectdraw] end

        // diff: [test_indirectdraw] start
        dataCtx.autoSpinMeshes[0].requestUpdate(allocator, currentFrame);
        dataCtx.interactiveMeshes[0].requestUpdate(
        ...
        commandBuffer.reset({});
        recordCommandBuffer(world, commandBuffer,
