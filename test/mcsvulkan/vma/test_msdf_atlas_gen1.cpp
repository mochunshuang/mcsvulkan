...
namespace font
{
    ...
    */
    ...
    struct glyph_info
    {
        ...
        double advance;         // 水平步进（用于布局）
        double distanceRange;   // msdf 需要
        // 可选：planeBounds 用于精确尺寸
    };
    ...
    class FontLibrary
    {
      ...
      public:
        FontLibrary() = default;
        void load(FontType type, uint32_t texIdx, uint32_t sampIdx,
            ...
            for (const auto &atlas : fonts_)
            {
                ...
                for (const auto &glyph : font.glyphs)
                {
                    if (unicode == glyph.unicode)
                    {
                        ...
                        if (!atlasBounds)
                        { // 空格等
                            return {.type = atlas.type,
                                    .texture_index = atlas.textureIndex,
                                    .sampler_index = atlas.samplerIndex,
                                    .uv_bounds = {.left = 0,
                                                  .bottom = 0,
                                                  .right = 0,
                                                  .top = 0},
                        ...
                        double left = atlasBounds->left / w;
                        double bottom = (h - atlasBounds->bottom) / h; // 翻转 Y
                        double right = atlasBounds->right / w;
                        double top = (h - atlasBounds->top) / h;
                        return {.type = atlas.type,
                                .texture_index = atlas.textureIndex,
                                .sampler_index = atlas.samplerIndex,
                                .uv_bounds = {.left = left,
        ...
        using uv_bounds = Font::Bounds;
        uv_bounds get(char32_t unicode)
        {
            for (const auto &atlas : fonts_)
            {
                const auto &font = atlas.font;
                auto atlasWidth = font.atlas.width;
                ...
                for (const auto &glyph : font.glyphs)
                {
                    if (unicode == glyph.unicode)
                    {
                        ...
                        auto raw_bounds = *atlasBounds;
                        double left = raw_bounds.left;
                        double bottom = raw_bounds.bottom;
                        double right = raw_bounds.right;
                        double top = raw_bounds.top;
                        // NOTE: 翻转Y
                        return {.left = left / atlasWidth,
                                .bottom = (atlasHeight - bottom) / atlasHeight,
                                .right = right / atlasWidth,
                                .top = (atlasHeight - top) / atlasHeight};
                    }
                }
            }
            throw std::runtime_error{"unsuported now."};
...
try
{
    ...
    font_lib.load(font::FontType::eMSDF, 0, 0, // textureIndex=0, samplerIndex=0
                  ...
                  JSON_PATH_0);
    font_lib.load(font::FontType::eMSDF, 1,
                  0, // 第二个字体用 textureIndex=1, samplerIndex=0
    ...
    auto [left, bottom, right, top] = font_lib.get(CHAR);
    ...
    for (char32_t ch : text)
    {
        ...
        // std::vector<Vertex> vertices = {
        //     {{-0.5f, -0.5f, 0.0F}, {1.0F, 0.0F, 0.0F}, {left, bottom}}, // 左下
        //     {{0.5f, -0.5f, 0.0F}, {0.0F, 1.0F, 0.0F}, {right, bottom}}, // 右下
        //     {{0.5f, 0.5f, 0.0F}, {0.0F, 0.0F, 1.0F}, {right, top}},     // 右上
        //     {{-0.5f, 0.5f, 0.0F}, {1.0F, 0.0F, 0.0F}, {left, top}}};    // 左上
        constexpr auto flip_y = -1;
        auto info = font_lib.getGlyph(ch);
        double distanceRange = info.distanceRange;
        float w = (info.uv_bounds.right - info.uv_bounds.left) * fontSize; // 近似宽度
        float h =
            flip_y * (info.uv_bounds.top - info.uv_bounds.bottom) * fontSize; // 近似高度

        glm::vec3 pos_left_bottom(cursorX, cursorY, 0);
        glm::vec3 pos_right_bottom(cursorX + w, cursorY, 0);
        glm::vec3 pos_right_top(cursorX + w, cursorY + h, 0);
        glm::vec3 pos_left_top(cursorX, cursorY + h, 0);

        // NOTE: 硬编码对 调试非常重要，找到了 flip_y
        //  pos_left_bottom = {-0.5f, -0.5f, 0.0F};
        //  pos_right_bottom = {0.5f, -0.5f, 0.0F};
        //  pos_right_top = {0.5f, 0.5f, 0.0F};
        //  pos_left_top = {-0.5f, 0.5f, 0.0F};
    ...
    const auto recordCommandBuffer = [&](const CommandBufferView &commandBuffer,
        ...
        // 绘制单个 字符
        {
            ...
            commandBuffer.pushConstants(*pipelineLayout, VK_SHADER_STAGE_VERTEX_BIT, 0,
                                        sizeof(PushConstants), &vertexBufferAddress);
