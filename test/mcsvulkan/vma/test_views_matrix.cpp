...
*/
    ...
    const auto recordCommandBuffer = [&](const CommandBufferView &commandBuffer,
        ...
        auto mvp = [&]() {
            ...
            //
            // 视图矩阵（View Matrix）将世界坐标变换到相机空间，其核心由 glm::lookAt(eye,
            // center, up) 定义：
            //   - eye：相机在世界中的位置
            //   - center：相机注视的目标点
            //   - up：相机的上方向向量（通常为 (0,1,0) 或 (0,0,1) 等）
            //
            // 下面每个测试用例都改变了 eye 的位置，而 center 和 up
            // 可能保持不变或随之变化， 目的是让你观察相机在3D空间中移动时场景的透视变化。
            //
            // 当前激活的用例：**用例 1**（默认位置 (2,2,2)）
            // --------------------------------------------------------------------

            // ----------------------------- 用例 1：静态默认视角
            // ---------------------------- 相机固定在 (2,2,2) 点，始终注视原点，上方向为
            // Z 轴 (0,0,1) 效果：物体呈现立体静态视图，适合作为参考基准
            // --------------------------------------------------------------------
            // ubo.view =
            ...
            // ----------------------------- 用例 2：X 轴方向平移（左右移动）
            // -------------------- 相机在 X 轴上随时间左右移动，始终注视原点
            // 效果：物体看起来左右摆动，体验视差效果
            // --------------------------------------------------------------------
            // glm::vec3 eyePos = glm::vec3(3.0F * sin(time), 0.0F, 3.0F); // X 轴摆动
            // ubo.view = glm::lookAt(eyePos, glm::vec3(0.0F, 0.0F, 0.0F),
            //                        glm::vec3(0.0F, 1.0F, 0.0F));

            // ----------------------------- 用例 3：Y 轴升降（上下移动）
            // -------------------- 相机在 Y 轴上随时间上下移动，始终注视原点
            // 效果：物体上下俯仰变化，显示不同高度视角
            // --------------------------------------------------------------------
            // glm::vec3 eyePos = glm::vec3(2.0F, 5 * sin(time), 2.0F); // Y 轴摆动
            // ubo.view = glm::lookAt(eyePos, glm::vec3(0.0F, 0.0F, 0.0F),
            //                        glm::vec3(0.0F, 1.0F, 0.0F));
            ...
            // 效果：物体拉近拉远，模拟推拉镜头效果
            // --------------------------------------------------------------------
            // glm::vec3 eyePos = glm::vec3(0.0F, 0.0F, 5.0F + 3.0F * sin(time)); //
            // Z轴前后 ubo.view = glm::lookAt(eyePos, glm::vec3(0.0F, 0.0F, 0.0F),
            //                        glm::vec3(0.0F, 1.0F, 0.0F));

            // ----------------------------- 用例 5：环绕运动（绕 Y 轴旋转）
            // -------------------- 相机在 XZ 平面上绕 Y 轴做圆周运动，始终注视原点
            // 效果：物体被环绕拍摄，展示各个侧面，是典型的“轨道相机”
            // --------------------------------------------------------------------
            // glm::vec3 eyePos =
            //     glm::vec3(4.0F * cos(time), 2.0F, 4.0F * sin(time)); // 绕 Y 轴旋转
            // ubo.view = glm::lookAt(eyePos, glm::vec3(0.0F, 0.0F, 0.0F),
            //                        glm::vec3(0.0F, 1.0F, 0.0F));

            // ----------------------------- 用例 6：倾斜环绕（绕空间任意轴）
            // -------------------- 相机绕倾斜轴旋转，同时注视原点，产生翻滚效果
            // 效果：类似过山车视角，展示非水平环绕
            // --------------------------------------------------------------------
            // float radius = 5.0F;
            // glm::vec3 eyePos = glm::vec3(radius * cos(time), radius * sin(time) * 0.5F,
            //                              radius * sin(time));
            // ubo.view = glm::lookAt(eyePos, glm::vec3(0.0F, 0.0F, 0.0F),
            //                        glm::vec3(0.0F, 1.0F, 0.0F));

            // ----------------------------- 用例 7：沿复杂轨迹运动（正弦波路径）
            // -------------------- 相机沿一条三维正弦曲线运动，始终注视原点
            // 效果：相机运动轨迹复杂，产生变幻的透视效果
            // --------------------------------------------------------------------
            // glm::vec3 eyePos =
            //     glm::vec3(4.0F * sin(time), 3.0F * cos(time * 1.5F), 4.0F * cos(time));
            // ubo.view = glm::lookAt(eyePos, glm::vec3(0.0F, 0.0F, 0.0F),
            //                        glm::vec3(0.0F, 1.0F, 0.0F));

            // ----------------------------- 用例 8：移动注视点 + 移动相机（跟拍）
            // --------------------
            // 相机位置和注视点同时移动，模拟跟拍效果（例如相机跟随移动物体）
            // 这里假设目标物体绕 Y 轴旋转，相机始终从侧面跟随
            // --------------------------------------------------------------------
            // 目标位置
            // glm::vec3 targetPos =
            //     glm::vec3(2.0F * cos(time * 0.5F), 0.0F, 2.0F * sin(time * 0.5F));
            // // 相机相对于目标的位置偏移
            // glm::vec3 eyeOffset = glm::vec3(4.0F, 2.0F, 4.0F);
            // glm::vec3 eyePos = targetPos + eyeOffset;
            // ubo.view = glm::lookAt(eyePos, targetPos, glm::vec3(0.0F, 1.0F, 0.0F));

            // ----------------------------- 用例 9：自由飞行（修正版）
            // --------------------
            // 相机沿一条螺旋路径移动，但始终注视原点，保证物体始终可见
            // 效果：相机在空间中围绕物体飞行，展现动态观察效果
            // --------------------------------------------------------------------
            glm::vec3 eyePos = glm::vec3(4.0F * sin(time * 0.5F), 2.0F * sin(time * 0.3F),
                                         4.0F * cos(time * 0.5F));
            ubo.view = glm::lookAt(eyePos, glm::vec3(0.0F, 0.0F, 0.0F),
                                   glm::vec3(0.0F, 1.0F, 0.0F));

            //
            // 通过切换上述任一用例，你可以观察到相机位置变化如何影响物体在屏幕上的呈现。
            // 视图矩阵本质上是一个将世界坐标系变换到相机坐标系的矩阵，其变化直接反映了观察者的空间状态。
            // 建议结合时间变量 time（已在代码中定义）来体验动态效果。
