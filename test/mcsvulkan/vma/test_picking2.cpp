...
*/
    ...
    // ========== 拾取资源 ==========
    // 创建拾取图像（R32G32_UINT）
    VkImageCreateInfo pickImageInfo = {VK_STRUCTURE_TYPE_IMAGE_CREATE_INFO};
    pickImageInfo.imageType = VK_IMAGE_TYPE_2D;
    pickImageInfo.format = VK_FORMAT_R32G32_UINT;
    ...
    pickImageInfo.initialLayout = VK_IMAGE_LAYOUT_UNDEFINED;

    VmaAllocationCreateInfo pickAllocInfo = {};
    pickAllocInfo.usage = VMA_MEMORY_USAGE_AUTO;
    pickAllocInfo.flags = VMA_ALLOCATION_CREATE_DEDICATED_MEMORY_BIT;
    ...
    pickViewInfo.image = pickingImage;
    pickViewInfo.viewType = VK_IMAGE_VIEW_TYPE_2D;
    pickViewInfo.format = VK_FORMAT_R32G32_UINT;
    pickViewInfo.subresourceRange.aspectMask = VK_IMAGE_ASPECT_COLOR_BIT;
    pickViewInfo.subresourceRange.levelCount = 1;
    pickViewInfo.subresourceRange.layerCount = 1;
    VkImageView pickingImageView = device.createImageView(pickViewInfo, nullptr);

    // ========== 拾取管线布局 ==========
    // 推送常量范围：mat4 + uint
    VkPushConstantRange pickPushRange = {
        .stageFlags = VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT,
        .offset = 0,
        .size = sizeof(glm::mat4) + sizeof(uint32_t) // 64+4=68，对齐到4即可
    };

    VkPipelineLayoutCreateInfo pickLayoutInfo = {
        VK_STRUCTURE_TYPE_PIPELINE_LAYOUT_CREATE_INFO};
    pickLayoutInfo.setLayoutCount = 1;
    pickLayoutInfo.pSetLayouts = &(*descriptorSetLayout); // 复用主渲染的描述符集布局
    pickLayoutInfo.pushConstantRangeCount = 1;
    pickLayoutInfo.pPushConstantRanges = &pickPushRange;
    VkPipelineLayout pickingPipelineLayout =
        device.createPipelineLayout(pickLayoutInfo, nullptr);

    // ========== 拾取管线 ==========
    // 顶点输入状态（匹配 Vertex 结构）
    VkVertexInputBindingDescription bindingDesc = {
        .binding = 0, .stride = sizeof(Vertex), .inputRate = VK_VERTEX_INPUT_RATE_VERTEX};
    VkVertexInputAttributeDescription attrDesc = {.location = 0,
                                                  .binding = 0,
    ...
    // 从文件加载拾接着色器（需要提前创建 picking.vert 和 picking.frag）
    auto pickStages = create_graphics_pipeline::makeStages(
        stage_info{.stage = VK_SHADER_STAGE_VERTEX_BIT,
    ...
    //     pickStages | std::views::transform([&](const stage_info &create) constexpr {
    //         return create(device);
    //     }) |
    //     std::ranges::to<std::vector<stage_info::result_type>>();
    // auto stages = stages_createInfos |
    //               std::views::transform(
    ...
    auto code = mcs::vulkan::read_file("shaders/picking.vert.spv");
    ...
    auto FragshaderModule =
        device.createShaderModule({.sType = sType<VkShaderModuleCreateInfo>(),
                                   ...
                                   .pCode = std::bit_cast<const uint32_t *>(code.data())},
                                  nullptr);
    std::cout << "stages FragshaderModule 完成\n";
    ...
    // 动态渲染信息
    VkPipelineRenderingCreateInfo pickRenderingInfo = {
        .sType = VK_STRUCTURE_TYPE_PIPELINE_RENDERING_CREATE_INFO,
        .colorAttachmentCount = 1,
        .pColorAttachmentFormats = &pickImageInfo.format,
        .depthAttachmentFormat = depthFormat_ref};

    VkGraphicsPipelineCreateInfo pickPipeInfo = {
        VK_STRUCTURE_TYPE_GRAPHICS_PIPELINE_CREATE_INFO};
    pickPipeInfo.pNext = &pickRenderingInfo;
    pickPipeInfo.stageCount = 2;
    pickPipeInfo.pStages = shaderStages;
    pickPipeInfo.pVertexInputState = &vertexInputState;
    // 其余状态与主渲染类似，可复用主渲染的配置（但需要单独创建）
    ...
    VkPipelineRasterizationStateCreateInfo rasterState = {
        ...
        .rasterizerDiscardEnable = VK_FALSE,
        .polygonMode = VK_POLYGON_MODE_FILL,
        .cullMode = VK_CULL_MODE_NONE, // 与主渲染一致
        .frontFace = VK_FRONT_FACE_CLOCKWISE,
        .lineWidth = 1.0f};
    ...
    VkPipelineDynamicStateCreateInfo dynamicState = {
        ...
        .dynamicStateCount = 2,
        .pDynamicStates = std::array<VkDynamicState, 2>{VK_DYNAMIC_STATE_VIEWPORT,
                                                        VK_DYNAMIC_STATE_SCISSOR}
                              .data()};

    pickPipeInfo.pInputAssemblyState = &inputAssembly;
    pickPipeInfo.pViewportState = &viewportState;
    pickPipeInfo.pRasterizationState = &rasterState;
    pickPipeInfo.pMultisampleState = &multisample;
    pickPipeInfo.pDepthStencilState = &depthStencil;
    pickPipeInfo.pColorBlendState = &colorBlend;
    pickPipeInfo.pDynamicState = &dynamicState;
    pickPipeInfo.layout = pickingPipelineLayout;
    pickPipeInfo.renderPass = VK_NULL_HANDLE; // 使用动态渲染
    ...
    const auto recordCommandBuffer = [&](const CommandBufferView &commandBuffer,
        ...
        if (enable_picking)
        {
            ...
            my_render::transition_image_layout(
                ...
                pickingImageCurrentLayout, VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL, 0,
            ...
            VkRenderingInfo pickRenderingInfo = {
                ...
                .layerCount = 1,
                .colorAttachmentCount = 1,
                .pColorAttachments = &pickColorAttachment,
                .pDepthAttachment = &pickDepthAttachment};
            commandBuffer.beginRendering(pickRenderingInfo);
            ...
            // 绘制第一个 mesh (ID=0)
            {
                ...
                struct PickPush
                {
                    glm::mat4 model;
                    uint32_t objectId;
                } push = {.model = mesh1_model, .objectId = 0};
            ...
            // 绘制第二个 mesh (ID=1)
            {
                struct PickPush
                {
                    glm::mat4 model;
                    uint32_t objectId;
                } push = {.model = mesh2_model, .objectId = 1};
                commandBuffer.pushConstants(pickingPipelineLayout,
                                            VK_SHADER_STAGE_VERTEX_BIT |
                                                VK_SHADER_STAGE_FRAGMENT_BIT,
                                            0, sizeof(push), &push);
    ...
    const auto recreateSwapChain = [&]() constexpr {
        ...
        vmaDestroyImage(allocator, pickingImage, pickingAllocation);

        // 重新创建，尺寸使用 swapchain.refImageExtent()
        VkImageCreateInfo newPickInfo = pickImageInfo; // 从原信息复制
        newPickInfo.extent = {swapchain.refImageExtent().width,
                              swapchain.refImageExtent().height, 1};
        vmaCreateImage(allocator, &newPickInfo, &pickAllocInfo, &pickingImage,
                       &pickingAllocation, nullptr);

        VkImageViewCreateInfo newViewInfo = pickViewInfo;
    ...
    while (window.shouldClose() == 0)
    {
        ...
        if (curPos != lastMouse)
        {
            ...
            VmaAllocationInfo stagingInfoMapped;
            ...
            // 提交并等待
            Fence copyFence{device, {.sType = VK_STRUCTURE_TYPE_FENCE_CREATE_INFO}};
    ...
    // diff: [test_picking] start
    device.destroyPipeline(pickingPipeline, nullptr);
    device.destroyPipelineLayout(pickingPipelineLayout, nullptr);
    device.destroyImageView(pickingImageView, nullptr);
    vmaDestroyImage(allocator, pickingImage, pickingAllocation);
