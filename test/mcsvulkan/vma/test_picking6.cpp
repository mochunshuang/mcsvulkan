...
// NOTE: 无法一对一映射，不可能自动配置，下面的代码是无用的。不过门道还是能看到一些了。 同步 不再难
namespace vulkan
{
    enum layout_type : std::uint8_t
    {
        eSRC,
        eDEST
    };
    template <layout_type type>
    static constexpr VkAccessFlags2 getVkAccessFlags2(VkImageLayout layout)
    {
        switch (layout)
        {
        // -------------------- 通用 / 未定义 --------------------
        ...
        case VK_IMAGE_LAYOUT_PREINITIALIZED:
            // src 必须包含 VK_ACCESS_2_HOST_WRITE_BIT
            return (type == eSRC) ? VK_ACCESS_2_HOST_WRITE_BIT : VK_ACCESS_2_NONE;
        ...
        case VK_IMAGE_LAYOUT_READ_ONLY_OPTIMAL:
            return (type == eSRC) ? VK_ACCESS_2_NONE
        ...
        // 分离深度/模板布局 —— 仍使用 DEPTH_STENCIL 访问位，但补充着色器读
        case VK_IMAGE_LAYOUT_DEPTH_READ_ONLY_STENCIL_ATTACHMENT_OPTIMAL: { // 深度只读（附件/着色器），模板读写
            if (type == eSRC)
                return VK_ACCESS_2_DEPTH_STENCIL_ATTACHMENT_WRITE_BIT; // 模板写
            return VK_ACCESS_2_DEPTH_STENCIL_ATTACHMENT_READ_BIT |
                   VK_ACCESS_2_DEPTH_STENCIL_ATTACHMENT_WRITE_BIT |
        ...
        case VK_IMAGE_LAYOUT_DEPTH_ATTACHMENT_STENCIL_READ_ONLY_OPTIMAL:
            // 深度读写，模板只读（附件/着色器）
            if (type == eSRC)
                return VK_ACCESS_2_DEPTH_STENCIL_ATTACHMENT_WRITE_BIT; // 深度写
            else
                return VK_ACCESS_2_DEPTH_STENCIL_ATTACHMENT_READ_BIT |
                       VK_ACCESS_2_DEPTH_STENCIL_ATTACHMENT_WRITE_BIT |
                       VK_ACCESS_2_SHADER_READ_BIT |
        ...
        case VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL:
            return (type == eSRC) ? VK_ACCESS_2_NONE : VK_ACCESS_2_TRANSFER_READ_BIT;
        ...
        case VK_IMAGE_LAYOUT_VIDEO_DECODE_SRC_KHR:
            return VK_ACCESS_2_NONE; // 保留

        case VK_IMAGE_LAYOUT_VIDEO_DECODE_DPB_KHR:
            return (type == eSRC) ? VK_ACCESS_2_VIDEO_DECODE_WRITE_BIT_KHR
        ...
        case VK_IMAGE_LAYOUT_VIDEO_ENCODE_DPB_KHR:
            return (type == eSRC) ? VK_ACCESS_2_VIDEO_ENCODE_WRITE_BIT_KHR
                                  : VK_ACCESS_2_VIDEO_ENCODE_READ_BIT_KHR |
        ...
        default:
            assert(!"Unhandled VkImageLayout in getVkAccessFlags2");
            return VK_ACCESS_2_NONE;
        }
    }
}; // namespace vulkan
...
struct my_render
{
    ...
    // diff: [test_picking6] 别想了 vulkan 自己都做不到 一对一映射。 多对一 一对一 多对多 混合，不是styple那种唯一
    // https://docs.vulkan.org/refpages/latest/refpages/source/VkImageLayout.html#:~:text=%E4%B8%8D%E5%BD%B1%E5%93%8D%E6%80%A7%E8%83%BD%E3%80%82-,VK_IMAGE_LAYOUT_GENERAL,-%E5%8F%AF%E4%BB%A5%E4%BD%9C%E4%B8%BA%E4%B8%80%E4%B8%AA
    /*
您要的一手证据链路如下：

URL 1 (Layout → Access): https://registry.khronos.org/vulkan/specs/latest/man/html/VkImageLayout.html 中每个布局的 "must only be used" 描述

URL 2 (Access → Stage): https://registry.khronos.org/vulkan/specs/latest/html/vkspec.html#synchronization-access-types-supported 中的 Table 4
*/
    // NOTE: 简化
...
try
{
    ...
    const auto recordCommandBuffer = [&](const CommandBufferView &commandBuffer,
                                         ...
                                         0, 1, &(descriptorSet), 0, nullptr);
        {
            ...
            commandBuffer.drawIndexed(input_mesh.frameBuffers[currentFrame].indexCount, 1,
                                      0, 0, 0);
        }
        {
            uint64_t vertexBufferDeviceAddress =
            ...
            commandBuffer.drawIndexed(input_mesh2.frameBuffers[currentFrame].indexCount,
                                      1, 0, 0, 0);
        }
        ...
        if (mouseValid)
        {
            ...
            // 绘制物体1
            {
                ...
                commandBuffer.drawIndexed(
                    input_mesh.frameBuffers[currentFrame].indexCount, 1, 0, 0, 0);
            }
            // 绘制物体2
            {
                PushConstantsPicking push{};
                ...
                commandBuffer.bindIndexBuffer(
                    ...
                    VK_INDEX_TYPE_UINT32);
                commandBuffer.drawIndexed(
                    input_mesh2.frameBuffers[currentFrame].indexCount, 1, 0, 0, 0);
            }
            commandBuffer.endRendering();
