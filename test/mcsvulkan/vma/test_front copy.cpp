...
struct FontChar
{
    float x, y, w, h;                                   // 在图集中的坐标和尺寸（像素）
    float advance;                                      // 步进（em）
    float planeLeft, planeBottom, planeRight, planeTop; // 平面坐标（em）
};
// diff: [test_front] end
...
*/
    ...
    {
        std::ifstream f("E:/0_github_project/mcsvulkan/assets/font/font_msyh.json");
        if (!f.is_open())
        {
            throw std::runtime_error("Failed to open font_msyh.json");
        }
        json data = json::parse(f);

        atlasWidth = data["atlas"]["width"];
        atlasHeight = data["atlas"]["height"];
        ...
        for (auto &glyph : data["glyphs"])
        {
            ...
            fc.x = glyph["atlasBounds"]["left"];
            fc.y = glyph["atlasBounds"]["top"];
            fc.w = glyph["atlasBounds"]["right"].get<float>() -
                   glyph["atlasBounds"]["left"].get<float>();
            fc.h = glyph["atlasBounds"]["bottom"].get<float>() -
                   glyph["atlasBounds"]["top"].get<float>();
            ...
            fontChars[unicode] = fc;
        }
        std::cout << "Loaded " << fontChars.size() << " glyphs.\n";
    }

    // 2. 加载字体纹理
    TextureImage fontTexture(allocator, device, commandPool, GRAPHICS_AND_PRESENT,
                             "E:/0_github_project/mcsvulkan/assets/font/font_msyh.png",
                             false, 0);

    // 3. 创建字体采样器（线性过滤）
    TextureSampler fontSampler(device, 0); // 线性采样器

    // 4. 生成文字网格（“你好”）
    auto generateTextMesh = [&](const std::string &text, float scale = 1.0f / 36.0f) {
        std::vector<Vertex> vertices;
        ...
        for (char32_t cp : utf32)
        {
            ...
            float u0 = fc.x / atlasWidth;
            float v0 = fc.y / atlasHeight;
            float u1 = (fc.x + fc.w) / atlasWidth;
            float v1 = (fc.y + fc.h) / atlasHeight;

            float x0 = posx + fc.planeLeft * scale;
            float x1 = posx + fc.planeRight * scale;
            float y0 = fc.planeBottom * scale;
            float y1 = fc.planeTop * scale;

            glm::vec3 color(1.0f); // 白色

            vertices.push_back({{x1, y1, 0.0f}, color, {u1, v1}});
            vertices.push_back({{x0, y1, 0.0f}, color, {u0, v1}});
            vertices.push_back({{x0, y0, 0.0f}, color, {u0, v0}});
            vertices.push_back({{x1, y0, 0.0f}, color, {u1, v0}});
        ...
        // for (auto &v : vertices)
        // {
        //     v.pos.x -= totalWidth / 2.0f;
        //     v.pos.y -= 0.5f * scale;
        //     v.pos.z = 0.2f; // 稍微靠前，避免被遮挡
        // }
        // 临时：放在屏幕中心（需传入屏幕尺寸，但这里无法获取，暂且固定一个位置）
        for (auto &v : vertices)
        {
            v.pos.x += 400.0f; // 假设屏幕宽800，居中偏移400
            v.pos.y += 300.0f; // 假设屏幕高600，居中偏移300
            v.pos.z = 0.0f;
        }
        return std::pair{vertices, indices};
    };

    auto [fontVertices, fontIndices] = generateTextMesh("你好", 1.0f / 36.0f);
    mesh_base fontMesh(allocator, commandPool, GRAPHICS_AND_PRESENT,
                       {fontVertices, fontIndices});
    // diff: [test_front] end
    ...
    // diff: [test_front] start
    // ---------- 字体管线创建 ----------
    // 4.1 字体描述符集布局（单个 combined image sampler）
    ...
    VkDescriptorSetLayoutCreateInfo fontDescLayoutInfo{};
    fontDescLayoutInfo.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_SET_LAYOUT_CREATE_INFO;
    fontDescLayoutInfo.bindingCount = 1;
    fontDescLayoutInfo.pBindings = &fontBinding;
    VkDescriptorSetLayout fontDescSetLayout =
        device.createDescriptorSetLayout(fontDescLayoutInfo, nullptr);

    // 4.2 字体描述符池
    VkDescriptorPoolSize fontPoolSize{};
    fontPoolSize.type = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
    fontPoolSize.descriptorCount = 1;
    VkDescriptorPoolCreateInfo fontPoolInfo{};
    fontPoolInfo.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_POOL_CREATE_INFO;
    fontPoolInfo.maxSets = 1;
    fontPoolInfo.poolSizeCount = 1;
    fontPoolInfo.pPoolSizes = &fontPoolSize;
    VkDescriptorPool fontDescPool = device.createDescriptorPool(fontPoolInfo, nullptr);

    // 4.3 分配字体描述符集
    VkDescriptorSetAllocateInfo fontDescAllocInfo{};
    fontDescAllocInfo.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_SET_ALLOCATE_INFO;
    fontDescAllocInfo.descriptorPool = fontDescPool;
    fontDescAllocInfo.descriptorSetCount = 1;
    fontDescAllocInfo.pSetLayouts = &fontDescSetLayout;
    auto fontDescriptorSets = device.allocateDescriptorSets(fontDescAllocInfo);
    VkDescriptorSet fontDescSet = fontDescriptorSets[0];

    // 4.4 更新字体描述符集（绑定纹理和采样器）
    VkDescriptorImageInfo fontImageInfo{};
    fontImageInfo.sampler = fontSampler.sampler();
    fontImageInfo.imageView = *fontTexture.imageView();
    ...
    device.updateDescriptorSets(1, &fontWrite, 0, nullptr);

    // 4.5 字体推送常量结构（在着色器中使用）
    struct FontPushConstants
    {
        glm::mat4 proj;  // 正交投影矩阵
        glm::vec4 color; // 字体颜色
    };

    // 4.6 字体管线布局
    VkPushConstantRange fontPcRange{};
    fontPcRange.stageFlags = VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT;
    fontPcRange.offset = 0;
    fontPcRange.size = sizeof(FontPushConstants);
    ...
    fontPipelineLayoutInfo.setLayoutCount = 1;
    fontPipelineLayoutInfo.pSetLayouts = &fontDescSetLayout;
    fontPipelineLayoutInfo.pushConstantRangeCount = 1;
    fontPipelineLayoutInfo.pPushConstantRanges = &fontPcRange;
    VkPipelineLayout fontPipelineLayout =
        device.createPipelineLayout(fontPipelineLayoutInfo, nullptr);

    // 4.7 创建字体管线（使用你的 create_graphics_pipeline 工具）
    // 注意：需要为字体管线配置合适的混合和深度状态
    auto fontPipeline =
        create_graphics_pipeline{}
            .setCreateInfo(
                {.pNext = make_pNext(structure_chain<VkPipelineRenderingCreateInfo>{
                 ...
                 .stages = create_graphics_pipeline::makeStages(
                     stage_info{.stage = VK_SHADER_STAGE_VERTEX_BIT,
                                .filePath = "shaders/font.vert.spv",
                                .pName = "main"},
                     stage_info{.stage = VK_SHADER_STAGE_FRAGMENT_BIT,
                                .filePath = "shaders/font.frag.spv",
                                .pName = "main"}),
                 .vertexInputState =
                     {// 顶点输入绑定描述（与 Vertex 结构匹配）
                 ...
                 .multisampleState =
                     {.rasterizationSamples = static_cast<VkSampleCountFlagBits>(
                          physical_device.getMaxUsableSampleCount()), // 与主渲染目标一致
                      .sampleShadingEnable = VK_FALSE,
                      .minSampleShading = 1.0F,
                      .sampleMask = {},
                      .alphaToCoverageEnable = VK_FALSE,
    ...
    {
        auto [fontVertices, fontIndices] = generateTextMesh("你好", 1.0f / 36.0f);
        std::cout << "fontVertices size: " << fontVertices.size()
                  << ", indices size: " << fontIndices.size() << std::endl;
        if (fontVertices.size() > 0)
        {
            std::cout << "first vertex pos: (" << fontVertices[0].pos.x << ", "
                      << fontVertices[0].pos.y << ", " << fontVertices[0].pos.z << ")"
                      << std::endl;
        }
    }
    ...
    const auto recordCommandBuffer = [&](const CommandBufferView &commandBuffer,
        ...
        {
            // 计算正交投影（屏幕空间，Y向下）
            VkExtent2D extent = imageExtent;
            glm::mat4 ortho = glm::ortho(0.0f, (float)extent.width, (float)extent.height,
                                         0.0f, -1.0f, 1.0f);

            // 推送常量：投影矩阵 + 白色
            FontPushConstants fontPc{ortho, glm::vec4(1.0F)};

            // 绑定字体管线
            commandBuffer.bindPipeline(VK_PIPELINE_BIND_POINT_GRAPHICS, *fontPipeline);
            commandBuffer.bindDescriptorSets(VK_PIPELINE_BIND_POINT_GRAPHICS,
                                             fontPipelineLayout, 0, 1, &fontDescSet, 0,
                                             nullptr);
            commandBuffer.pushConstants(fontPipelineLayout,
                                        VK_SHADER_STAGE_VERTEX_BIT |
                                            VK_SHADER_STAGE_FRAGMENT_BIT,
                                        0, sizeof(FontPushConstants), &fontPc);

            // 绑定字体网格的顶点/索引缓冲（注意使用当前帧的缓冲区）
            commandBuffer.bindVertexBuffers(
                0, 1, fontMesh.frameBuffers[currentFrame].vertexBuffer.buffer(), {0});
            commandBuffer.bindIndexBuffer(
                fontMesh.frameBuffers[currentFrame].indexBuffer.buffer(), 0,
                mesh_base::indexType());

            // 绘制
            commandBuffer.drawIndexed(static_cast<uint32_t>(fontIndices.size()), 1, 0, 0,
                                      0);
        }
        // diff: [test_front] end
    ...
    device.destroyPipelineLayout(fontPipelineLayout, nullptr);
    device.destroyDescriptorSetLayout(fontDescSetLayout, nullptr);
    device.destroyDescriptorPool(fontDescPool, nullptr);
    // diff: [test_front] end
