...
layout(location = 7) out flat uint
    modulateFlag; // NOTE: flat意味着禁用插值，每个片段获得相同的值（来自引发顶点）
// diff: [test_dod12] end
...
void main()
{
    ...
    fragTexCoord = inst.uvTransform.offset +
                   v.texCoord * inst.uvTransform.scale; //diff: [test_dod9] 实例化的基础
