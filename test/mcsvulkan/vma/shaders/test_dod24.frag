...
layout(location = 1) out uvec4 outPicking;
...
pc;
const int RENDER_VERSION_BIT_WIDTH = 16;
#define PRIMITIVE_MASK ((1u << (32 - RENDER_VERSION_BIT_WIDTH)) - 1u)
...
void main()
{
    ...
    uint packed_z = (pc.render_version << RENDER_VERSION_BIT_WIDTH) |
                    (uint(gl_PrimitiveID) & PRIMITIVE_MASK);
    ...
    case 2: { // Glyph：与 test_sdf.frag 的 case 2 完全一致
        ...
        outPicking = uvec4(type_id, inst.entity_index, packed_z, inst.hover_fn);
    }
    break;
    ...
    case 5: { // TYPE_ROUND_RECT：圆角阴影矩形（照抄 test_sdf case 0；不 discard；整块可拾取）
        ...
        outPicking = uvec4(type_id, inst.entity_index, packed_z, ~0u); // hover 未绑定
    }
    break;
    case 4: {                 // TYPE_MESH：普通绘制（无 SDF），实心；实体 = 实例 id
        outColor = fragColor; // 每顶点颜色插值（纯色 / 渐变填充）
        outPicking = uvec4(type_id, instanceId, packed_z, ~0u); // 未绑定
    }
    break;
    default:
        break;
