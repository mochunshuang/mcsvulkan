...
// const float PX_RANGE=8.;//diff: 改成从顶点得到
...
void main(){
    if(fragFontType==FONT_NONE){
        outColor=vec4(fragColor,1.);
    }
    else if(fragFontType==FONT_BITMAP){
        // 彩色位图（如表情符号）：直接采样纹理颜色
        vec4 texColor=texture(sampler2D(textures[nonuniformEXT(fragTextureIndex)],
        samplers[nonuniformEXT(fragSamplerIndex)]),fragTexCoord);
        ...
        if(modulateFlag==1u){
            outColor=vec4(fragColor*opacity,opacity);// 顶点颜色调制
        }else{
            outColor=vec4(vec3(opacity),opacity);// 纯白色（或任何默认颜色）
        }
    }
}