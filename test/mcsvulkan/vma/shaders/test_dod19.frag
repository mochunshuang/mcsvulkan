...
layout(location = 4) in flat uint
    fragObjectId; // diff:[test_indirectdraw_no_pick] 接收物体 ID
...
float screenPxRange()
{
    ivec2 texSize = textureSize(sampler2D(textures[nonuniformEXT(fragTextureIndex)],
...
void main()
{
    ...
    if (fontType == FONT_NONE)
    {
        ...
        vec4 texColor = texture(sampler2D(textures[nonuniformEXT(fragTextureIndex)],
                                          samplers[nonuniformEXT(fragSamplerIndex)]),
                                fragTexCoord);
    ...
    {
        // MSDF / SDF 等距离场字体渲染
        vec3 msd = texture(sampler2D(textures[nonuniformEXT(fragTextureIndex)],
                                     samplers[nonuniformEXT(fragSamplerIndex)]),
                           fragTexCoord)
                       .rgb;
    ...
    outPicking =
        uvec2(fragObjectId,
              gl_PrimitiveID); // diff:[test_indirectdraw_no_pick] 直接使用内置变量
}