...
*/
    ...
    const auto recordCommandBuffer = [&](const CommandBufferView &commandBuffer,
        ...
        auto mvp = [&]() {
            ...
            // NOTE: 不允许这样。仅仅一份
            //  if (obj_id == input_mesh_id)
            //      // 模型矩阵：随时间旋转
            //  ;
            //  else if (obj_id == input_mesh2_id)
            //      // 模型矩阵：随时间旋转
        ...
        }
        {
            ...
            // 测试2：X轴方向往复平移（基础平移）
            mesh_base::updateObjectData(input_mesh2.frameBuffers[currentFrame], [&] {
                float offset = sin(time * 2.0F) * 2.0F; // 在[-2, 2]区间移动
            ...
            // 测试3：均匀缩放（基础缩放）
            mesh_base::updateObjectData(input_mesh2.frameBuffers[currentFrame], [&] {
                float scaleFactor = 1.0F + sin(time * 3.0F) * 0.5F; // 范围[0.5, 1.5]
                return glm::scale(glm::mat4(1.0F), glm::vec3(scaleFactor));
            });
#endif

#if 0
            // 测试4：组合变换（先缩放，再旋转，最后平移）
            mesh_base::updateObjectData(input_mesh2.frameBuffers[currentFrame], [&] {
                glm::mat4 model = glm::mat4(1.0F);
                // 缩放 0.8 倍
                model = glm::scale(model, glm::vec3(0.8F));
                // 绕 Y 轴旋转，随时间变化
                model = glm::rotate(model, time * glm::radians(30.0F),
                                    glm::vec3(0.0F, 1.0F, 0.0F));
                // 沿 X 和 Y 方向平移
                model = glm::translate(model, glm::vec3(1.5F, sin(time) * 1.0F, 0.0F));
                return model;
            });
#endif

#if 1
            // 测试5：复杂的集成叠加（模拟绕任意轴旋转 + 缩放 + 偏移）
            mesh_base::updateObjectData(input_mesh2.frameBuffers[currentFrame], [&] {
                glm::mat4 model = glm::mat4(1.0F);
                // 先平移至中心（假设物体原本在原点）
                model = glm::translate(model, glm::vec3(0.0F, 0.0F, 0.0F));
                // 绕 X 轴旋转 20 度，绕 Y 轴旋转随时间变化
                model =
                    glm::rotate(model, glm::radians(20.0F), glm::vec3(1.0F, 0.0F, 0.0F));
                model = glm::rotate(model, time * glm::radians(60.0F),
                                    glm::vec3(0.0F, 1.0F, 0.0F));
                // 不均匀缩放
