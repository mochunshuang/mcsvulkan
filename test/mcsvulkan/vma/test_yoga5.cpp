...
bool isTriangleCounterClockwise(const glm::vec2 &p0, const glm::vec2 &p1,
                                const glm::vec2 &p2)
{
    // 有向面积 = (x2-x0)*(y1-y0) - (y2-y0)*(x1-x0)
    float area = (p2.x - p0.x) * (p1.y - p0.y) - (p2.y - p0.y) * (p1.x - p0.x);
    return area > 0.0f; // 正 => 逆时针，负 => 顺时针
}
// 校验整个网格的所有三角形是否均为逆时针绕序
bool validateMeshWinding(const std::span<const Vertex> &vertices,
                         const std::span<const uint32_t> &indices)
{
    for (size_t i = 0; i + 2 < indices.size(); i += 3)
    {
        ...
        glm::vec2 p0{vertices[i0].pos.x, vertices[i0].pos.y};
        glm::vec2 p1{vertices[i1].pos.x, vertices[i1].pos.y};
        glm::vec2 p2{vertices[i2].pos.x, vertices[i2].pos.y};
        if (!isTriangleCounterClockwise(p0, p1, p2))
        {
            std::println(stderr, "ERROR: Triangle {},{},{} is clockwise (not CCW).", i0,
                         i1, i2);
            return false;
        }
    }
...
try
{
    ...
    //diff: [test_yoga5] 自 [test_yoga2] 而来，重点是坐标系，MVP，UV 的方向和基础摸底
...
*/
                // diff: [test_yoga5]  整体-Y解决方案 引入新BUG
                ...
                // diff: [test_yoga5]  顶点剔除的 解决方案
                // // // 第一个三角形：lb(0) → rt(2) → rb(1)  （逆时针）
                // indices.push_back(baseVertex + 0);
                // indices.push_back(baseVertex + 2);
                // indices.push_back(baseVertex + 1);
                // // 第二个三角形：lb(0) → lt(3) → rt(2)  （逆时针）
                // indices.push_back(baseVertex + 0);
                ...
                //NOTE: 不通过
                // assert(validateMeshWinding({vertices.end() - 4, 4},
                //                            {indices.end() - 6, 6}) &&
                //        "in generate_line_meshes");
    ...
    constexpr auto test_layout_ndc = false;
    auto generateVerticesFromLayout = [](Layout &layout, layout_size size,
        ...
        if constexpr (test_layout_ndc)
        {
            ...
            auto appendRect = [&](float x, float y, float w, float h,
                ...
                // 四个顶点（位置按需，这里只填 pos，其他属性调用后自己填）
                outVerts.push_back({{x, y + h, 0},
                                    .color = {0, 1, 0},
                                    .texCoord = {0, 0},
                                    .textureIndex = 0,
                                    .samplerIndex = 0,
                ...
                // 索引（顺时针：左下-右下-右上, 左下-右上-左上）
                outIndices.insert(outIndices.end(), {base + 0, base + 1, base + 2,
                                                     base + 0, base + 2, base + 3});

                {
                    glm::vec2 lb = {outVerts[base + 0].pos.x, outVerts[base + 0].pos.y};
                    glm::vec2 rb = {outVerts[base + 1].pos.x, outVerts[base + 1].pos.y};
                    glm::vec2 rt = {outVerts[base + 2].pos.x, outVerts[base + 2].pos.y};
                    glm::vec2 lt = {outVerts[base + 3].pos.x, outVerts[base + 3].pos.y};
                    assert(isTriangleCounterClockwise(lb, rb, rt)); // 三角形 (0,1,2)
                    assert(isTriangleCounterClockwise(lb, rt, lt)); // 三角形 (0,2,3)
                }
            };
...
y从上到下:[x,-1]->[x,1]
*/
            const double FONT_SIZE_PX = 0.04;
            const double LINE_HEIGHT_PX = FONT_SIZE_PX * 1.5; //
            const double ORIGIN_X = -0.5;
            const double ORIGIN_Y = -0.5; // 第一行基线 Y（向下为正）
            const double MAX_LINE_WIDTH = 0.5;
            return generate_line_meshes(allocator, commandPool, GRAPHICS_AND_PRESENT,
                                        test_shape_result, break_result.types,
                                        MAX_LINE_WIDTH, FONT_SIZE_PX, ORIGIN_X, ORIGIN_Y,
                                        0.0, LINE_HEIGHT_PX);
    ...
    auto updateUniformBuffer = [&](uint32_t currentFrame) {
        ...
        // diff: [test_yoga5] BUG的来源
        // ubo.proj[1][1] *= -1; //NOTE: BVUG 的来源. 配置perspective 才需要
    ...
    const auto recordCommandBuffer = [&](const CommandBufferView &commandBuffer,
...

        {
            ...
            {
                //diff: [test_yoga5] start 通过矩阵化NDC表达合并到 MVP的中
                float w = static_cast<float>(imageExtent.width);
                float h = static_cast<float>(imageExtent.height);

                /*
该矩阵将像素坐标 (x, y) 映射到 NDC：
x_ndc = 2x/w - 1，y_ndc = 2y/h - 1。
当 y=0（屏幕顶部）→ y_ndc = -1（Vulkan 顶部），
当 y=h（屏幕底部）→ y_ndc = +1（Vulkan 底部），完全符合 Vulkan NDC 规范。
*/
                glm::mat4 ndcMatrix = glm::mat4(1.0f);
                if (test_layout_ndc)
                {
                    ndcMatrix[0][0] = 2.0f / w;
                    ndcMatrix[1][1] = 2.0f / h; // 标准映射
                    ndcMatrix[3][0] = -1.0f;
                    ndcMatrix[3][1] = -1.0f;
                }
