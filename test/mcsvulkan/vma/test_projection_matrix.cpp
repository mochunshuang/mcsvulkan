...
*/
    ...
    const auto recordCommandBuffer = [&](const CommandBufferView &commandBuffer,
        ...
        auto mvp = [&]() {
            ...
            // diff: [test_projection_matrix] start
            // 投影矩阵：透视投影
            // ============================================================
            // 透视投影测试用例（激活其中一个，其余注释掉）
            // ============================================================
            ...
            // ----------------------------- 用例 1：标准 45° 视角
            // ---------------------------- 最常用的透视投影，fov=45°，近平面
            // 0.1，远平面 10.0 效果：物体符合人眼透视，近大远小，两个物体都在可见范围内
            // --------------------------------------------------------------------
            // ubo.proj = glm::perspective(glm::radians(45.0f), aspect, 0.1f, 10.0f);

            // ----------------------------- 用例 2：广角镜头（大 fov）
            // ------------------------ fov=90°，视野变宽，物体显得更小，边缘拉伸明显
            // 适合模拟鱼眼或广角相机效果
            // --------------------------------------------------------------------
            ...
            // ----------------------------- 用例 3：长焦镜头（小 fov）
            // ------------------------
            // fov=30°，视野变窄，物体显得更大，透视感减弱（接近正交）
            // 适合模拟望远镜或远距离观察
            // --------------------------------------------------------------------
            // ubo.proj = glm::perspective(glm::radians(30.0f), aspect, 0.1f, 10.0f);

            // ----------------------------- 用例 4：极端广角（fov=120°）
            // ----------------------
            // fov=120°，视野极大，边缘畸变严重，可能看到近平面边缘的物体
            // 注意：过大的 fov 可能导致物体变形严重
            // --------------------------------------------------------------------
            // ubo.proj = glm::perspective(glm::radians(120.0f), aspect, 0.1f, 10.0f);

            // ----------------------------- 用例 5：近平面拉远
            // --------------------------------- 将近平面从 0.1 改为 1.0，原本在 0.1~1.0
            // 之间的部分被裁剪 效果：如果物体距离相机小于 1.0，将消失
            // --------------------------------------------------------------------
            // ubo.proj = glm::perspective(glm::radians(45.0f), aspect, 1.0f, 10.0f);

            // ----------------------------- 用例 6：远平面拉近
            // --------------------------------- 将远平面从 10.0 改为 5.0，距离超过 5.0
            // 的物体被裁剪 效果：第二个物体（z=0.5 位置）可能仍在范围内，但远处的背景消失
            // --------------------------------------------------------------------
            // ubo.proj = glm::perspective(glm::radians(45.0f), aspect, 0.1f, 5.0f);

            // ----------------------------- 用例 7：极端远平面
            // --------------------------------- 远平面设为
            // 100.0，几乎不裁剪远处物体，但深度精度可能下降
            // --------------------------------------------------------------------
            // ubo.proj = glm::perspective(glm::radians(45.0f), aspect, 0.1f, 100.0f);

            // ----------------------------- 用例 8：反转深度范围（Vulkan 需要）
            // ---------------- Vulkan 的深度范围是 [0,1]，glm::perspective 默认生成
            // OpenGL 风格的 [-1,1] 深度 但 glm 有 GLM_FORCE_DEPTH_ZERO_TO_ONE
            // 宏控制。您的代码中已有此宏， 所以不需要额外反转。但如果不确定，可以强制反转
            // Y 轴，已经做了： ubo.proj[1][1] *= -1; 这个翻转已经统一处理，下面不再重复
            // --------------------------------------------------------------------

            // ----------------------------- 用例 9：自定义非对称平截头体
            // ---------------------- 使用 glm::frustum
            // 手动定义左右下上近远六个平面，创造非对称透视 例如：左=-0.5, 右=1.5,
            // 下=-1.0, 上=1.0，会使图像偏向一侧
            // --------------------------------------------------------------------
            // ubo.proj = glm::frustum(-0.5f, 1.5f, -1.0f, 1.0f, 0.1f, 10.0f);

            // ----------------------------- 用例 10：改变宽高比（模拟窗口拉伸）
            // ---------------- 手动指定宽高比为 1.0（正方形），即使窗口是 800x600
            // 效果：物体看起来被水平压缩或拉伸，因为投影映射到非正方形视口
            // --------------------------------------------------------------------
            // ubo.proj = glm::perspective(glm::radians(45.0f), 1.0f, 0.1f, 10.0f);

            // ----------------------------- 用例 11：无限远平面投影
            // ---------------------------- 使用
            // glm::infinitePerspective，远平面为无穷远，避免远处裁剪
            // 适合星空等场景，但深度精度有限
            // --------------------------------------------------------------------
            ubo.proj = glm::infinitePerspective(glm::radians(45.0f), aspect, 0.1f);

            // diff: [test_projection_matrix] end
