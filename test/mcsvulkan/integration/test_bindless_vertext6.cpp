...
try
{
    ...
    const std::vector<Vertex> vertices_pixel = {
...
*/
    const std::vector<uint32_t> indices_pixel = {
        0, 1, 2, // 左上 → 右上 → 右下 （面积正，顺时针）
        0, 2, 3  // 左上 → 右下 → 左下 （面积正，顺时针）
    };

    mesh_base input_mesh_pixel{
        allocator, commandPool, GRAPHICS_AND_PRESENT, {vertices_pixel, indices_pixel}};

    auto &indexs = input_mesh_pixel.queue_data.indices;
    ...
    // 索引数据
    const std::vector<uint32_t> indices_ndc = {0, 1, 2, 0, 2, 3};
    mesh_base input_mesh_ndc{
        allocator, commandPool, GRAPHICS_AND_PRESENT, {vertices_ndc, indices_ndc}};
    auto &indexs3D = input_mesh_ndc.queue_data.indices;
    ...
    const auto recordCommandBuffer = [&](const CommandBufferView &commandBuffer,
        ...
        {
            //diff: [test_bindless_vertext6] start
            ...
            PushConstants pushConstants = {
                .vertexBufferAddress =
                    input_mesh_ndc.getVertexBufferAddress(currentFrame),
                .mvp = mvp};
            ...
            commandBuffer.bindIndexBuffer(
                input_mesh_ndc.frameBuffers[currentFrame].indexBuffer.buffer(), 0,
                mesh_base::indexType());
        ...
        {
            //diff: [test_bindless_vertext6] start
            auto enableRotation = false;
            glm::mat4 mvp =
                enableRotation ? rotation * ndcMatrix : ndcMatrix; // 先 NDC 补偿，再旋转
            //diff: [test_bindless_vertext6] end

            // diff: [new] 即使使用设备地址，Vulkan仍需要绑定索引缓冲区.来变量顶点数组
            PushConstants pushConstants = {
                .vertexBufferAddress =
                    input_mesh_pixel.getVertexBufferAddress(currentFrame),
                .mvp = mvp};
            ...
            commandBuffer.bindIndexBuffer(
                input_mesh_pixel.frameBuffers[currentFrame].indexBuffer.buffer(), 0,
                mesh_base::indexType());
    ...
    const auto drawFrame = [&]() constexpr {
        ...
        input_mesh_pixel.requestUpdate(allocator, currentFrame);
        // diff: end
    ...
    while (window.shouldClose() == 0)
    {
        ...
        if (now - lastUpdate > std::chrono::seconds(2))
        {
            ...
            if (isTriangle)
            {
                ...
                input_mesh_pixel.queueUpdate(VERTICES, INDICES);
            }
            else
            {
                // 只记录要更新的数据，不立即执行
                input_mesh_pixel.queueUpdate(vertices_pixel, indices_pixel);
            }
