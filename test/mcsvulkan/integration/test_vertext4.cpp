...
*/
    ...
    auto vertexStaging = create_staging_buffer(device, maxVertexSize);
    auto indexStaging = create_staging_buffer(device, maxIndexSize);
    auto indirectStaging =
        create_staging_buffer(device, sizeof(VkDrawIndexedIndirectCommand));

    // 状态控制
    bool isQuad = true;              // 当前形状
    int geometryUpdateCount = 0;     // 还需更新几个飞行帧（0 表示无更新）
    ...
    const auto recordCommandBuffer = [&](const CommandBufferView &commandBuffer,
        ...
        if (geometryUpdateCount > 0)
        {
            ...
            commandBuffer.copyBuffer(
                vertexStaging.buffer(), vertexBuffer.buffer(),
                std::array<VkBufferCopy, 1>{VkBufferCopy{0, 0, vsize}});
            commandBuffer.copyBuffer(
                indexStaging.buffer(), indexBuffer.buffer(),
                std::array<VkBufferCopy, 1>{VkBufferCopy{0, 0, isize}});
            commandBuffer.copyBuffer(indirectStaging.buffer(), indirectBuffer.buffer(),
                                     std::array<VkBufferCopy, 1>{VkBufferCopy{
                                         0, 0, sizeof(VkDrawIndexedIndirectCommand)}});

            const std::array<VkBufferMemoryBarrier2, 3> barriers{
    ...
    const auto drawFrame = [&]() {
        ...
        //diff: [test_vertext4] start
        // ---------- 新增：几何更新 ----------
        // 仅当请求更新且上一轮拷贝全部完成（count == 0）时，才写入 staging
        if (needGeometryUpdate && geometryUpdateCount == 0)
        {
            ...
            // 写入 staging（此时当前帧 fence 已信号，GPU 不再使用 staging）
            memcpy(vertexStaging.mapPtr(), newVerts.data(),
                   newVerts.size() * sizeof(Vertex));
            memcpy(indexStaging.mapPtr(), newIdxs.data(),
                   newIdxs.size() * sizeof(uint32_t));

            // 更新间接命令（只需改 indexCount，其余不变）
            indirectCmds[0].indexCount = static_cast<uint32_t>(newIdxs.size());
            memcpy(indirectStaging.mapPtr(), indirectCmds.data(),
                   sizeof(VkDrawIndexedIndirectCommand));

            // 标记接下来的 MAX_FRAMES_IN_FLIGHT 个帧需要执行拷贝
