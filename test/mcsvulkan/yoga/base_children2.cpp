...
struct Vertex
{
    glm::vec3 pos; // 改为 vec3，默认 z = 0.0f
    glm::vec3 color;
};
static_assert(alignof(Vertex) == 4, "check alignof error");
static_assert(sizeof(Vertex) == 24, "check sizeof error"); // 3*4 + 3*4 = 24
...
void base()
{
    ...
    Node screen{
        "screen", Config{YGConfigNew()}.setUseWebDefaults(false),
        Style{.width = {Pixels{800}}, .height = {Pixels{600}}},
        Node{
            "root",
            Style{
                .padding = {10_px},
    ...
    Layout newLayout{newRoot};
    newLayout.calculate({500, 400});
    newLayout.print();

    std::println("\n=== Original tree after move ===");
