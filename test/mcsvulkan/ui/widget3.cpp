...
struct Entity
{
    using release_type = std::function_ref<void(uint32_t) noexcept>;
...
struct TraitBase
{
    ...
    template <typename SoaCtx>
    static auto release(SoaCtx &soaCtx) -> std::function_ref<void(uint32_t) noexcept>
    {
        constexpr auto members = std::remove_cvref_t<decltype(soaCtx)>::members;
        constexpr auto I = type_id(soaCtx);
        return [&](uint32_t entity_id) noexcept {
                                                      ...
                                                      bool isRow, float &outTotalFlex)
{
    ...
    for (auto &child : node.children)
    {
        ...
        if (size_t(child.ref.type_id) == type_id)
        {
...
// 全局文本测量函数指针，测试前设置
using MeasureFunc = MeasuredSize (*)(const Entity &, Constraints);
...
struct TextStyleTrait : TraitBase<"texts">
{
                                 ...
                                 Constraints borderBC, const EdgeInsets &pad)
    {
        ...
        auto [width, height] =
            selfSoa.template view_entity<"width", "height">(0, ref.entity_id);
