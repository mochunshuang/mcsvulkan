...
{
    const auto m =
        do_get_member<"margin">(soaCtx, child.ref).template cast_to<EdgeInsets>();
...
struct ContainerStyleTrait
{
                       ...
                       const EdgeInsets &pad)
    {
        ...
        childBC =
            childBC.deflate(do_get_member<"margin">(soaCtx, child.ref)
                                .template cast_to<EdgeInsets>()); // 扣除子节点 margin
                                                      ...
                                                      bool isRow, float &outTotalFlex)
{
    ...
    for (auto &child : node.children)
    {
        ...
        if (size_t(child.ref.kind) == type_id)
        {
            ...
            const auto m =
                do_get_member<"margin">(soaCtx, real.ref).template cast_to<EdgeInsets>();
...
constexpr auto initSoaData()
{
    ...
    constexpr auto layout = [](auto &soaCtx, Node &node, Constraints constraints) {
        ...
        const auto &padding =
            do_get_member<"padding">(soaCtx, ref).template cast_to<EdgeInsets>();
...
bool tryLayout(Node &node, Constraints c, const std::string &expectedError = "")
{
    ...
    {
        const auto m =
            do_get_member<"margin">(soaCtx, node.ref).template cast_to<EdgeInsets>();
        c = c.deflate(m); // 根节点 margin 由父级模拟扣除
...
static std::vector<ScreenGeometry> extractScreenGeometries(const Node &root)
{
    ...
    auto dfs = [&](this auto &self, const Node &node, float parentX, float parentY) {
        ...
        {
            const auto pad =
                do_get_member<"padding">(soaCtx, node.ref).template cast_to<EdgeInsets>();
            const auto border =
                do_get_member<"border">(soaCtx, node.ref).template cast_to<EdgeInsets>();
...
bool runTests()
{
    return test_specific_layout_equivalence();
}
...
int main()
{
    assert([]() constexpr {
        ...
        auto index = soa.containers.new_entity(width, height, margin, padding, border,
                                               alignment, constraints);
        auto ref = get_member_impl<"width">(
            soa, {WidgetKind{decltype(soa)::find_name("containers")}, index});

        ref = soa.get_member.template operator()<"width">(
            soa, {WidgetKind{decltype(soa)::find_name("containers")}, index});

        auto do_get_member = [&]<static_string name>(const WidgetRef &ref) -> auto {
            return soa.get_member.template operator()<name>(soa, ref);
        };
        ref = do_get_member.template operator()<"width">(
            {WidgetKind{decltype(soa)::find_name("containers")}, index});

        auto &ref_width = any_cast<std::optional<float>>(ref);
        auto &ref_width2 = ref.cast_to<std::optional<float>>();

        auto ret = ref.value_or(std::optional<float>{1.0});

        return ret && (*ret == 1.0) && not ref_width.has_value() &&
               not ref_width2.has_value();
    }());
