...
static float childMainAxisLength(const FlatNode &child, bool isRow)
{
    return isRow ? child.geometry.w : child.geometry.h;
}
static float childCrossAxisLength(const FlatNode &child, bool isRow)
{
    return isRow ? child.geometry.h : child.geometry.w;
}

static void setChildMainAxisPosition(FlatNode &child, AxisDirection dir, float pos,
                                     float childMainLen, float parentMainSize,
                                     float padStart, float padEnd)
{
    float physical = isAxisForward(dir) ? padStart + pos
...
static void setChildCrossAxisPosition(FlatNode &child, AxisDirection dir, float pos,
...
static Size childFullSize(auto &soaCtx, const FlatNode &child)
{
    EdgeInsets m = do_get_member<"margin">(soaCtx, child.ref).miss_return(EdgeInsets{});
...
static void positionChildByAlignment(auto &soaCtx, FlatNode &child,
...
// ====================== Container Style Trait ======================
struct ContainerStyleTrait : TraitBase<"containers">
{
    static constexpr bool has_single_child = true; // 仅允许一个孩子
...
static std::vector<FlexChildInfo> collectFlexChildren(auto &soaCtx, FlatLayoutTree &tree,
                                                      ...
                                                      float &outTotalFlex)
{
    ...
    for (int c = tree.nodes[parentIdx].firstChild; c != -1; c = tree.nodes[c].nextSibling)
    {
...
static std::vector<float> layoutChildrenNaturally(auto &soaCtx, FlatLayoutTree &tree,
...
static void distributeFlexSpace(auto &soaCtx, FlatLayoutTree &tree,
                                const std::vector<FlexChildInfo> &infos,
...
static void positionChildrenInFlex(
    ...
    float padCrossStart, float padCrossEnd, CrossAxisAlignment crossAlign, float mainGap,
    float mainStartOff, float parentMainSize, float parentCrossSize)
{
...
template <bool isRow>
void layoutFlexImpl(auto &soaCtx, FlatLayoutTree &tree, int nodeIdx, Constraints borderBC,
                    const EdgeInsets &pad, AxisDirection dir,
                    ...
                    std::optional<float> h)
{
    ...
    if (!isBoundedMain)
    {
        for (int c = node.firstChild; c != -1; c = tree.nodes[c].nextSibling)
            if (tree.nodes[c].ref.type_id ==
                (uint32_t)std::remove_cvref_t<decltype(soaCtx)>::find_name("expandeds"))
                throw std::logic_error("Row/Column '" + node.name +
    ...
    auto naturalMain = layoutChildrenNaturally(soaCtx, tree, infos, isRow, isBoundedMain,
    ...
    if (!infos.empty())
    {
        ...
        node.baseline =
            isRow ? first.baseline + first.geometry.y : first.baseline + first.geometry.x;
    }
    else
    {
...
// ====================== Text Style Trait（无 measure，统一调用 soaCtx.measure） ======================
using MeasureFunc = MeasuredSize (*)(uint32_t type_id, const Entity &, Constraints);
...
struct TextStyleTrait : TraitBase<"texts">
{
                       ...
                       Constraints borderBC, const EdgeInsets & /*pad*/)
    {
        ...
        MeasuredSize measured =
            soaCtx.measure(soaCtx, node.ref.type_id, node.ref, borderBC);
        float w = width.value_or(measured.size.width);
        float h = height.value_or(measured.size.height);
        Size size = borderBC.clamp({w, h});
        node.geometry = {0, 0, size.width, size.height};
        node.baseline = measured.baseline > 0 ? measured.baseline : size.height;
    }
};
...
// ====================== SOA 上下文初始化（新的 measure 设计） ======================
constexpr auto initSoaData(MeasureFunc user_measure = nullptr)
{
    ...
    // 内置默认测量：通过编译期反射访问实体的 width/height 字段
    constexpr auto default_measure = [](auto &soaCtx, uint32_t type_id, const Entity &ref,
                                        Constraints c) -> MeasuredSize {
        constexpr auto members = std::remove_cvref_t<decltype(soaCtx)>::members;
        MeasuredSize result = {{80.0f, 40.0f}, 32.0f}; // 绝对默认值
        bool handled = false;
        template for (constexpr auto I : std::ranges::views::indices(members.size()))
        {
            if (I == size_t(type_id))
            {
                ...
                if constexpr (requires { typename MemberType::trait_type; })
                {
                    auto &self = soaCtx.[:members[I]:];
                    // 尝试读取 width/height 字段（如果存在）
                    if constexpr (MemberType::has_member("width") &&
                                  MemberType::has_member("height"))
                    {
                        auto [wOpt, hOpt] = self.template view_entity<"width", "height">(
                            0, ref.entity_id);
                        float w = wOpt.value_or(80.0f);
                        float h = hOpt.value_or(40.0f);
                        result = {{w, h}, h * 0.8f};
                        handled = true;
                    }
                }
    ...
    // 构造最终的 measure 函数：优先使用用户提供的，否则用内置默认
    auto final_measure = [default_measure, user_measure](auto &soaCtx, uint32_t type_id,
                                                         const Entity &ref,
                                                         Constraints bc) -> MeasuredSize {
        if (user_measure)
            return user_measure(type_id, ref, bc);
        else
            return default_measure(soaCtx, type_id, ref, bc);
    };
    ...
    constexpr auto layout = [](auto &soaCtx, FlatLayoutTree &tree, int nodeIdx,
        ...
        // 叶子节点测量阶段：使用 soaCtx.measure
        if (node.firstChild == -1 &&
            (!do_get_member<"width">(soaCtx, ref).value_or(std::optional<float>{}) ||
             !do_get_member<"height">(soaCtx, ref).value_or(std::optional<float>{})))
        {
            MeasuredSize natural = soaCtx.measure(soaCtx, ref.type_id, ref, borderBC);
            if (!do_get_member<"width">(soaCtx, ref).value_or(std::optional<float>{}))
                borderBC = borderBC.applyFixedWidth(natural.size.width);
            if (!do_get_member<"height">(soaCtx, ref).value_or(std::optional<float>{}))
    ...
    return make_aggregate<
        "soaData", ContainerStyleObject::trait_type::name,
        RowStyleObject::trait_type::name, ColumnStyleObject::trait_type::name,
        ExpandedStyleObject::trait_type::name, TextStyleObject::trait_type::name,
        "measure", "get_member", "layout">(std::move(containers), std::move(rows),
                                           std::move(columns), std::move(expandeds),
                                           std::move(texts), std::move(final_measure),
                                           std::move(get_member), std::move(layout));
}
...
{
    auto soaCtx = initSoaData(); // 使用内置默认测量
    FlatLayoutTree tree;
    tree.reserve(10);
    int row = tree.addNode("row",
                           RowStyleTrait::make(soaCtx, 300, 100, {}, {},
                           ...
                           -1);
    int t1 = tree.addNode("t1", TextStyleTrait::make(soaCtx, std::nullopt, 40),
                          row); // height=40 -> baseline=32
    int t2 = tree.addNode("t2", TextStyleTrait::make(soaCtx, std::nullopt, 20),
                          row); // height=20 -> baseline=16
    if (!tryLayout(soaCtx, tree, row, {0, 300, 0, 100}))
        return false;
...
void buildApp(auto &soaCtx, FlatLayoutTree &tree, const AppState &state)
{
    ...
    // 使用一个 Column 包裹 header 和 count_label，作为 root 的唯一子节点
    int mainColumn =
...
bool test_resource_cleanup()
{
    // 第一个块：创建并销毁一系列实体
    {
        auto soaCtx = initSoaData();
        {
            ...
            // 将 c1 和 t1 放入一个 Column 中，Column 作为 Container 的唯一子节点
...
bool test_dsl_nested_expanded()
{
    ...
    bool ok = true;
    // 注意：这些索引依赖于树的构造顺序，实际需要根据具体构建顺序调整。
    // 这里我们使用节点名称查找，而不是硬编码索引，以避免脆弱性。
...
bool test_container_single_child_constraint()
{
    ...
    tree.reserve(10);
    int root = dsl_container(soaCtx, tree, -1, "root", 200, 200);
    dsl_text(soaCtx, tree, root, "first_child", 30, 30); // 第一个孩子，OK
