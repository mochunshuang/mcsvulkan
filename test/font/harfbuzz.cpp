                                     ...
                                     FT_Library ft_lib)
{
    ...
    for (const auto &run : shaping_runs)
    {
        ...
        else
        {
            ...
            hb_face_t *hb_face =
                hb_ft_face_create(ft_face, nullptr); // 由 hb_face 管理 FT_Face
            hb_font = hb_font_create(hb_face);
            hb_face_destroy(hb_face); // hb_font 已持有引用
        ...
        for (unsigned int i = 0; i < glyph_count; ++i)
        {
            ...
            // cluster 是相对于子数组的偏移，转换为全局索引
            g.cluster = start + glyph_infos[i].cluster;
            // HarfBuzz 使用 26.6 定点数（1/64 像素），转换为 double 方便后续计算
