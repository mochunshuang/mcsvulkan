...
// 辅助函数：在画布上绘制一个像素
void draw_pixel(FT_Bitmap &canvas, int x, int y, uint8_t color)
{
    if (x >= 0 && x < static_cast<int>(canvas.width) && y >= 0 &&
        y < static_cast<int>(canvas.rows))
    {
        canvas.buffer[y * canvas.pitch + x] = color;
    }
}

// Bresenham 直线算法
void draw_line(FT_Bitmap &canvas, int x0, int y0, int x1, int y1, uint8_t color)
{
...
// 在画布上绘制文本（左上角坐标 (x, y)），使用指定像素大小
void draw_text(FT_Library lib, FT_Face face, const std::string &text, int x, int y,
               FT_Bitmap &canvas, int pixel_size)
{
    // 临时保存当前像素大小，但为了不影响外部，我们使用独立的 face 实例
    // 由于 face 会改变状态，我们在这里重新设置
    FT_Set_Pixel_Sizes(face, 0, pixel_size);
    ...
    for (char ch : text)
    {
        ...
        int dst_y = pen_y - slot->bitmap_top; // 画布 Y 向下，所以减去

        // 将字形位图复制到画布
        for (unsigned int row = 0; row < bmp.rows; ++row)
        {
            const uint8_t *src = bmp.buffer + row * bmp.pitch;
            uint8_t *dst_line = canvas.buffer + (dst_y + row) * canvas.pitch + dst_x;
            for (unsigned int col = 0; col < bmp.width; ++col)
            {
                if (dst_x + col >= 0 && dst_x + col < static_cast<int>(canvas.width) &&
                    dst_y + static_cast<int>(row) >= 0 &&
                    dst_y + static_cast<int>(row) < static_cast<int>(canvas.rows))
                {
                    // 字形像素为 0（黑）时画黑色，否则保持原样（白色背景）
                    if (src[col] > 128) // 灰度阈值，避免模糊边缘太淡
                        dst_line[col] = 255;
                    else
                        dst_line[col] = 0; // 黑色
                }
...
void test_chapter_g_glyph_metrics(FT_Library lib, const std::string &filename)
{
    ...
    const int target_min = static_cast<int>(std::min(canvas_w, canvas_h) * 0.75); // 450
    ...
    auto compute_origin = [&](int left, int top, int w, int h, int &ox, int &oy,
                              bool vertical) -> bool {
        // 字形在画布上的坐标范围（相对于原点）
        // 水平布局：左上角 (ox + left, oy - top)
        // 垂直布局：左上角 (ox + left, oy + top)
        // 要求：
        //   0 <= ox + left <= canvas_w - w
        //   若 vertical: 0 <= oy + top <= canvas_h - h
        //   若 !vertical: 0 <= oy - top <= canvas_h - h  且 0 <= oy <= canvas_h
        // 同时原点本身必须在画布内：0 <= ox < canvas_w, 0 <= oy < canvas_h

        int ox_min, ox_max;
        ox_min = std::max(0, -left);
        ox_max = std::min(canvas_w - 1, canvas_w - left - w);
        if (ox_min > ox_max)
            return false;
        ...
        {
            // 水平布局：oy - top >= 0  => oy >= top
            //          oy - top + h <= canvas_h => oy <= canvas_h - h + top
    ...
    {
        std::println(
            "警告：水平字形太大，无法同时显示完整字形和原点，将使用固定原点 (200,400)");
        origin_x_h = 200;
        origin_y_h = 400;
        // 简单检查是否可行
    }
    ...
    {
        std::println(
            "警告：垂直字形太大，无法同时显示完整字形和原点，将使用固定原点 (200,400)");
        origin_x_v = 200;
        origin_y_v = 400;
    }
    ...
    // ========== 创建画布 ==========
    std::vector<uint8_t> canvas_h_data(canvas_w * canvas_h, 255);
    FT_Bitmap canvas_h_bmp = {(unsigned int)canvas_h,
                              ...
                              0,
                              nullptr};
    std::vector<uint8_t> canvas_v_data(canvas_w * canvas_h, 255);
    FT_Bitmap canvas_v_bmp = canvas_h_bmp;
    canvas_v_bmp.buffer = canvas_v_data.data();

    // 辅助函数：画箭头（从原点沿轴正方向画一小段，末端带箭头）
    auto draw_axis_arrow = [&](FT_Bitmap &bmp, int ox, int oy, int dir_x, int dir_y,
                               uint8_t color) {
        ...
        for (int i = 1; i <= arrow_len; ++i)
        {
            ...
            if (x >= 0 && x < canvas_w && y >= 0 && y < canvas_h)
                bmp.buffer[y * canvas_w + x] = color;
        }
        // 画箭头头部（两条斜线）
        int tip_x = ox + arrow_len * dir_x;
        int tip_y = oy + arrow_len * dir_y;
        if (dir_x != 0)
        {
            // 水平箭头
            draw_line(bmp, tip_x, tip_y, tip_x - dir_x * 3, tip_y - 2, color);
            draw_line(bmp, tip_x, tip_y, tip_x - dir_x * 3, tip_y + 2, color);
        }
        else if (dir_y != 0)
        {
            // 垂直箭头
            draw_line(bmp, tip_x, tip_y, tip_x - 2, tip_y - dir_y * 3, color);
            draw_line(bmp, tip_x, tip_y, tip_x + 2, tip_y - dir_y * 3, color);
        }
    };
    ...
    // 绘制水平示意图
    {
        ...
        for (unsigned int row = 0; row < bmp_h.rows; ++row)
        {
            ...
            uint8_t *dst = canvas_h_bmp.buffer + canvas_row * canvas_w + dst_x;
        ...
        if (origin_y_h >= 0 && origin_y_h < canvas_h)
            for (int x = 0; x < canvas_w; ++x)
                canvas_h_bmp.buffer[origin_y_h * canvas_w + x] = axis;
        // Y轴
        if (origin_x_h >= 0 && origin_x_h < canvas_w)
            for (int y = 0; y < canvas_h; ++y)
                canvas_h_bmp.buffer[y * canvas_w + origin_x_h] = axis;
        // 原点标记（小圆点）
        for (int dy = -2; dy <= 2; ++dy)
            for (int dx = -2; dx <= 2; ++dx)
                if (dx * dx + dy * dy <= 4)
                {
                    ...
                    if (x >= 0 && x < canvas_w && y >= 0 && y < canvas_h)
                        canvas_h_bmp.buffer[y * canvas_w + x] = axis;
                }

        // 在原点处绘制方向箭头
        draw_axis_arrow(canvas_h_bmp, origin_x_h, origin_y_h, 1, 0,
                        axis); // X正方向（右）
        draw_axis_arrow(canvas_h_bmp, origin_x_h, origin_y_h, 0, -1,
                        axis); // Y正方向（上）
    }
    ...
    // 绘制垂直示意图
    {
        ...
        int dst_y = origin_y_v + top_v_manual; // 垂直Y向下，所以加上
        ...
        for (unsigned int row = 0; row < bmp_v.rows; ++row)
        {
            ...
            uint8_t *dst = canvas_v_bmp.buffer + canvas_row * canvas_w + dst_x;
        ...
        if (origin_y_v >= 0 && origin_y_v < canvas_h)
            for (int x = 0; x < canvas_w; ++x)
                canvas_v_bmp.buffer[origin_y_v * canvas_w + x] = axis;
        // Y轴
        if (origin_x_v >= 0 && origin_x_v < canvas_w)
            for (int y = 0; y < canvas_h; ++y)
                canvas_v_bmp.buffer[y * canvas_w + origin_x_v] = axis;
        // 原点标记
        for (int dy = -2; dy <= 2; ++dy)
            for (int dx = -2; dx <= 2; ++dx)
                if (dx * dx + dy * dy <= 4)
                {
                    ...
                    if (x >= 0 && x < canvas_w && y >= 0 && y < canvas_h)
                        canvas_v_bmp.buffer[y * canvas_w + x] = axis;
                }

        // 在原点处绘制方向箭头
        draw_axis_arrow(canvas_v_bmp, origin_x_v, origin_y_v, 1, 0,
                        axis); // X正方向（右）
        draw_axis_arrow(canvas_v_bmp, origin_x_v, origin_y_v, 0, 1,
                        axis); // Y正方向（下）
    }
