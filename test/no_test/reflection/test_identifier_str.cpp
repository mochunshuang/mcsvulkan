...
//  error: uncaught exception of type 'std::meta::exception'; 'what()': 'reflection with has_identifier false'
// static_assert(identifier_of(^^int) == "int");
// static_assert(identifier_of(^^double) == "double"); //NOTE: 编译期错误
...
using T = int;
using T0 = T;
static_assert(display_string_of(^^T) != "int");
//NOTE: 别名不简单的。 运行时打印看看就知道了
static_assert(display_string_of(^^T) == "T {aka int}");
static_assert(display_string_of(^^T0) == "T0 {aka int}");
static_assert(display_string_of(dealias(^^T)) == "int");
static_assert(display_string_of(dealias(^^T0)) == "int");
...
try
{
    //  display_string_of(^^T): T {aka int}
    std::println(" display_string_of(^^T): {}", display_string_of(^^T));
    std::println(" display_string_of(^^T0): {}", display_string_of(^^T0));
    std::println(" display_string_of(dealias(^^T)): {}", display_string_of(dealias(^^T)));
    std::println(" display_string_of(dealias(^^T0)): {}",
                 display_string_of(dealias(^^T0)));
    {
        int v{};
        auto lam = [=]() {
            return v;
