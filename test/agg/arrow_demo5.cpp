...
int main()
{
    ...
    double origin_y = centerY - bbox_center_y; // 原来是加号，改为减号

    // ★★★ 正确计算屏幕边界（只计算一次）★★★
    double screen_left = origin_x + xMin;
    double screen_right = origin_x + xMax;
    double screen_top = origin_y + yMin;    // 正确：上边界 y 较小
    double screen_bottom = origin_y + yMax; // 正确：下边界 y 较大
    ...
    if (screen_top < 20)
        origin_y = 20 - yMin; // 由 screen_top = origin_y + yMin < 20 推出
    if (screen_bottom > HEIGHT - 20)
        origin_y =
            HEIGHT - 20 - yMax; // 由 screen_bottom = origin_y + yMax > HEIGHT-20 推出

    // 重新计算屏幕边界（基于调整后的原点）
    screen_left = origin_x + xMin;
    screen_right = origin_x + xMax;
    screen_top = origin_y + yMin; // 保持正确公式
    ...
    // 输出 PPM
    std::ofstream out("arrow_demo5.ppm", std::ios::binary);
    out << "P6\n" << WIDTH << " " << HEIGHT << "\n255\n";
