...
namespace agg
{
                              ...
                              const trans_affine &mtx, PathStorage &path)
    {
        ...
        for (n = 0; n < outline.n_contours; n++)
        {
            ...
            if (flip_y)
                y1 = -y1;
            mtx.transform(&x1, &y1);
            path.move_to(value_type(dbl_to_int26p6(x1)), value_type(dbl_to_int26p6(y1)));
    ...
    void decompose_ft_bitmap_mono(const FT_Bitmap &bitmap, int x, int y, bool flip_y,
    ...
    void decompose_ft_bitmap_gray8(const FT_Bitmap &bitmap, int x, int y, bool flip_y,
                                   Rasterizer &ras, Scanline &sl,
    ...
    //------------------------------------------------------------------------
    font_engine_freetype_base::font_engine_freetype_base(bool flag32, unsigned max_faces)
        : m_flag32(flag32), m_change_stamp(0), m_last_error(0), m_name(0),
          m_name_len(256 - 16 - 1), m_face_index(0), m_char_map(FT_ENCODING_NONE),
          m_signature(new char[256 + 256 - 16]), m_height(0), m_width(0), m_hinting(true),
          m_flip_y(false), m_library_initialized(false), m_library(0),
          m_faces(new FT_Face[max_faces]), m_face_names(new char *[max_faces]),
          m_num_faces(0), m_max_faces(max_faces), m_cur_face(0), m_resolution(0),
          m_glyph_rendering(glyph_ren_native_gray8), m_glyph_index(0), m_data_size(0),
          m_data_type(glyph_data_invalid), m_bounds(1, 1, 0, 0), m_advance_x(0.0),
          m_advance_y(0.0),
    ...
    //------------------------------------------------------------------------
    bool font_engine_freetype_base::load_font(const char *font_name, unsigned face_index,
        ...
        if (m_library_initialized)
        {
            ...
            else
            {
                if (m_num_faces >= m_max_faces)
                {
                    ...
                    std::memcpy(m_faces, m_faces + 1,
                                (m_max_faces - 1) * sizeof(FT_Face));
                    std::memcpy(m_face_names, m_face_names + 1,
                ...
                {
                    m_last_error = FT_New_Memory_Face(
                        m_library, (const FT_Byte *)font_mem, font_mem_size, face_index,
                        &m_faces[m_num_faces]);
                }
                else
                {
    ...
    void font_engine_freetype_base::update_signature()
    {
        if (m_cur_face && m_name)
        {
            ...
            std::sprintf(m_signature, "%s,%u,%d,%d,%d:%dx%d,%d,%d,%08X", m_name,
                         m_char_map, m_face_index, int(m_glyph_rendering), m_resolution,
                         m_height, m_width, int(m_hinting), int(m_flip_y), gamma_hash);
                ...
                m_glyph_rendering == glyph_ren_agg_gray8)
            {
                ...
                std::sprintf(buf, ",%08X%08X%08X%08X%08X%08X", dbl_to_plain_fx(mtx[0]),
    ...
    bool font_engine_freetype_base::prepare_glyph(unsigned glyph_code)
    {
        ...
        m_last_error = FT_Load_Glyph(m_cur_face, m_glyph_index,
        ...
        if (m_last_error == 0)
        {
            ...
            case glyph_ren_agg_mono:
                if (m_last_error == 0)
                {
                    ...
                    {
                        m_path32.remove_all();
                        decompose_ft_outline(m_cur_face->glyph->outline, m_flip_y,
                                             m_affine, m_path32);
