    ...
    for (size_t i = 0; i < glyph_cnt; ++i)
        for (size_t i = 0; i < glyph_cnt; ++i)
        {
            ...
            if (FT_HAS_COLOR(current_face))
            {
                ...
                FT_Error error = FT_Palette_Data_Get(current_face, &palette_data);
                if (error == 0 && palette_data.num_palettes > 0)
                {
                    // 使用第一个调色板（索引0通常是默认调色板）
                    FT_UShort palette_index = 0;
                    // 确保索引在有效范围内
                    if (palette_index < palette_data.num_palettes)
                    {
                        FT_Color *palette = nullptr;
                        if (FT_Palette_Select(current_face, palette_index, &palette) == 0)
                        {
                            // 遍历图层
                            ...
                            while (FT_Get_Color_Glyph_Layer(
                                current_face, g.index, &layer_glyph, &layer_color_index,
                                &iterator))
                            {
                                if (FT_Load_Glyph(current_face, layer_glyph,
                                                  FT_LOAD_NO_BITMAP))
                                    continue;

                                // 直接使用8位颜色值
                                FT_Color color = palette[layer_color_index];
                                agg::rgba8 render_color(color.red, color.green,
                                                        color.blue, 255);
...
int main()
{
    ...
    for (int max_width : widths)
    {
        ...
        char filename[64];
        std::snprintf(filename, sizeof(filename), "unibreak3_%d.ppm", max_width);
