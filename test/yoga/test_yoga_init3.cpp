...
try
{
    ...
    // diff: [test_yoga_init2] start
    // ==================== Yoga 布局初始化 ====================
    // 全局或函数内持有 Yoga 节点引用
    YGNodeRef root = nullptr;
    YGNodeRef topNode = nullptr;    // 红色上条
    YGNodeRef bottomNode = nullptr; // 绿色下条
    YGNodeRef leftNode = nullptr;   // 蓝色左条
    YGNodeRef rightNode = nullptr;  // 黄色右条

    // 中心盒子及其内部子盒子（中心固定大小，内部子盒拉伸）
    YGNodeRef centerNode = nullptr;      // 灰色中心盒
    YGNodeRef innerTopNode = nullptr;    // 青色内上条
    YGNodeRef innerBottomNode = nullptr; // 品红内下条
    YGNodeRef innerLeftNode = nullptr;   // 橙色内左条
    YGNodeRef innerRightNode = nullptr;  // 紫色内右条

    // 初始化 Yoga 节点树（硬编码绝对定位，实现四周边条随窗口拉伸，中心固定）
    auto initYogaNodes = [&](uint32_t width, uint32_t height) {
        if (root)
            YGNodeFreeRecursive(root);

        root = YGNodeNew();
        // 根节点尺寸设为窗口大小（相对布局，子节点绝对定位时相对于根）
        YGNodeStyleSetWidth(root, static_cast<float>(width));
        YGNodeStyleSetHeight(root, static_cast<float>(height));
        YGNodeStyleSetPositionType(root, YGPositionTypeRelative);

        // ----- 上矩形（红色）：宽度 = 窗口宽度，高度固定 100，贴在顶部 -----
        topNode = YGNodeNew();
        YGNodeStyleSetPositionType(topNode, YGPositionTypeAbsolute);
        YGNodeStyleSetPosition(topNode, YGEdgeTop, 0);
        YGNodeStyleSetPosition(topNode, YGEdgeLeft, 0);
        YGNodeStyleSetWidth(topNode, static_cast<float>(width)); // 随窗口拉伸
        YGNodeStyleSetHeight(topNode, 100.0f);                   // 固定高度
        YGNodeInsertChild(root, topNode, 0);

        // ----- 下矩形（绿色）：宽度 = 窗口宽度，高度固定 100，贴在底部 -----
        bottomNode = YGNodeNew();
        YGNodeStyleSetPositionType(bottomNode, YGPositionTypeAbsolute);
        YGNodeStyleSetPosition(bottomNode, YGEdgeBottom, 0);
        YGNodeStyleSetPosition(bottomNode, YGEdgeLeft, 0);
        YGNodeStyleSetWidth(bottomNode, static_cast<float>(width)); // 随窗口拉伸
        YGNodeStyleSetHeight(bottomNode, 100.0f);
        YGNodeInsertChild(root, bottomNode, 1);

        // ----- 左矩形（蓝色）：高度 = 窗口高度 - 200，宽度固定 100，贴在左侧 -----
        leftNode = YGNodeNew();
        YGNodeStyleSetPositionType(leftNode, YGPositionTypeAbsolute);
        YGNodeStyleSetPosition(leftNode, YGEdgeTop, 100.0f);
        YGNodeStyleSetPosition(leftNode, YGEdgeLeft, 0);
        YGNodeStyleSetWidth(leftNode, 100.0f);
        YGNodeStyleSetHeight(leftNode, static_cast<float>(height) - 200.0f); // 随窗口拉伸
        YGNodeInsertChild(root, leftNode, 2);

        // ----- 右矩形（黄色）：高度 = 窗口高度 - 200，宽度固定 100，贴在右侧 -----
        rightNode = YGNodeNew();
        YGNodeStyleSetPositionType(rightNode, YGPositionTypeAbsolute);
        YGNodeStyleSetPosition(rightNode, YGEdgeTop, 100.0f);
        YGNodeStyleSetPosition(rightNode, YGEdgeRight, 0);
        YGNodeStyleSetWidth(rightNode, 100.0f);
        YGNodeStyleSetHeight(rightNode,
                             static_cast<float>(height) - 200.0f); // 随窗口拉伸
        YGNodeInsertChild(root, rightNode, 3);

        // ----- 中心盒子（灰色）：固定大小 200x200，绝对定位居中 -----
        const float centerWidth = 200.0f;
        const float centerHeight = 200.0f;
        centerNode = YGNodeNew();
        YGNodeStyleSetPositionType(centerNode, YGPositionTypeAbsolute);
        // 居中计算：left = (窗口宽 - 200)/2，top = (窗口高 - 200)/2
        YGNodeStyleSetPosition(centerNode, YGEdgeLeft, (width - centerWidth) / 2.0f);
        YGNodeStyleSetPosition(centerNode, YGEdgeTop, (height - centerHeight) / 2.0f);
        YGNodeStyleSetWidth(centerNode, centerWidth); // 固定大小，不随窗口拉伸
        YGNodeStyleSetHeight(centerNode, centerHeight);
        YGNodeInsertChild(root, centerNode, 4);

        // --- 内部上子盒（青色）：贴在中心盒顶部，宽度拉伸至与父容器等宽，高度占 20% ---
        innerTopNode = YGNodeNew();
        YGNodeStyleSetPositionType(innerTopNode, YGPositionTypeAbsolute);
        YGNodeStyleSetPosition(innerTopNode, YGEdgeTop, 0);
        YGNodeStyleSetPosition(innerTopNode, YGEdgeLeft, 0);
        YGNodeStyleSetPosition(innerTopNode, YGEdgeRight,
                               0);                     // 关键：拉伸至父容器左右边界
        YGNodeStyleSetHeightPercent(innerTopNode, 20); // 高度为父容器（中心盒）的 20%
        YGNodeInsertChild(centerNode, innerTopNode, 0);

        // --- 内部下子盒（品红）：贴在中心盒底部，宽度拉伸，高度占 20% ---
        innerBottomNode = YGNodeNew();
        YGNodeStyleSetPositionType(innerBottomNode, YGPositionTypeAbsolute);
        YGNodeStyleSetPosition(innerBottomNode, YGEdgeBottom, 0);
        YGNodeStyleSetPosition(innerBottomNode, YGEdgeLeft, 0);
        YGNodeStyleSetPosition(innerBottomNode, YGEdgeRight, 0); // 拉伸至父容器左右边界
        YGNodeStyleSetHeightPercent(innerBottomNode, 20);
        YGNodeInsertChild(centerNode, innerBottomNode, 1);

        // --- 内部左子盒（橙色）：贴在中心盒左侧，垂直方向留出 20% 边距，宽度占 20% ---
        innerLeftNode = YGNodeNew();
        YGNodeStyleSetPositionType(innerLeftNode, YGPositionTypeAbsolute);
        YGNodeStyleSetPositionPercent(innerLeftNode, YGEdgeTop, 20.0f);    // 顶部留白 20%
        YGNodeStyleSetPositionPercent(innerLeftNode, YGEdgeBottom, 20.0f); // 底部留白 20%
        YGNodeStyleSetPosition(innerLeftNode, YGEdgeLeft, 0);
        YGNodeStyleSetWidthPercent(innerLeftNode, 20); // 宽度为父容器（中心盒）的 20%
        YGNodeInsertChild(centerNode, innerLeftNode, 2);

        // --- 内部右子盒（紫色）：贴在中心盒右侧，垂直方向留出 20% 边距，宽度占 20% ---
        innerRightNode = YGNodeNew();
        YGNodeStyleSetPositionType(innerRightNode, YGPositionTypeAbsolute);
        YGNodeStyleSetPositionPercent(innerRightNode, YGEdgeTop, 20.0f);
        YGNodeStyleSetPositionPercent(innerRightNode, YGEdgeBottom, 20.0f);
        YGNodeStyleSetPosition(innerRightNode, YGEdgeRight, 0);
        YGNodeStyleSetWidthPercent(innerRightNode, 20);
        YGNodeInsertChild(centerNode, innerRightNode, 3);
    };

    // 窗口大小改变时，仅更新需要随窗口拉伸的属性（中心盒子的位置会重新居中，但尺寸不变）
    auto updateYogaNodesSize = [&](uint32_t newWidth, uint32_t newHeight) {
        // 根节点尺寸更新为新的窗口尺寸
        YGNodeStyleSetWidth(root, static_cast<float>(newWidth));
        YGNodeStyleSetHeight(root, static_cast<float>(newHeight));

        // 上下条宽度随窗口拉伸
        YGNodeStyleSetWidth(topNode, static_cast<float>(newWidth));
        YGNodeStyleSetWidth(bottomNode, static_cast<float>(newWidth));

        // 左右条高度随窗口拉伸（窗口高 - 200）
        YGNodeStyleSetHeight(leftNode, static_cast<float>(newHeight) - 200.0f);
        YGNodeStyleSetHeight(rightNode, static_cast<float>(newHeight) - 200.0f);

        // 中心盒子：重新计算居中位置（尺寸 200x200 不变）
        const float centerWidth = 200.0f;
        const float centerHeight = 200.0f;
        float centerX = (newWidth - centerWidth) / 2.0f;
        float centerY = (newHeight - centerHeight) / 2.0f;
        YGNodeStyleSetPosition(centerNode, YGEdgeLeft, centerX);
        YGNodeStyleSetPosition(centerNode, YGEdgeTop, centerY);
        // 显式确保尺寸不变（虽然从未改变，但此处保证语义）
        YGNodeStyleSetWidth(centerNode, centerWidth);
        YGNodeStyleSetHeight(centerNode, centerHeight);
    };

    // 从 Yoga 布局生成顶点数据（在每一帧或窗口 resize 时调用，获取最新的布局位置）
    auto generateVerticesFromYoga = [&](YGNodeRef rootNode, uint32_t screenWidth,
        ...
        // 屏幕坐标转 NDC（-1 到 1）
        auto screenToNDC = [screenWidth, screenHeight](float x, float y) -> glm::vec2 {
            return {(x / screenWidth) * 2.0f - 1.0f, (y / screenHeight) * 2.0f - 1.0f};
        };

        // 根据节点类型返回颜色（仅用于演示，实际可传入动态颜色）
        auto getColor = [&](YGNodeRef node) -> glm::vec3 {
            if (node == topNode)
                return {1.0f, 0.0f, 0.0f}; // 红
            if (node == bottomNode)
                return {0.0f, 1.0f, 0.0f}; // 绿
            if (node == leftNode)
                return {0.0f, 0.0f, 1.0f}; // 蓝
            if (node == rightNode)
                return {1.0f, 1.0f, 0.0f}; // 黄
            if (node == centerNode)
                return {0.5f, 0.5f, 0.5f}; // 灰
            if (node == innerTopNode)
                return {0.0f, 1.0f, 1.0f}; // 青
            if (node == innerBottomNode)
                return {1.0f, 0.0f, 1.0f}; // 品红
            if (node == innerLeftNode)
                return {1.0f, 0.5f, 0.0f}; // 橙
            if (node == innerRightNode)
                return {0.5f, 0.0f, 1.0f}; // 紫
            return {1.0f, 1.0f, 1.0f};
        };

        // 递归遍历节点，累积绝对坐标
        std::function<void(YGNodeRef, float, float)> traverse =
            [&](YGNodeRef node, float parentLeft, float parentTop) {
                ...
                {
                    traverse(YGNodeGetChild(node, i), left, top);
                }
            };
        traverse(rootNode, 0.0f, 0.0f);
    };
    // diff: [test_yoga_init2] end

    // 初始化 Yoga 节点（使用初始窗口尺寸）
    initYogaNodes(WIDTH, HEIGHT);
    std::vector<Vertex> yogaVertices;
    std::vector<uint32_t> yogaIndices;
    generateVerticesFromYoga(root, WIDTH, HEIGHT, yogaVertices, yogaIndices);
    ...
    const auto recreateSwapChain = [&]() {
        ...
        // 更新 Yoga 节点尺寸（关键：必须更新所有依赖宽高的属性）
        updateYogaNodesSize(newWidth, newHeight);

        // 重新生成顶点/索引
        std::vector<Vertex> newVerts;
        std::vector<uint32_t> newIndices;
        generateVerticesFromYoga(root, newWidth, newHeight, newVerts, newIndices);
