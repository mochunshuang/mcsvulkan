...
try
{
    ...
    YGNodeRef topNode = nullptr;
    YGNodeRef bottomNode = nullptr;
    YGNodeRef leftNode = nullptr;
    YGNodeRef rightNode = nullptr;
    // 初始化 Yoga 节点树（硬编码绝对定位）
    auto initYogaNodes = [&](uint32_t width, uint32_t height) {
        ...
        // 上矩形：top=0, left=0, width=width, height=100
        topNode = YGNodeNew();
        ...
        YGNodeStyleSetWidth(topNode, static_cast<float>(width));
        YGNodeStyleSetHeight(topNode, 100.0f);
        YGNodeInsertChild(root, topNode, 0);

        // 下矩形：bottom=0, left=0, width=width, height=100
        ...
        YGNodeStyleSetWidth(bottomNode, static_cast<float>(width));
        YGNodeStyleSetHeight(bottomNode, 100.0f);
        YGNodeInsertChild(root, bottomNode, 1);

        // 左矩形：top=100, left=0, width=100, height=height-200
        leftNode = YGNodeNew();
        ...
        YGNodeStyleSetHeight(leftNode, static_cast<float>(height) - 200.0f);
        YGNodeInsertChild(root, leftNode, 2);

        // 右矩形：top=100, right=0, width=100, height=height-200
        rightNode = YGNodeNew();
        ...
        YGNodeStyleSetHeight(rightNode, static_cast<float>(height) - 200.0f);
    ...
    // 从 Yoga 布局生成顶点数据（与原 buildMeshHardcoded 完全等价）
    auto generateVerticesFromYoga = [](YGNodeRef rootNode, uint32_t screenWidth,
        ...
        for (uint32_t i = 0; i < YGNodeGetChildCount(rootNode); ++i)
        {
            YGNodeRef child = YGNodeGetChild(rootNode, i);
            float left = YGNodeLayoutGetLeft(child);
            float top = YGNodeLayoutGetTop(child);
            float width = YGNodeLayoutGetWidth(child);
            float height = YGNodeLayoutGetHeight(child);
            ...
            glm::vec2 p0 = screenToNDC(left, bottom);  // 左下
            glm::vec2 p1 = screenToNDC(right, bottom); // 右下
            glm::vec2 p2 = screenToNDC(left, top);     // 左上
            glm::vec2 p3 = screenToNDC(right, top);    // 右上

            uint32_t base = static_cast<uint32_t>(outVerts.size());
            outVerts.push_back({p0, colors[i]});
            outVerts.push_back({p1, colors[i]});
            outVerts.push_back({p2, colors[i]});
            outVerts.push_back({p3, colors[i]});
