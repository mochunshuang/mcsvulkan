...
// -------------------- 编译期配置 --------------------
// 模式选择：true  = 从硬编码文本提取唯一字符（支持中文等多语言）
//           false = 生成 ASCII 32~126（适合英文测试）
constexpr bool kUnicodeMode = true; // 根据需要修改

// Unicode 模式下使用的输入文本
const std::string kInputText = "Hello, 世界! こんにちは 123";

// 根据模式选择字体路径
const char *kFontPath = kUnicodeMode ? "C:/Windows/Fonts/msyh.ttc"   // 支持中文的字体
                                     : "C:/Windows/Fonts/arial.ttf"; // 英文字体
...
class SdfAtlasPacker
{
  ...
  private:
    int m_width, m_height, m_glyphSize;
...
int main()
{
    try
    {
        // 设置locale以支持UTF-8（仅影响mbrtoc32，不影响其他）
        ...
        if constexpr (kUnicodeMode)
        {
            inputText = kInputText;
            std::cout << "Mode: Unicode (from hardcoded text)\n";
        }
        else
        {
            // ASCII 模式也使用一个默认文本用于整形演示，但字符集为全部ASCII
            inputText = "Hello, Atlas!";
            std::cout << "Mode: ASCII (32-126)\n";
        }
        ...
        {
            // 从输入文本提取唯一字符
            std::vector<char32_t> codepointsVec = utf8_to_codepoints(inputText);
            std::set<uint32_t> uniqueSet;
            for (char32_t cp : codepointsVec)
                uniqueSet.insert(static_cast<uint32_t>(cp));
            codepoints.assign(uniqueSet.begin(), uniqueSet.end());
        }
        else
        {
            // ASCII 模式：生成 32~126
        ...
        // 1. 初始化字体加载器
        FontLoader font(fontPath, DEFAULT_FONT_SIZE);
        ...
        // 3. 打包SDF图集
        SdfAtlasPacker sdfPacker(ATLAS_WIDTH, ATLAS_HEIGHT, GLYPH_SIZE);
        size_t idx = 0;
        for (uint32_t cp : codepoints)
        {
        ...
        // 4. 打包普通灰度图集
        RawAtlasPacker rawPacker(ATLAS_WIDTH, ATLAS_HEIGHT);
        ...
        auto shaped = shaper.shape(inputText);
        std::cout << "\nShaped text: " << inputText << "\n";
