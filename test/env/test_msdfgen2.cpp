...
constexpr int ASCII_START = 32;               // 起始 ASCII 码（空格）
constexpr int ASCII_END = 126;                // 结束 ASCII 码（波浪号）
...
std::string safe_filename(char32_t codepoint)
{
    ...
    if (it != specialMap.end())
        return it->second;
    // 其他情况用码点
    return "char_" + std::to_string(static_cast<int>(codepoint));
}
...
class FontLoader
{
  public:
    ...
    Glyph getGlyphMetrics(uint32_t codepoint) const
    {
        ...
        // 将 FreeType 的 26.6 定点数转换为浮点数（像素）
...
int main()
{
    try
    {
        ...
        std::vector<msdfgen::Bitmap<float, 1>> sdfBitmaps; // SDF位图
        std::vector<std::vector<uint8_t>> rawBitmaps;      // 普通灰度位图数据
        std::vector<std::pair<int, int>> rawSizes;         // 普通位图的尺寸
        ...
        for (const auto &pair : rawAtlasPos)
        {
            ...
            if (it == glyphMap.end())
                continue;
            int w = rawSizes[cp - ASCII_START].first; // 索引与codepoints顺序一致
            int h = rawSizes[cp - ASCII_START].second;
            if (w == 0 || h == 0)
                continue;
        ...
        std::string testText = "Hello, Atlas!";
        auto shaped = shaper.shape(testText);
        std::cout << "\nShaped text: " << testText << "\n";
