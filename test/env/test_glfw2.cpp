...
// Slang着色器源码 - 在一个文件中写两个着色器
const char *slangShaderSource = R"(
// 三角形绘制示例 - Slang着色器
...
// 使用slangc.exe编译着色器的函数
std::vector<uint32_t> compileShaderWithSlangc(const std::string &shaderSource,
                                              const std::string &shaderName,
                                              const std::string &slangcPath)
{
    std::cout << "使用slangc编译着色器: " << shaderName << std::endl;

    // 创建临时Slang文件
    std::string slangFile = shaderName + ".slang";
    std::string spvFile = shaderName + ".spv";

    // 1. 创建临时文件保存Slang源码
    std::ofstream out(slangFile);
    if (!out)
    {
        std::cerr << "无法创建临时文件: " << slangFile << std::endl;
        return {};
    }
    out << shaderSource;
    out.close();

    // 2. 构建命令 - 参考CMake输出的命令格式
    std::string command = slangcPath + " " + slangFile + " -target spirv" +
                          " -profile spirv_1_4" + " -emit-spirv-directly" +
                          " -fvk-use-entrypoint-name" + " -entry vertMain" +
                          " -entry fragMain" + " -o " + spvFile;
    ...
    std::cout << "✓ Slang着色器编译成功: " << shaderName << " (" << buffer.size()
              << " 字)" << std::endl;
...
int main()
{
    std::cout << "=== 三角形绘制测试 (Volk + VMA + Slang着色器) ===" << std::endl;

    // Slang编译器路径
    std::string slangcPath = "E:/mysoftware/VulkanSDK/1.4.321.1/Bin/slangc.exe";
    ...
    // 创建窗口
    GLFWwindow *window =
        glfwCreateWindow(800, 600, "Vulkan Triangle with Slang", nullptr, nullptr);
    if (!window)
    {
    ...
    try
    {
        ...
        appInfo.pApplicationName = "TriangleTest with Slang";
        ...
        // 10. 使用slangc编译着色器
        std::cout << "\n=== 使用slangc编译着色器 ===" << std::endl;
        auto shaderCode =
            compileShaderWithSlangc(slangShaderSource, "triangle_shader", slangcPath);

        if (shaderCode.empty())
        {
            throw std::runtime_error("Slang着色器编译失败");
        }
        ...
        VkPipeline graphicsPipeline;
        {
            ...
            vertShaderStageInfo.pName = "vertMain"; // Slang中的顶点着色器入口点
            ...
            fragShaderStageInfo.pName = "fragMain"; // Slang中的片段着色器入口点
