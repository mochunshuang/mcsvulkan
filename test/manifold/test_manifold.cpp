...
try
{
    ...
    auto buildMeshHardcoded = [&](std::vector<Vertex> &outVerts,
        ...
        // 屏幕坐标 → NDC 转换（返回 glm::vec2）
        auto screenToNDC = [width, height](float screenX, float screenY) -> glm::vec2 {
            float ndcX = (screenX / width) * 2.0f - 1.0f;
            float ndcY = (screenY / height) * 2.0f - 1.0f; // 顶部 → -1，底部 → 1
            return {ndcX, ndcY};
        };
        ...
        std::array<Rect, 4> rects = {
            Rect{0, 0, (float)width, 100, {1.0f, 0.0f, 0.0f}}, // 上：红
            Rect{0,
                 (float)height - 100,
                 (float)width,
                 (float)height,
                 {0.0f, 1.0f, 0.0f}},                                   // 下：绿
            Rect{0, 100, 100, (float)height - 100, {0.0f, 0.0f, 1.0f}}, // 左：蓝
            Rect{(float)width - 100,
                 100,
                 (float)width,
                 (float)height - 100,
                 {1.0f, 1.0f, 0.0f}} // 右：黄
        ...
        for (const auto &r : rects)
        {
            ...
            contour.push_back({screenToNDC(r.left, r.bottom).x,
                               screenToNDC(r.left, r.bottom).y}); // 左下
            contour.push_back({screenToNDC(r.right, r.bottom).x,
                               screenToNDC(r.right, r.bottom).y}); // 右下
            contour.push_back(
                {screenToNDC(r.right, r.top).x, screenToNDC(r.right, r.top).y}); // 右上
            contour.push_back(
                {screenToNDC(r.left, r.top).x, screenToNDC(r.left, r.top).y}); // 左上

            manifold::Polygons polygons;
            polygons.push_back(contour);

            // 挤出高度为 0，生成平面网格
            // auto manifoldObj = manifold::Manifold::Extrude(polygons, 0.0);
            auto manifoldObj = manifold::Manifold::Extrude(polygons, 0.001);

            // 获取 MeshGL 数据（单精度浮点数）
            auto meshGL = manifoldObj.GetMeshGL();

            // 提取顶点位置（vertProperties 每 numProp 个元素为一个顶点，前三个为 x, y,
            // z）
            const auto &vertProps = meshGL.vertProperties;
            uint32_t numProp = meshGL.numProp;
            uint32_t vertexCount = static_cast<uint32_t>(vertProps.size() / numProp);

            // 添加位置和颜色
            for (uint32_t v = 0; v < vertexCount; ++v)
            {
                size_t offset = v * numProp;
                float x = vertProps[offset + 0];
                float y = vertProps[offset + 1];
                // z 分量忽略（挤出高度为 0，应为 0）
                allPositions.push_back({x, y});
                allColors.push_back(r.color);
            }

            // 提取三角形索引（triVerts 每三个为一组，表示一个三角形的三个顶点索引）
            const auto &triVerts = meshGL.triVerts;
            for (uint32_t idx : triVerts)
            {
        ...
        // 组装最终的 Vertex 数组
        outVerts.reserve(allPositions.size());
        for (size_t i = 0; i < allPositions.size(); ++i)
        {
            outVerts.push_back({allPositions[i], allColors[i]});
        }
        outIndices = std::move(allIndices);

        std::cout << "Generated vertices: " << outVerts.size()
                  << ", indices: " << outIndices.size() << std::endl;
    };
    // 生成初始顶点/索引
    std::vector<Vertex> yogaVertices;
    std::vector<uint32_t> yogaIndices;
    buildMeshHardcoded(yogaVertices, yogaIndices, VkExtent2D{WIDTH, HEIGHT});
