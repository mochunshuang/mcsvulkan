...
*/
    auto buildMeshHardcoded = [&](std::vector<Vertex> &outVerts,
        ...
        // ---------- 构建阶段：使用 CrossSection API ----------
        // 外矩形尺寸（像素）
        float outerW = width - 200.0f;
        float outerH = height - 200.0f;
        // 内矩形尺寸（像素）
        float innerW = outerW - 120.0f;
        float innerH = outerH - 120.0f;

        // 创建居中正方形（默认中心在原点）
        auto outerSquare = manifold::CrossSection::Square({outerW, outerH}, true);
        auto innerSquare = manifold::CrossSection::Square({innerW, innerH}, true);

        // 将正方形从像素单位缩放并平移到正确的 Manifold 坐标位置
        // 注意：Square 默认尺寸是像素值，需要先缩放再平移
        auto scaleFactor = 2.0f / manifold::vec2(width, height);
        auto translateOffset = manifold::vec2(0.0f, 0.0f); // 居中已在原点，无需平移

        auto outerCS = outerSquare.Scale(scaleFactor).Translate(translateOffset);
        auto innerCS = innerSquare.Scale(scaleFactor).Translate(translateOffset);

        // 布尔减法：生成带孔截面
        auto finalCS = outerCS - innerCS;
        ...
        // 分别提取外矩形和最终截面的信息
        infoExtractor(outerCS, "Outer Square");
        infoExtractor(finalCS, "Final (Outer - Inner)");

        // ---------- 拉伸为 3D 网格 ----------
        auto plate = manifold::Manifold::Extrude(finalCS.ToPolygons(), 1e-4);
