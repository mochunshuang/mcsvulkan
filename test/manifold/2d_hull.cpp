...
*/
    ...
    // ------------------------------------------------------------
    // 1. Hull —— 凸包：生成覆盖多个图形的最小凸多边形
    //    演示：连接两个分离的圆，形成“药丸形”（胶囊形）截面
    // ------------------------------------------------------------
    auto buildMeshHull = [&](std::vector<Vertex> &outVerts,
                             std::vector<uint32_t> &outIndices, VkExtent2D screenSize) {
        outVerts.clear();
        outIndices.clear();
        auto scale = 2.0f / manifold::vec2(screenSize.width, screenSize.height);

        // 创建两个分离的圆：左侧圆半径 60，右侧圆半径 40（不同大小以凸显凸包效果）
        auto circleLeft =
            manifold::CrossSection::Circle(60.0f, 32).Translate({-120.0f, 0.0f});
        auto circleRight =
            manifold::CrossSection::Circle(40.0f, 32).Translate({120.0f, 0.0f});

        std::cout << "=== Hull Demo ===\n";
        std::cout << "  Left  circle: center(-120, 0), radius 60\n";
        std::cout << "  Right circle: center( 120, 0), radius 40\n";

        // 将两个圆组合为一个截面（Union），然后求凸包。
        // 注意：Hull 直接作用于 CrossSection 对象，传入多边形列表的并集即可。
        auto combined = circleLeft + circleRight;
        auto hulled = combined.Hull();

        // 也可以直接对多个多边形求凸包：
        // std::vector<manifold::CrossSection> shapes = {circleLeft, circleRight};
        // auto hulled = manifold::CrossSection::Hull(shapes);

        // 应用像素到 NDC 的缩放
        hulled = hulled.Scale(scale);

        // 赋予明亮的橙黄色，拉伸为极薄网格并提取顶面
        extrudeTopMesh(hulled, outVerts, outIndices, glm::vec3(1.0f, 0.7f, 0.1f), "Hull");
    };
    // ------------------------------------------------------------
    // 2. Hull 作用于点集 —— 从离散点生成最小凸多边形底板
    // ------------------------------------------------------------
    auto buildMeshHullFromPoints = [&](std::vector<Vertex> &outVerts,
        ...
        // 定义 6 个散点（模拟螺栓孔位）
        std::vector<manifold::vec2> points = {{-130, -80}, {130, -80}, {-160, 20},
                                              {160, 20},   {-80, 100}, {80, 100}};

        // 将每个点转为微小的圆（直径 4px），使其可见
        manifold::CrossSection markers;
        for (const auto &p : points)
        {
            auto dot = manifold::CrossSection::Circle(4.0f, 8).Translate(p);
            markers = markers.IsEmpty() ? dot : markers + dot;
        }

        // 对点集求凸包（需要将点转为简单多边形）
        // 方法：每个点构造一个退化的多边形（单点），然后 Union 后 Hull
        manifold::CrossSection pointCloud;
        for (const auto &p : points)
        {
            // 用极小半径的圆代表点，或者直接构造一个退化的三角形
            auto pt = manifold::CrossSection::Circle(0.1f, 3).Translate(p);
            pointCloud = pointCloud.IsEmpty() ? pt : pointCloud + pt;
        }
        auto hullFromPoints = pointCloud.Hull();

        // 将凸包内缩 6px，形成有均匀边距的底板轮廓
        auto plate = hullFromPoints.Offset(-6.0, manifold::CrossSection::JoinType::Round);

        // 显示底板 + 定位点
        auto result = plate + markers;
        result = result.Scale(scale);

        extrudeTopMesh(result, outVerts, outIndices, glm::vec3(0.2f, 0.8f, 0.4f),
                       "Hull from points");
    };

    // ------------------------------------------------------------
    // 3. 多图形凸包 —— 包裹圆、矩形、多边形，生成外框
    // ------------------------------------------------------------
    auto buildMeshHullMultiShape = [&](std::vector<Vertex> &outVerts,
                                       std::vector<uint32_t> &outIndices,
                                       VkExtent2D screenSize) {
        outVerts.clear();
        outIndices.clear();
        ...
        // 创建三个形状：一个圆、一个矩形、一个旋转的三角形
        auto circle = manifold::CrossSection::Circle(45.0f, 24).Translate({-110, -30});
        auto rect =
            manifold::CrossSection::Square({70.0f, 50.0f}, true).Translate({30, 80});

        // 构造一个三角形（用多边形）
        std::vector<manifold::vec2> triPts = {{-40, -50}, {40, -50}, {0, 30}};
        auto triangle = manifold::CrossSection(triPts).Rotate(30).Translate({130, -60});

        // 合并三个图形
        auto shapes = circle + rect + triangle;

        // 求凸包（直接作用于合并后的截面）
        auto hulled = shapes.Hull();

        // 对凸包进行圆角处理，使其更美观
        auto roundedHull = hulled.Offset(8.0, manifold::CrossSection::JoinType::Round)
                               .Offset(-8.0, manifold::CrossSection::JoinType::Round);

        // 显示原始形状（半透明效果用不同颜色？这里直接显示凸包）
        auto result = roundedHull.Scale(scale);
        extrudeTopMesh(result, outVerts, outIndices, glm::vec3(0.9f, 0.3f, 0.5f),
                       "Multi-shape hull");
    };
    // ------------------------------------------------------------
    // 4. 凸包组合应用 —— 连接两个轴承座，并添加安装孔
    // ------------------------------------------------------------
    auto buildMeshHullWithEars = [&](std::vector<Vertex> &outVerts,
                                     std::vector<uint32_t> &outIndices,
                                     VkExtent2D screenSize) {
        outVerts.clear();
        outIndices.clear();
        ...
        // 两个大圆（轴承座外径）
        auto bearing1 = manifold::CrossSection::Circle(60.0f, 36).Translate({-140, 0});
        auto bearing2 = manifold::CrossSection::Circle(60.0f, 36).Translate({140, 0});

        // 两个小圆（轴孔）
        auto hole1 = manifold::CrossSection::Circle(25.0f, 20).Translate({-140, 0});
        auto hole2 = manifold::CrossSection::Circle(25.0f, 20).Translate({140, 0});

        // 连接部分（凸包）
        auto hullBody = (bearing1 + bearing2).Hull();

        // 在凸包两端外侧增加安装耳（通过额外的小矩形或圆，再次凸包）
        auto earLeft = manifold::CrossSection::Circle(20.0f, 12).Translate({-200, 0});
        auto earRight = manifold::CrossSection::Circle(20.0f, 12).Translate({200, 0});
        auto bodyWithEars = (hullBody + earLeft + earRight).Hull();

        // 减去轴孔和安装孔
        auto screwHoleL = manifold::CrossSection::Circle(8.0f, 10).Translate({-200, 0});
        auto screwHoleR = manifold::CrossSection::Circle(8.0f, 10).Translate({200, 0});
        auto finalShape = bodyWithEars - hole1 - hole2 - screwHoleL - screwHoleR;

        finalShape = finalShape.Scale(scale);
        extrudeTopMesh(finalShape, outVerts, outIndices, glm::vec3(0.4f, 0.5f, 0.9f),
                       "Hull with mounting ears");
    };
    ...
    auto buildMeshHardcoded = [&](std::vector<Vertex> &outVerts,
                                  std::vector<uint32_t> &outIndices,
                                  VkExtent2D screenSize) {
        // buildMeshHull(outVerts, outIndices, screenSize); // 凸包
        // buildMeshHullFromPoints(outVerts, outIndices, screenSize);
        // buildMeshHullMultiShape(outVerts, outIndices, screenSize);
        buildMeshHullWithEars(outVerts, outIndices, screenSize);
    };
    //diff: [2d_hull] end
    // 生成初始顶点/索引
