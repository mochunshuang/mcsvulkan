...
try
{
    ...
    // 硬编码生成上下左右四个矩形，验证坐标系对齐（NDC: x右正，y上正）
    auto buildMeshHardcoded0 = [&](std::vector<Vertex> &outVerts,
        ...
        // 屏幕坐标 → NDC
        auto toNDC = [width, height](float x, float y) -> glm::vec2 {
            return {(x / width) * 2.0f - 1.0f, (y / height) * 2.0f - 1.0f};
        };

        float left = 100.0f;
        float top = 100.0f;
        float right = width - 100.0f;
        float bottom = height - 100.0f;
        float holeLeft = left + 60.0f;
        float holeTop = top + 60.0f;
        float holeRight = right - 60.0f;
        float holeBottom = bottom - 60.0f;

        // 8 个顶点：外环逆时针，内环顺时针
        glm::vec2 outer[4] = {
            toNDC(left, bottom),  // 0 左下
            toNDC(right, bottom), // 1 右下
            toNDC(right, top),    // 2 右上
            toNDC(left, top)      // 3 左上
        };
        glm::vec2 inner[4] = {
            toNDC(holeLeft, holeBottom),  // 4 左下
            toNDC(holeRight, holeBottom), // 5 右下
            toNDC(holeRight, holeTop),    // 6 右上
            toNDC(holeLeft, holeTop)      // 7 左上
        };

        // 构建顶点数组
        outVerts.resize(8);
        for (int i = 0; i < 4; ++i)
            outVerts[i] = {outer[i], {1.0f, 0.5f, 0.2f}};
        for (int i = 0; i < 4; ++i)
            outVerts[4 + i] = {inner[i], {1.0f, 0.5f, 0.2f}};

        // 构建三角形索引：将环形分解为 8 个四边形，每个拆成 2 个三角形
        // 四边形 i: 外顶点 i, 外顶点 (i+1)%4, 内顶点 (i+1)%4, 内顶点 i
        ...
        for (int i = 0; i < 4; ++i)
        {
            ...
            uint32_t out2 = (i + 1) % 4;
            uint32_t in1 = 4 + i;
            uint32_t in2 = 4 + ((i + 1) % 4);
            // 四边形拆成两个三角形（顺时针方向确保背面剔除正确，因为NDC Y翻转？）
            // 为了保证最终显示方向正确，这里用逆时针顺序（从正面看）
            addTri(out1, out2, in2);
            addTri(out1, in2, in1);
        }

        std::cout << "手工构建环形: 顶点 " << outVerts.size() << ", 三角形 "
                  << outIndices.size() / 3 << "\n";
    };
    /*
    Manifold 使用 Y 向上 的右手坐标系，其顶面三角形是逆时针（CCW）顺序。Vulkan NDC 是 Y
    向下 的左手坐标系，你设置了 frontFace =
    VK_FRONT_FACE_COUNTER_CLOCKWISE（顺时针为正面），这意味着：

    对顶点坐标做 Y 翻转（y = -y）后，原本的 CCW 三角形会自动变成 CW，正好符合 Vulkan
    的正面定义。

    无需交换任何索引。当前代码在筛选顶面后又交换了 i1 和 i2，反而将 CW 又变回了
    CCW，导致背面剔除生效，画面漆黑。
    */
    auto buildMeshHardcoded = [&](std::vector<Vertex> &outVerts,
        ...
        // 1. 像素坐标 → Manifold 坐标系（Y向上，原点中心，范围[-1,1]）
        auto toManifold = [&](float x, float y) -> manifold::vec2 {
            return {(x / width) * 2.0f - 1.0f, 1.0f - (y / height) * 2.0f};
        };

        float left = 100, top = 100, right = width - 100, bottom = height - 100;
        float hl = left + 60, ht = top + 60, hr = right - 60, hb = bottom - 60;

        // 外环 CCW，内环 CW（Manifold 多边形规则）
        manifold::SimplePolygon outer = {toManifold(left, bottom),
                                         toManifold(right, bottom),
                                         toManifold(right, top), toManifold(left, top)};
        manifold::SimplePolygon inner = {toManifold(hl, hb), toManifold(hl, ht),
                                         toManifold(hr, ht), toManifold(hr, hb)};

        manifold::CrossSection cs({outer, inner},
                                  manifold::CrossSection::FillRule::Positive);
        auto plate = manifold::Manifold::Extrude(cs.ToPolygons(), 1e-4);
        auto mesh = plate.GetMeshGL();
        ...
        std::vector<uint32_t> topFaceIndices; // 原始索引（三个一组）
