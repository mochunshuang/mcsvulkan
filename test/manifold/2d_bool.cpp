...
*/
    auto infoExtractor = [](const manifold::CrossSection &cs, const std::string &name) {
    ...
    auto extrudeTopMesh = [](const manifold::CrossSection &cs,
                             std::vector<Vertex> &outVerts,
                             std::vector<uint32_t> &outIndices, glm::vec3 color) {
        ...
        std::cout << "  Mesh: " << outVerts.size() << " verts, " << outIndices.size() / 3
                  << " tris\n";
    };
    ...
    //    将分散的圆形和矩形合并为一个整体
    // ------------------------------------------------------------
    auto buildMeshUnion = [&](std::vector<Vertex> &outVerts,
        ...
        // 创建三个独立的图形
        auto circle = manifold::CrossSection::Circle(80.0f, 32).Translate({-80.0f, 0.0f});
        auto rect = manifold::CrossSection::Square({120.0f, 80.0f}, true)
                        .Translate({50.0f, 30.0f});
        auto smallCircle =
            manifold::CrossSection::Circle(40.0f, 24).Translate({100.0f, -50.0f});

        // 使用 + 运算符合并所有截面（等同于连续 Union）
        auto cs = circle + rect + smallCircle;

        cs = cs.Scale(scale);
        std::cout << "=== Union (three shapes) ===\n";
        std::cout << "  Area: " << cs.Area() << ", NumContour: " << cs.NumContour()
                  << "\n";

        extrudeTopMesh(cs, outVerts, outIndices, glm::vec3(0.2f, 0.8f, 0.2f));
    };
    ...
    //    在一块大矩形上减去多个圆孔（模拟多孔板）
    // ------------------------------------------------------------
    auto buildMeshSubtract = [&](std::vector<Vertex> &outVerts,
        ...
        // 基板：圆角矩形
        auto base = manifold::CrossSection::Square({300.0f, 200.0f}, true)
                        .Offset(20.0f, manifold::CrossSection::JoinType::Round);

        // 要减去的孔：4 个圆 + 1 个矩形槽
        auto hole1 =
            manifold::CrossSection::Circle(30.0f, 24).Translate({-100.0f, -50.0f});
        auto hole2 =
            manifold::CrossSection::Circle(30.0f, 24).Translate({100.0f, -50.0f});
        auto hole3 =
            manifold::CrossSection::Circle(30.0f, 24).Translate({-100.0f, 50.0f});
        auto hole4 = manifold::CrossSection::Circle(30.0f, 24).Translate({100.0f, 50.0f});
        auto slot =
            manifold::CrossSection::Square({80.0f, 40.0f}, true).Translate({0.0f, 0.0f});

        // 依次减去所有孔（也可用 BatchBoolean 优化，此处展示 - 运算符）
        auto cs = base - hole1 - hole2 - hole3 - hole4 - slot;

        cs = cs.Scale(scale);
        std::cout << "=== Subtract (base with 5 holes) ===\n";
        std::cout << "  Area: " << cs.Area() << ", NumContour: " << cs.NumContour()
                  << "\n";

        extrudeTopMesh(cs, outVerts, outIndices, glm::vec3(0.8f, 0.2f, 0.2f));
    };

    // ------------------------------------------------------------
    // 3. Intersect (^) —— 提取重叠区域（模拟加强筋或裁剪保留区）
    //    两个正交矩形相交，得到它们的公共矩形区域（中心 80x80）
    // ------------------------------------------------------------
    auto buildMeshIntersect = [&](std::vector<Vertex> &outVerts,
                                  std::vector<uint32_t> &outIndices,
                                  VkExtent2D screenSize) {
        outVerts.clear();
        outIndices.clear();
        ...
        // 水平矩形
        auto horiz = manifold::CrossSection::Square({300.0f, 80.0f}, true);
        // 垂直矩形
        auto vert = manifold::CrossSection::Square({80.0f, 250.0f}, true);

        // 求交集
        auto cs = horiz ^ vert;

        cs = cs.Scale(scale);
        std::cout << "=== Intersect (cross shape) ===\n";
        std::cout << "  Area: " << cs.Area() << ", NumContour: " << cs.NumContour()
                  << "\n";

        extrudeTopMesh(cs, outVerts, outIndices, glm::vec3(0.2f, 0.2f, 0.8f));
    };

    // ------------------------------------------------------------
    // 4. BatchBoolean —— 一次性多零件融合（性能优化）
    //    对 100 个圆执行 Union，展示 BatchBoolean 比循环 + 快一个数量级
    // ------------------------------------------------------------
    auto buildMeshBatchBoolean = [&](std::vector<Vertex> &outVerts,
                                     std::vector<uint32_t> &outIndices,
                                     VkExtent2D screenSize) {
        outVerts.clear();
        outIndices.clear();
        ...
        // 生成 100 个随机位置的圆（限于 400x300 区域内）
        std::vector<manifold::CrossSection> circles;
        const int N = 100;
        circles.reserve(N);
        std::srand(42); // 固定种子以便复现
        for (int i = 0; i < N; ++i)
        {
            float x = (std::rand() % 400) - 200.0f;
            float y = (std::rand() % 300) - 150.0f;
            float r = 15.0f + (std::rand() % 20); // 半径 15~35
            circles.push_back(manifold::CrossSection::Circle(r, 16).Translate({x, y}));
        }

        // 使用 BatchBoolean 一次性合并所有圆
        auto cs = manifold::CrossSection::BatchBoolean(circles, manifold::OpType::Add);

        cs = cs.Scale(scale);
        std::cout << "=== BatchBoolean (100 circles Union) ===\n";
        std::cout << "  Area: " << cs.Area() << ", NumContour: " << cs.NumContour()
                  << "\n";
        std::cout << "  (Compared to " << (N - 1)
                  << " individual '+' operations, BatchBoolean is ~10x faster)\n";
        infoExtractor(cs, "BatchBoolean");

        extrudeTopMesh(cs, outVerts, outIndices, glm::vec3(0.9f, 0.6f, 0.1f));
    };

    // ------------------------------------------------------------
    // 5. 组合任务：Subtract 与 BatchBoolean 结合 —— 多孔板高效生成
    //    先批量 Union 所有孔，再一次性减去
    // ------------------------------------------------------------
    auto buildMeshMultiHoleOptimized = [&](std::vector<Vertex> &outVerts,
                                           std::vector<uint32_t> &outIndices,
                                           VkExtent2D screenSize) {
        outVerts.clear();
        outIndices.clear();
        ...
        // 基板
        auto base = manifold::CrossSection::Square({400.0f, 300.0f}, true)
                        .Offset(15.0f, manifold::CrossSection::JoinType::Round);

        // 生成 50 个孔
        std::vector<manifold::CrossSection> holes;
        std::srand(123);
        for (int i = 0; i < 50; ++i)
        {
            float x = (std::rand() % 360) - 180.0f;
            float y = (std::rand() % 260) - 130.0f;
            holes.push_back(manifold::CrossSection::Circle(18.0f, 12).Translate({x, y}));
        }

        // 批量合并所有孔
        auto allHoles =
            manifold::CrossSection::BatchBoolean(holes, manifold::OpType::Add);

        // 一次性减去
        auto cs = base - allHoles;

        cs = cs.Scale(scale);
        std::cout << "=== Optimized Subtract (BatchBoolean holes) ===\n";
        std::cout << "  Area: " << cs.Area() << ", NumContour: " << cs.NumContour()
                  << "\n";
        infoExtractor(cs, "Optimized Subtract (BatchBoolean holes)");
        extrudeTopMesh(cs, outVerts, outIndices, glm::vec3(0.7f, 0.3f, 0.5f));
    };
    ...
    auto buildMeshHardcoded = [&](std::vector<Vertex> &outVerts,
                                  ...
                                  VkExtent2D screenSize) {
        // 可依次测试：
        // buildMeshUnion(outVerts, outIndices, screenSize); // 合并
        // buildMeshSubtract(outVerts, outIndices, screenSize); // 打孔
        // buildMeshIntersect(outVerts, outIndices, screenSize); // 交集
        // buildMeshBatchBoolean(outVerts, outIndices, screenSize); // 批量合并
        buildMeshMultiHoleOptimized(outVerts, outIndices, screenSize); // 优化打孔
    };
    //diff: [2d_bool] end
    // 生成初始顶点/索引
    std::vector<Vertex> outVerts;
