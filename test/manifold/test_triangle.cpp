...
try
{
    ...
    auto buildMeshHardcoded = [&](std::vector<Vertex> &outVerts,
        ...
        struct Rect
        {
            float left, top, right, bottom; // 屏幕坐标（左上为原点）
            glm::vec3 color;
        };

        // 上（红）: 顶部横条
        Rect top = {0, 0, (float)width, 100, {1.0f, 0.0f, 0.0f}};
        Rect bottom = {
            0, (float)height - 100, (float)width, (float)height, {0.0f, 1.0f, 0.0f}};
        Rect left = {0, 100, 100, (float)height - 100, {0.0f, 0.0f, 1.0f}};
        Rect right = {(float)width - 100,
                      100,
                      (float)width,
                      (float)height - 100,
                      {1.0f, 1.0f, 0.0f}};

        std::array<Rect, 4> rects = {top, bottom, left, right};

        uint32_t offset = 0;
        for (const auto &r : rects)
        {
            // 将四个角点从屏幕坐标转为 NDC
            glm::vec2 p0 = screenToNDC(r.left, r.bottom);  // 左下
            glm::vec2 p1 = screenToNDC(r.right, r.bottom); // 右下
            glm::vec2 p2 = screenToNDC(r.left, r.top);     // 左上
            glm::vec2 p3 = screenToNDC(r.right, r.top);    // 右上

            outVerts.push_back({p0, r.color});
            outVerts.push_back({p1, r.color});
            outVerts.push_back({p2, r.color});
            outVerts.push_back({p3, r.color});

            /*
             NOTE: 1.在标准的Vulkan坐标系里，NDC空间中的Y轴是指向上方的
             NOTE: 2.而屏幕的坐标系通常是以左上角为原点，Y轴向下的
             差异解决：调整顶点顺序 (推荐，更符合直觉)，无需视口变换
            核心思路：承认并理解翻转的存在。既然屏幕空间会被翻转，那就在源头上（NDC空间）用顺时针（CW）
顺序定义顶点。这样经过Y轴翻转后，它们在屏幕空间恰好变成你想要的逆时针（CCW） 顺序。
Pipeline配置：保持标准设置。frontFace = VK_FRONT_FACE_COUNTER_CLOCKWISE，cullMode =
VK_CULL_MODE_BACK_BIT
            */
            // 用顺时针（CW） 顺序定义顶点。
            // 这样经过Y轴翻转后，它们在屏幕空间恰好变成你想要的逆时针（CCW） 顺序。
            outIndices.push_back(offset + 0);
            outIndices.push_back(offset + 1);
            outIndices.push_back(offset + 2); // 三角形1: 0-1-2 (NDC顺时针)

            outIndices.push_back(offset + 1);
            outIndices.push_back(offset + 3);
            outIndices.push_back(offset + 2); // 三角形2: 1-3-2 (NDC顺时针)
            offset += 4;
        }
    };
    // 生成初始顶点/索引
