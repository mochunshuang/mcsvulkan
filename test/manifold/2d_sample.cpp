...
*/
    ...
    //diff: [2d_sample] start
    // ---------- 通用顶面提取与网格构建辅助函数（避免重复代码）----------
    auto extrudeTopMesh =
        [](const manifold::CrossSection &cs, std::vector<Vertex> &outVerts,
           std::vector<uint32_t> &outIndices, glm::vec3 color, const std::string &name) {
            // 打印截面基本信息（合并到此处）
            std::cout << "=== Extruding: " << name << " ===\n";
            ...
            const double eps = 1e-6, thickness = 1e-4;
            const auto &triVerts = mesh.triVerts;
            std::vector<uint32_t> topIndices;
            for (size_t i = 0; i < triVerts.size(); i += 3)
            {
                uint32_t i0 = triVerts[i], i1 = triVerts[i + 1], i2 = triVerts[i + 2];
                if (std::abs(positions[i0].z - thickness) < eps &&
                    std::abs(positions[i1].z - thickness) < eps &&
                    std::abs(positions[i2].z - thickness) < eps)
                {
                    topIndices.insert(topIndices.end(), {i0, i1, i2});
                }
            }

            std::unordered_map<uint32_t, uint32_t> oldToNew;
            for (uint32_t oldIdx : topIndices)
            {
            ...
            std::cout << "  Mesh: " << outVerts.size() << " verts, "
                      << outIndices.size() / 3 << " tris\n";
        };

    // ============================================================================
    // 第二部分 & 第三部分：组合任务独立函数（每个任务可单独测试）
    // ============================================================================

    // ------------------------------------------------------------
    // 任务 1：制作圆角矩形板
    // 组合链：Square → Offset
    // 解释：Square 生成直角矩形，Offset 正偏移并将尖角膨胀为圆角
    // ------------------------------------------------------------
    auto buildMeshTask1_RoundedRect = [&](std::vector<Vertex> &outVerts,
                                          std::vector<uint32_t> &outIndices,
                                          VkExtent2D screenSize) {
        outVerts.clear();
        outIndices.clear();
        auto scale = 2.0f / manifold::vec2(screenSize.width, screenSize.height);

        float w = 300.0f, h = 200.0f, r = 30.0f;
        auto rect = manifold::CrossSection::Square({w, h}, true);
        auto rounded = rect.Offset(r, manifold::CrossSection::JoinType::Round);

        rounded = rounded.Scale(scale);
        extrudeTopMesh(rounded, outVerts, outIndices, glm::vec3(0.2f, 0.7f, 0.3f),
                       "Task1: Rounded Rect");
    };

    // ------------------------------------------------------------
    // 任务 2：制作带均匀边距的镂空
    // 组合链：Square → Offset(-) → Subtract
    // 解释：外框矩形向内偏移形成内框，相减得到壁厚均匀的边框
    // ------------------------------------------------------------
    auto buildMeshTask2_HollowFrame = [&](std::vector<Vertex> &outVerts,
                                          std::vector<uint32_t> &outIndices,
                                          VkExtent2D screenSize) {
        outVerts.clear();
        outIndices.clear();
        ...
        float outerW = 400.0f, outerH = 280.0f, thickness = 25.0f;
        auto outer = manifold::CrossSection::Square({outerW, outerH}, true);
        auto inner = outer.Offset(-thickness, manifold::CrossSection::JoinType::Square);
        auto frame = outer - inner;

        // 可选：加一个中心圆孔展示多层镂空
        auto hole = manifold::CrossSection::Circle(40.0f, 24);
        frame = frame - hole;

        frame = frame.Scale(scale);
        extrudeTopMesh(frame, outVerts, outIndices, glm::vec3(0.1f, 0.5f, 0.8f),
                       "Task2: Hollow Frame");
    };

    // ------------------------------------------------------------
    // 任务 3：在一块板上阵列打孔
    // 组合链：Circle → Translate → BatchBoolean (这里用循环 Subtract 模拟)
    // 解释：基板为 Square，孔为 Circle，通过 Translate 阵列，最后批量减去
    // ------------------------------------------------------------
    auto buildMeshTask3_HoleArray = [&](std::vector<Vertex> &outVerts,
                                        std::vector<uint32_t> &outIndices,
                                        VkExtent2D screenSize) {
        outVerts.clear();
        outIndices.clear();
        ...
        // 基板
        auto plate = manifold::CrossSection::Square({450.0f, 300.0f}, true);

        // 生成 5x3 孔阵列
        float spacingX = 80.0f, spacingY = 70.0f;
        int cols = 5, rows = 3;
        ...
        for (int i = 0; i < cols; ++i)
        {
            for (int j = 0; j < rows; ++j)
            {
                ...
                auto hole = manifold::CrossSection::Circle(12.0f, 16).Translate({x, y});
                plate = plate - hole;
            }
        }

        plate = plate.Scale(scale);
        extrudeTopMesh(plate, outVerts, outIndices, glm::vec3(0.8f, 0.6f, 0.2f),
                       "Task3: Hole Array");
    };

    // ------------------------------------------------------------
    // 任务 4：制作异形倒角字体（模拟） 先构建实心形状，再挖孔
    // 组合链：Text轮廓 → Offset → Simplify
    // 说明：Manifold 无内置 Text，故用复杂多边形模拟文字轮廓，
    //       通过 Offset 正偏移将尖角变圆，再 Simplify 清理碎片。
    // ------------------------------------------------------------
    auto buildMeshTask4_ChamferText = [&](std::vector<Vertex> &outVerts,
                                          std::vector<uint32_t> &outIndices,
                                          VkExtent2D screenSize) {
        outVerts.clear();
        outIndices.clear();
        ...
        // 外轮廓（逆时针）
        std::vector<manifold::vec2> outerPoly = {{-50, -80}, {50, -80}, {0, 80}};
        // 内孔轮廓（顺时针）
        std::vector<manifold::vec2> innerPoly = {{-30, -40}, {0, 20}, {30, -40}};

        // 使用复合多边形构造（自动处理内外关系）
        std::vector<manifold::SimplePolygon> polys = {outerPoly, innerPoly};
        auto textShape = manifold::CrossSection(polys);

        // 圆角处理与碎片清理
        auto roundedText = textShape.Offset(6.0, manifold::CrossSection::JoinType::Round);
        auto cleanText = roundedText.Simplify(0.5);

        if (cleanText.IsEmpty())
        {
            std::cerr << "Task4: Resulting cross-section is still empty!\n";
            return;
        }

        cleanText = cleanText.Scale(scale);
        extrudeTopMesh(cleanText, outVerts, outIndices, glm::vec3(0.9f, 0.4f, 0.6f),
                       "Task4: Chamfer Text (simulated)");
    };

    // ------------------------------------------------------------
    // 任务 5：制作连接两个圆的“骨头”形（药丸形）
    // 组合链：Circle(A) + Circle(B) → Hull
    // 解释：两个分离的圆通过 Hull 生成覆盖它们的最小凸包，形成平滑连接杆
    // ------------------------------------------------------------
    auto buildMeshTask5_BoneShape = [&](std::vector<Vertex> &outVerts,
                                        std::vector<uint32_t> &outIndices,
                                        VkExtent2D screenSize) {
        outVerts.clear();
        outIndices.clear();
        ...
        auto circleLeft = manifold::CrossSection::Circle(50.0f, 32).Translate({-120, 0});
        auto circleRight = manifold::CrossSection::Circle(40.0f, 32).Translate({120, 0});

        auto combined = circleLeft + circleRight;
        auto bone = combined.Hull();

        // 可选：添加两个小圆孔在两端
        auto holeL = manifold::CrossSection::Circle(20.0f, 20).Translate({-120, 0});
        auto holeR = manifold::CrossSection::Circle(15.0f, 20).Translate({120, 0});
        bone = bone - holeL - holeR;

        bone = bone.Scale(scale);
        extrudeTopMesh(bone, outVerts, outIndices, glm::vec3(1.0f, 0.6f, 0.1f),
                       "Task5: Bone Shape (Hull)");
    };

    // ------------------------------------------------------------
    // 任务 6：分离排版中的多个零件
    // 组合链：Compose → Decompose
    // 解释：将多个图形合并后再分解成独立轮廓，展示拆分后各自独立的能力
    // ------------------------------------------------------------
    auto buildMeshTask6_DecomposeLayout = [&](std::vector<Vertex> &outVerts,
                                              std::vector<uint32_t> &outIndices,
                                              VkExtent2D screenSize) {
        outVerts.clear();
        outIndices.clear();
        ...
        // 创建三个互不相连的形状
        auto rect = manifold::CrossSection::Square({60, 60}, true).Translate({-100, 0});
        auto circle = manifold::CrossSection::Circle(40, 24).Translate({100, 0});
        auto triangle =
            manifold::CrossSection({{-40, -40}, {40, -40}, {0, 40}}).Translate({0, 100});

        // 合并为一个整体截面
        auto composed = rect + circle + triangle;

        // 分解为独立零件
        auto parts = composed.Decompose();

        // 为了可视化，我们将分解后的零件分别赋予不同颜色并平移排列
        // 这里我们只取第一个零件显示，或者将所有零件平移后合并显示
        manifold::CrossSection layout;
        float offsetX = -200.0f;
        for (size_t i = 0; i < parts.size(); ++i)
        {
            auto movedPart = parts[i].Translate({offsetX + i * 130.0f, -50.0f});
            layout = (i == 0) ? movedPart : layout + movedPart;
        }

        layout = layout.Scale(scale);
        extrudeTopMesh(layout, outVerts, outIndices, glm::vec3(0.5f, 0.8f, 0.3f),
                       "Task6: Decompose Layout");
    };

    // ------------------------------------------------------------
    // 任务 7：生成不规则的艺术边框（波浪边缘）
    // 组合链：Square → Warp (正弦函数)
    // 解释：通过 Warp 遍历顶点，根据角度或坐标修改 Y 值，产生周期波浪效果
    // ------------------------------------------------------------
    auto buildMeshTask7_WavyBorder = [&](std::vector<Vertex> &outVerts,
                                         std::vector<uint32_t> &outIndices,
                                         VkExtent2D screenSize) {
        outVerts.clear();
        outIndices.clear();
        ...
        // 矩形尺寸与细分
        float w = 400.0f, h = 250.0f;
        int samplesPerSide = 40; // 每边采样数，越多波浪越平滑
        ...
        for (int i = 0; i < samplesPerSide; ++i)
        {
            ...
            points.push_back({-hw + w * t, -hh});
        }
        // 右边：下 → 上
        for (int i = 0; i < samplesPerSide; ++i)
        {
            float t = i / (float)samplesPerSide;
            points.push_back({hw, -hh + h * t});
        }
        // 顶边：右 → 左
        for (int i = 0; i < samplesPerSide; ++i)
        {
        ...
        auto baseRect = manifold::CrossSection(points);

        // Warp：在 Y 方向叠加正弦波浪（完全参照参考代码风格）
        auto wavy = baseRect.Warp([](manifold::vec2 &v) {
            v.y += 15.0f * std::sin(v.x * 0.05f); // 幅度15px，频率0.05 rad/px
        });

        // 向内偏移形成边框（带圆角）
        auto wavyFrame = wavy.Offset(-12.0, manifold::CrossSection::JoinType::Round);

        wavyFrame = wavyFrame.Scale(scale);
        extrudeTopMesh(wavyFrame, outVerts, outIndices, glm::vec3(0.3f, 0.7f, 0.9f),
                       "Task7: Wavy Border");
    };

    // ------------------------------------------------------------
    // 在 buildMeshHardcoded 中切换测试用例
    // ------------------------------------------------------------
    auto buildMeshHardcoded = [&](std::vector<Vertex> &outVerts,
                                  std::vector<uint32_t> &outIndices,
                                  VkExtent2D screenSize) {
        // 取消注释想要测试的任务
        // buildMeshTask1_RoundedRect(outVerts, outIndices, screenSize);
        // buildMeshTask2_HollowFrame(outVerts, outIndices, screenSize);
        // buildMeshTask3_HoleArray(outVerts, outIndices, screenSize);
        // buildMeshTask4_ChamferText(outVerts, outIndices, screenSize);
        // buildMeshTask5_BoneShape(outVerts, outIndices, screenSize);
        // buildMeshTask6_DecomposeLayout(outVerts, outIndices, screenSize);
        buildMeshTask7_WavyBorder(outVerts, outIndices, screenSize);
    };
