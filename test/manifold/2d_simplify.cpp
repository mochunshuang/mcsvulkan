...
*/
        ...
        for (uint32_t oldIdx : topIndices)
        {
            if (oldToNew.find(oldIdx) == oldToNew.end())
            {
                ...
                outVerts.push_back({{positions[oldIdx].x, -positions[oldIdx].y},
                                    glm::vec3(0.2f, 0.6f, 0.9f)}); // 修整模块统一颜色
            }
    ...
    // 1. Offset 正偏移 —— 圆角矩形（任务1：Square → Offset 实现圆角）
    auto buildMeshOffsetRoundRect = [&](auto &V, auto &I, auto S) {
        /* 上述实现 1 */
        V.clear();
        I.clear();
        auto [w, h] = S;
        auto scale = 2.0f / manifold::vec2(w, h);
        auto rect = manifold::CrossSection::Square({300.0f, 200.0f}, true);
        auto cs =
            rect.Offset(30.0f, manifold::CrossSection::JoinType::Round).Scale(scale);
        std::cout << "=== Offset RoundRect ===\nArea: " << cs.Area()
                  << ", NumVert: " << cs.NumVert() << "\n";
        extrudeTopMesh(cs, V, I);
    };
    // 2. Offset 负偏移 —— 均匀壁厚边框（任务2：Square → Offset(-) → 相减）
    auto buildMeshOffsetBorder = [&](auto &V, auto &I, auto S) {
        V.clear();
        I.clear();
        auto [w, h] = S;
        auto scale = 2.0f / manifold::vec2(w, h);
        auto outer = manifold::CrossSection::Square({300.0f, 200.0f}, true);
        auto inner = outer.Offset(-25.0f, manifold::CrossSection::JoinType::Miter);
        auto cs = (outer - inner).Scale(scale);
        std::cout << "=== Offset Border ===\nArea: " << cs.Area()
                  << ", NumContour: " << cs.NumContour() << "\n";
        extrudeTopMesh(cs, V, I);
    };
    // 3. Offset 不同连接类型对比 + Simplify 清理碎面
    auto buildMeshOffsetSimplify = [&](auto &V, auto &I, auto S) {
        V.clear();
        I.clear();
        auto [w, h] = S;
        auto scale = 2.0f / manifold::vec2(w, h);
        std::vector<manifold::vec2> starPoints;
        const int pts = 10;
        const float outerR = 150.0f, innerR = 70.0f;
        for (int i = 0; i < pts * 2; ++i)
        {
            float r = (i % 2 == 0) ? outerR : innerR;
            float ang = i * manifold::kPi / pts;
            starPoints.push_back({r * std::cos(ang), r * std::sin(ang)});
        }
        auto star = manifold::CrossSection(starPoints);
        auto cs = star.Offset(40.0f, manifold::CrossSection::JoinType::Round)
                      .Simplify(0.5f)
                      .Scale(scale);
        std::cout << "=== Offset Star + Simplify ===\nArea: " << cs.Area()
                  << ", NumVert: " << cs.NumVert() << "\n";
        extrudeTopMesh(cs, V, I);
    };
    // 4. Offset 内缩倒角示例（制作内倒角孔）
    auto buildMeshOffsetChamfer = [&](auto &V, auto &I, auto S) {
        V.clear();
        I.clear();
        auto [w, h] = S;
        auto scale = 2.0f / manifold::vec2(w, h);
        auto base = manifold::CrossSection::Square({200.0f, 200.0f}, true);
        auto hole = manifold::CrossSection::Circle(60.0f, 32);
        auto chamferedHole =
            hole.Offset(-10.0f, manifold::CrossSection::JoinType::Square);
        auto cs = (base - chamferedHole).Scale(scale);
        std::cout << "=== Offset Square Join (Chamfer) ===\nArea: " << cs.Area() << "\n";
        extrudeTopMesh(cs, V, I);
    };

    // 在 buildMeshHardcoded 中切换想看的修整效果
    auto buildMeshHardcoded = [&](std::vector<Vertex> &outVerts,
                                  std::vector<uint32_t> &outIndices,
                                  VkExtent2D screenSize) {
        // 可依次尝试以下四种典型修整案例：
        // buildMeshOffsetRoundRect(outVerts, outIndices, screenSize); // 圆角矩形
        // buildMeshOffsetBorder(outVerts, outIndices, screenSize); // 均匀边框
        // buildMeshOffsetSimplify(outVerts, outIndices, screenSize); // 星形偏移+清理
        buildMeshOffsetChamfer(outVerts, outIndices, screenSize); // 内倒角孔
    };
    //diff: [2d_simplify] end
    // 生成初始顶点/索引
    std::vector<Vertex> outVerts;
    std::vector<uint32_t> outIndices;
