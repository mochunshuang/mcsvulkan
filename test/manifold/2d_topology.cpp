...
*/
    ...
    // ------------------------------------------------------------
    // 1. Decompose —— 分离独立零件（用于分别赋予不同颜色/高度）
    //    先 Union 两个不相连的图形，再拆分为独立组件
    // ------------------------------------------------------------
    auto buildMeshDecompose = [&](std::vector<Vertex> &outVerts,
        ...
        // 创建两个不相连的图形：左侧圆，右侧矩形
        auto circle =
            manifold::CrossSection::Circle(80.0f, 32).Translate({-120.0f, 0.0f});
        auto rect = manifold::CrossSection::Square({100.0f, 100.0f}, true)
                        .Translate({120.0f, 0.0f});

        // 通过 Union 合并（拓扑上仍为分离状态）
        auto combined = circle + rect;

        // 分解为独立轮廓
        auto components = combined.Decompose();
        std::cout << "=== Decompose Demo ===\n";
        std::cout << "  Decomposed into " << components.size()
                  << " separate components\n\n";

        // 为不同组件赋予不同颜色（模拟分别拉伸不同高度）
        glm::vec3 colors[2] = {{1.0f, 0.5f, 0.0f}, {0.0f, 0.6f, 1.0f}};
        for (size_t i = 0; i < components.size(); ++i)
        {
            auto comp = components[i].Scale(scale);
            extrudeTopMesh(comp, outVerts, outIndices, colors[i % 2],
                           "Component " + std::to_string(i));
        }
    };

    // ------------------------------------------------------------
    // 2. Compose —— 重新组合独立零件（手动组装多个截面）
    //    先分解一个组合图形，再通过 Compose 合并子集
    // ------------------------------------------------------------
    auto buildMeshCompose = [&](std::vector<Vertex> &outVerts,
                                std::vector<uint32_t> &outIndices,
                                VkExtent2D screenSize) {
        outVerts.clear();
        outIndices.clear();
        ...
        // 创建三个独立图形
        auto circle1 =
            manifold::CrossSection::Circle(50.0f, 24).Translate({-100.0f, 0.0f});
        auto circle2 =
            manifold::CrossSection::Circle(50.0f, 24).Translate({100.0f, 0.0f});
        auto centerRect =
            manifold::CrossSection::Square({80.0f, 80.0f}, true).Translate({0.0f, 0.0f});

        // 先通过 Union 合并（演示 Compose 也可直接组合未合并的截面）
        auto combined = circle1 + circle2 + centerRect;
        auto parts = combined.Decompose(); // 获得三个独立组件

        std::cout << "=== Compose Demo ===\n";
        std::cout << "  Original combined shape has " << parts.size() << " components\n";

        // 选择前两个组件（左侧圆 + 中心矩形），用 Compose 重组
        std::vector<manifold::CrossSection> selected = {parts[0], parts[2]};
        auto recomposed = manifold::CrossSection::Compose(selected);

        // 也可以直接平移后再组合（演示手动组装）
        auto movedCircle = parts[1].Translate({0.0f, 80.0f}); // 把右侧圆上移
        selected.push_back(movedCircle);
        auto finalComposed = manifold::CrossSection::Compose(selected);

        finalComposed = finalComposed.Scale(scale);
        extrudeTopMesh(finalComposed, outVerts, outIndices, glm::vec3(0.8f, 0.4f, 0.8f),
                       "Composed (2 components + moved)");
    };
    ...
    auto buildMeshHardcoded = [&](std::vector<Vertex> &outVerts,
                                  std::vector<uint32_t> &outIndices,
                                  VkExtent2D screenSize) {
        // buildMeshDecompose(outVerts, outIndices, screenSize); // 分离独立零件
        buildMeshCompose(outVerts, outIndices, screenSize); // 重新组合
    };
    //diff: [2d_topology] end
    // 生成初始顶点/索引
