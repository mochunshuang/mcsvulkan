...
namespace mcs::vulkan::ecs
{
    ...
    struct soa_memory
    {
        ...
        static consteval auto get_field_by_name(static_string name) -> size_type
        {
            ...
            if (idx == ~0u)
                throw std::meta::exception{"not find field",
      ...
      public:
            ...
            : capacity_{capacity}, alloc_{std::move(alloc)}
        {
            if (capacity > 0)
            {
                template for (constexpr auto I : std::views::indices(ptr_members.size()))
                {
                    ...
                    using FieldType =
                        std::remove_reference_t<decltype(field)>::value_type;
        ...
        constexpr decltype(auto) view(this auto &&self) noexcept
        {
            return std::views::zip(std::span{
                std::forward<decltype(self)>(self).template field<name>().data(),
                self.capacity_}...);
        }
