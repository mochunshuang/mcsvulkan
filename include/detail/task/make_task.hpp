...
namespace mcs::vulkan::task
{
    ...
    namespace detail
    {
        ...
        {
            auto candidates =
                std::vector<schedulable_task>{schedulable_task{genTask()}...};
            ...
            std::vector<Edge> all_edge = [&]() {
                ...
                for (auto &candidate : candidates)
                {
                    ...
                    if (candidate.fixed.has_value())
                    {
                        ...
                        push_edge_if_not_find(
                            {.from = anchor, .to = cur_task, .gap = gap});
                        push_edge_if_not_find(
                            {.from = cur_task, .to = anchor, .gap = -gap});
                    }
                    else
                    {
                        for (auto pre : candidate.befores)
                            push_edge_if_not_find(
                                {.from = pre, .to = cur_task, .gap = 1});
                        for (auto post : candidate.afters)
                            push_edge_if_not_find(
                                {.from = cur_task, .to = post, .gap = 1});
                    }
                }
                return edge;
            }();
            ...
            for (size_t k = 1; k < order.size(); ++k)
            {
                ...
                if (fixed_info[prev] && fixed_info[curr] && dist[prev] == dist[curr])
                {
                    ...
                    throw std::meta::exception{
                        std::format(
                            "Fixed tasks '{}' and '{}' conflict at same position {}",
            ...
            consteval
            {
                ...
                for (const task_info &spec : task_sequence)
                {
                    nsdms.push_back(std::meta::data_member_spec(
                        spec.function, {.name = spec.name.view()}));
                }
    ...
    struct make_task<init_task<task...>, genTask...>
        : detail::gen_task<std::array<task_info, sizeof...(task)>{task...},
                           genTask...>::type
    {
        using gen_type =
        ...
        static constexpr auto task_sequence_string()
        {
            return gen_type::task_sequence_string();
        }
        static constexpr auto task_sequence_string_detail()
        {
            return gen_type::task_sequence_detail_string();
        }
        template <size_t I>
