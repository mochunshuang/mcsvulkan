...
namespace mcs::vulkan::font
{
    namespace detail
    {
        template <typename FontSelector>
        static constexpr FontSelector::select_result_type select_font_with_callback(
            FontSelector &selector, char32_t codepoint, hb_script_t script)
        {
            auto [font, lang] = selector.selectFont(codepoint, script);
            if (lang == nullptr)
                lang = FontSelector::getFallbackLanguage(script, font);
...
run；否则，即使方向一致，也需要进一步按脚本或字体支持拆分 run，回退到其他字体。
     */
    template <typename FontSelector>
        requires(requires(FontSelector &selector, char32_t codepoint,
                          hb_script_t script) {
            detail::select_font_with_callback(selector, codepoint, script);
            typename FontSelector::select_result_type;
            typename FontSelector::font_context_type;
        })
    constexpr static auto assign_fonts(const bidi::visual_result &script_runs,
                                       FontSelector &selector)
    {
        using shape_info_type = shape_info<typename FontSelector::font_context_type>;
        using shape_run_type = shape_run<shape_info_type>;
        std::vector<shape_run_type> result;
             ...
             const auto &bidi_run : script_runs.visual_bidi_runs)
        {
            ...
            std::vector<shape_info_type> runs;
