...
#include "harfbuzz/tag_to_language.hpp"
...
namespace mcs::vulkan::font
{
    ...
    class GenFontSelector
    {
        ...
        {
            using harfbuzz::tag_to_language;
            ...
            {
                return {.font = font, .language = tag ? tag_to_language(*tag) : nullptr};
            }
      ...
      public:
        ...
        using language_type = harfbuzz::language_type;
        ...
        */
        ...
        constexpr GenFontSelector(FontFactory *factory, hb_tag_t preference) noexcept
            : factory_{factory}, language_{harfbuzz::make_language_type(preference)}
        {
        }
        constexpr GenFontSelector(FontFactory *factory,
                                  const std::string_view &bcp47) noexcept
            : GenFontSelector(factory, harfbuzz::bcp47_to_tag(bcp47))
        {
        }
        ...
        constexpr auto &&setPreferenceTag(this auto &&self,
                                          hb_tag_t preferenceTag) noexcept
        {
            self.language_ = harfbuzz::make_language_type(preferenceTag);
            return std::forward<decltype(self)>(self);
        }
        // NOTE: 语言 地区
        constexpr auto &&setPreferenceLanguage(this auto &&self,
                                               const std::string_view &bcp47) noexcept
        {
            self.language_ = harfbuzz::make_language_type(harfbuzz::bcp47_to_tag(bcp47));
            return std::forward<decltype(self)>(self);
        }
        constexpr auto &&load(this auto &&self, FontInfo info)
        {
            const FontContext *newFont = self.factory_->make(std::move(info));
            mcs::vulkan::MCS_ASSERT(newFont != nullptr);
            self.selectable_.emplace_back(newFont);
            return std::forward<decltype(self)>(self);
        }

        constexpr auto &&load(this auto &&self, std::vector<FontInfo> infos)
        {
            for (auto &info : infos)
                self.load(std::move(info));
            return std::forward<decltype(self)>(self);
        }
        ...
        constexpr auto &&setCandidate(this auto &&self,
                                      std::vector<FontInfo> candidate) noexcept
        {
            self.candidate_ = std::move(candidate);
            return std::forward<decltype(self)>(self);
        }
        ...
        constexpr void setNotdefFont(this auto &&self,
                                     const FontContext *newFont) noexcept
        {
            self.notdefFont_ = newFont;
            return std::forward<decltype(self)>(self);
        }
        constexpr void setNotdefFont(this auto &&self, int index) noexcept
        {
            self.notdefFont_ = self.selectable_[index];
            assert((self.notdefFont_->glyph_index_to_glyphs.contains(0)));
            return std::forward<decltype(self)>(self);
        }
